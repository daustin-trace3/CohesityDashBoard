// vCenter Governance — ported from frontend/src/pages/vcenter/VcGovernancePage.jsx.
import { ClipboardCheck, GitCompareArrows, Wrench, HardDrive } from '../icons.jsx';
import {
...
function DriftSection({ rows, dataAvailable }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><GitCompareArrows size={15} className="text-brand" /> ESXi Configuration Drift</p>
...
function ToolsSection({ rows }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Wrench size={15} className="text-brand" /> Outdated VMware Tools</p>
...
function OrphanSection({ rows }) {
  ...
  return (
    <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><HardDrive size={15} className="text-brand" /> Orphaned VMDKs</p>
      <p className="text-[11px] text-ink-faint mb-3">
...
export default function VcGovernancePage() {
  ...
  const load = React.useCallback(() => apiFetch('/vcenter/governance')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ drift: [], outdatedTools: [], orphans: [], orphanBytes: 0, driftDataAvailable: false })), []);
