...
describe('install-from-url policy', () => {
  const post = (body) => request(app).post('/api/plugins/install-from-url').set('x-api-key', API_KEY).send(body);

  it('refuses plain http', async () => {
    const res = await post({ url: 'http://marketplace.austihome.com/files/x.iccplugin' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/https/);
  });

  it('refuses a host that is not an allowed plugin source', async () => {
    const res = await post({ url: 'https://evil.example/files/x.iccplugin' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/not an allowed plugin source/);
  });

  it('refuses loopback and metadata targets even when the allowlist is wide open', async () => {
    process.env.PLUGIN_INSTALL_ALLOWED_HOSTS = '*';
    for (const url of ['https://127.0.0.1:3002/x', 'https://localhost/x', 'https://169.254.169.254/latest/meta-data', 'https://[::1]/x']) {
      const res = await post({ url });
      expect(res.status, url).toBe(400);
      expect(res.body.error, url).toMatch(/not allowed/);
    }
  });

  it('refuses credentials in the URL', async () => {
    const res = await post({ url: 'https://user:pw@marketplace.austihome.com/files/x.iccplugin' });
    expect(res.status).toBe(400);
  });
});
