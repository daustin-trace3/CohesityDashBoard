...
//
// Copied VERBATIM from backend/db/migrations/vcenter.js, same scope id
// 'vcenter' — schema_migrations already has scope='vcenter' rows on any
// instance that ran the built-in platform, so these are skipped on install
// and existing production data is adopted intact.
module.exports = [
  {
    version: 1,
    up(db) {
  ...
  },
  {
    version: 8,
    up(db) {
      // Site capacity: manual cluster/datastore -> site mapping, hourly
      // per-cluster / per-datastore / per-VM capacity history (365d / 365d /
      // 90d retention) feeding the Site Capacity, Trends and Failover
      // Explorer pages.
      db.exec(`
        ...
        CREATE TABLE IF NOT EXISTS vcenter_capacity_history (
          ...
          vcpu_allocated INTEGER, vmem_mb_allocated INTEGER,
  ...
  },
  {
    ...
    up(db) {
      ...
      `);
      // The host may already have added this column (built-in migration v10
      // shares the 'vcenter' scope); SQLite has no ADD COLUMN IF NOT EXISTS.
      const cols = db.prepare('PRAGMA table_info(vcenter_vcenters)').all().map((c) => c.name);
      if (!cols.includes('owner_tag_category')) db.exec('ALTER TABLE vcenter_vcenters ADD COLUMN owner_tag_category TEXT;');
    },
  },
];
