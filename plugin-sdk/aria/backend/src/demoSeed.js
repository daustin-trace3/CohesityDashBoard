...
//
// Ported from backend/demo/generators/aria.js. ALL inserts here run ONLY
// behind the DASHBOARD_DEMO==='1' gate — see seedAriaDemo() below, called
...
// directly, and issue-history reconcile is repointed at this plugin's own
// ./issues (threaded with coreApi) instead of the host's ariaIssues module.
//
// DEVIATION FROM THE BUILT-IN's wipe strategy: aria_instances itself is
// NEVER wiped/deleted (it is the user-facing connection table — an admin
// could register a real Aria instance on a demo instance and that must
...
// (aria_deployments/requests/endpoints/... scoped by instance_id) are wiped
// before reseeding, children before parents — any independently-registered
// real instance's data is untouched.
const { randInt, pick, chance, rngFor } = require('./demoRng');
...
// Children->parents, scoped by instance_id — aria_instances itself is NEVER
// wiped (see module header).
const DEMO_CHILD_TABLES = [
  'aria_deployment_resources', 'aria_metrics_history', 'aria_blueprints',
  'aria_flavor_mappings', 'aria_image_mappings', 'aria_images',
  'aria_approvals', 'aria_runs', 'aria_catalog_sources', 'aria_projects',
  'aria_endpoints', 'aria_requests', 'aria_deployments',
];

function seedAria(db, { now, encrypt, coreApi }) {
  db.prepare(`
  ...
  // reseeds (aria_instances is never deleted — see module header).
  const upsertInstance = db.prepare(`
    INSERT INTO aria_instances (name, host, username, domain, encrypted_credentials, ssl_verify,
      ...
      last_poll_status, last_poll_error, last_poll_at)
    VALUES (?, ?, 'demo-viewer', NULL, ?, 0, 15, ?, ?, 1, ?, ?, ?, ?, 'success', NULL, ?)
    ON CONFLICT(name) DO UPDATE SET
      host = excluded.host, encrypted_credentials = excluded.encrypted_credentials,
      version = excluded.version, api_version = excluded.api_version,
      cert_subject = excluded.cert_subject, cert_issuer = excluded.cert_issuer,
      cert_valid_from = excluded.cert_valid_from, cert_valid_to = excluded.cert_valid_to,
      last_poll_status = excluded.last_poll_status, last_poll_error = excluded.last_poll_error,
      last_poll_at = excluded.last_poll_at, updated_at = datetime('now')
  `);
  const getInstanceId = db.prepare('SELECT id FROM aria_instances WHERE name = ?');
  ...
  const instances = FIXTURE_INSTANCES.map((inst) => {
    ...
    upsertInstance.run(
      ...
      new Date(now - randInt(rng, 2, 14) * 60000).toISOString()
    );
    const instanceId = getInstanceId.get(inst.name).id;
  ...
  const fixtureIds = instances.map((i) => i.instanceId);
  ...
  const depStmt = db.prepare(`
    INSERT INTO aria_deployments (instance_id, deployment_id, name, project_name, status,
      created_by, created_at_src, updated_at_src, lease_expire_at, resource_count, raw_status_detail, owned_by)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const reqStmt = db.prepare(`
    INSERT OR IGNORE INTO aria_requests (instance_id, request_id, deployment_id, name, status,
      requested_by, created_at_src, updated_at_src, detail)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const projStmt = db.prepare(`
    INSERT INTO aria_projects (instance_id, project_id, name, description)
    VALUES (?, ?, ?, ?)
  `);
  const catStmt = db.prepare(`
    INSERT INTO aria_catalog_sources (instance_id, source_id, name, type, items_imported,
      items_found, last_import_at, last_import_errors)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const bpStmt = db.prepare(`
    INSERT INTO aria_blueprints (instance_id, blueprint_id, name, project_name, status, updated_at_src, image_refs)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  ...
  const totals = {
    instances: 0, deployments: 0, requests: 0, endpoints: 0, projects: 0, catalogSources: 0,
    runs: 0, approvals: 0, images: 0, imageMappings: 0, flavors: 0, blueprints: 0,
  };
  let depIdSeq = 1000;
  let reqIdSeq = 5000;
  let runIdSeq = 9000;
  let apprIdSeq = 3000;
  const firstDeps = []; // for the Server 360 join fixture below
  ...
  for (const inst of instances) {
    ...
    projects.forEach((name, idx) => {
      projStmt.run(instanceId, `proj-${idx + 1}`, name, `${name} workloads`);
      totals.projects += 1;
    });
    ...
    for (let d = 0; d < deploymentCount; d++) {
      ...
      depStmt.run(instanceId, deploymentId, name, project, status,
        ...
        failed ? 'Provisioning failed: resource quota exceeded' : null,
    ...
    if (deployments.length) firstDeps.push({ instanceId, ...deployments[0] });
    ...
    for (let r = 0; r < requestCount; r++) {
      ...
      reqStmt.run(instanceId, requestId, dep.deploymentId, pick(rng, REQUEST_ACTIONS),
        ...
        failed ? 'Request failed: timeout waiting for provider response' : null);
      totals.requests += 1;
    }
    ...
    endpointDefs.forEach((ep, idx) => {
      ...
      epStmt.run(instanceId, `ep-${idx + 1}`, isIntegration ? 'integration' : 'cloud-account',
        ...
        JSON.stringify({ state: unhealthy ? 'ERROR' : 'OK' }));
      totals.endpoints += 1;
    });
    ...
    CATALOG_SOURCES.forEach((src, idx) => {
      ...
      catStmt.run(instanceId, `cat-${idx + 1}`, src.name, src.type, imported, found,
        ...
        hasErrors ? 'Failed to import 3 items: authentication error against upstream registry' : null);
    ...
    for (const region of REGIONS) {
      for (const img of FABRIC_IMAGES) {
        ...
        imgStmt.run(instanceId, `img-${imgIdSeq}`, img.name, img.desc,
          ...
          new Date(now - randInt(rng, 0, 3) * 86400000).toISOString());
        imgIdSeq += 1;
        totals.images += 1;
      }
      IMAGE_MAPPINGS.forEach((m) => {
        imgMapStmt.run(instanceId, `imgprof-${region}`, `${region.split(':')[1]}-images`, region,
          ...
          m.os, `${m.mapping} standard build`);
        totals.imageMappings += 1;
      });
      FLAVOR_MAPPINGS.forEach((f) => {
        flavorStmt.run(instanceId, `${region.split(':')[1]}-flavors`, region, f.name, f.cpu, f.memMb);
        totals.flavors += 1;
      });
    }
    ...
    for (let r = 0; r < runCount; r++) {
      ...
      runStmt.run(instanceId, kind, runId, `${kind}-${pick(rng, DEPLOYMENT_ROLES)}-run`,
        ...
        failed ? `${kind} run failed: script exited non-zero` : null);
      totals.runs += 1;
    }
    ...
    approvals.forEach((a) => {
      apprStmt.run(instanceId, `appr-${apprIdSeq++}`, 'Deployment request approval',
        ...
        new Date(now - a.agedDays * 86400000).toISOString());
      totals.approvals += 1;
    });
  ...
  // ── Deployment resources (Server 360 join): first deployment of each
  // instance owns fixture VM IPs, so the correlated view links vRA -> vCenter
  // -> other platforms end to end.
  firstDeps.forEach((dep, i) => {
    drStmt.run(dep.instanceId, dep.deploymentId, `res-${dep.deploymentId}-0`,
      `${dep.name}-vm-01`, JSON.stringify([`10.100.11.1${1 + i}`]));
  });

  // ── Issue lifecycle history — the real reconcile against the just-seeded
  // inventory so aria_issue_history keys match computeIssues exactly. Uses
  // this plugin's own ./issues (NOT the host's services/ariaIssues, which
  // would fail to resolve inside a bundled plugin) — repointing this require
  // is one of the two silent porting traps the conversion skill calls out. ──
  let issueHistoryTotal = 0;
  try {
    const { reconcileIssueHistory } = require('./issues');
    reconcileIssueHistory(coreApi);
    issueHistoryTotal = db.prepare('SELECT COUNT(*) n FROM aria_issue_history').get().n;
  } catch (err) {
    coreApi.logger.warn(`[demoSeed] aria issue history reconcile skipped: ${err.message}`);
  }

  return { ...totals, issueHistory: issueHistoryTotal };
}
...
function seedAriaDemo(coreApi) {
  const db = coreApi.db;
  return db.transaction(() => seedAria(db, { now: Date.now(), encrypt: coreApi.encryption.encrypt, coreApi }))();
}

module.exports = { seedAria, seedAriaDemo };
