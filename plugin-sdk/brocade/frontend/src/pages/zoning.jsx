import { Network, X, ListTree, History } from '../icons.jsx';
import client from '../api.js';
import {
  useToast, PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated, portalOrInline,
  BRAND, fmtWhen, parseJsonArr,
} from '../ui.jsx';
...
export default function BrocadeZoningPage() {
  ...
  const [zoning, setZoning] = React.useState(null);
  const [changes, setChanges] = React.useState(null);
  const [tab, setTab] = React.useState('browser');
  const [detailZone, setDetailZone] = React.useState(null);
  const [lastRefreshed, setLastRefreshed] = React.useState(null);

  const loadFabrics = React.useCallback(() => client.get('/brocade/zoning/fabrics')
    .then(({ data }) => {
  ...
  React.useEffect(() => { loadFabrics(); }, []); // eslint-disable-line react-hooks/exhaustive-deps

  const loadZoning = React.useCallback(() => {
    if (!fabric) return;
  ...
  React.useEffect(() => { loadZoning(); loadChanges(); }, [loadZoning, loadChanges]);
