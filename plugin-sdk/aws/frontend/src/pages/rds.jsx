// Ported from frontend/src/pages/aws/AwsRdsPage.jsx — createPortal(...,
// document.body) replaced with ui.jsx's portalOrInline() (window.ReactDOM
// has no createPortal), components/TrendChart replaced with charts.jsx's
// TrendChart (window.Chart).
import { Database, LineChartIcon, X } from '../icons.jsx';
import {
  apiFetch, PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated, Spinner, portalOrInline,
  useTableControls, SortTh, TableControls, TablePager, BRAND, fmtNum, fmtBytes, fmtPct, fmtWhen,
} from '../ui.jsx';
...
function RdsHistoryModal({ instance, onClose }) {
  ...
  React.useEffect(() => {
    setRows(null);
    setError(false);
    apiFetch('/aws/rds/history', { params: { dbId: instance.dbId, days: 90 } })
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="min-w-0">
            <h2 className="text-sm font-bold text-ink truncate flex items-center gap-2">
              <LineChartIcon size={15} className="text-brand" /> {instance.dbId} — free storage (90d)
            </h2>
...
export default function AwsRdsPage() {
  ...
  const [warnPct, setWarnPct] = React.useState(STORAGE_WARN_DEFAULT);
  const [lastRefreshed, setLastRefreshed] = React.useState(null);
  const [historyInstance, setHistoryInstance] = React.useState(null);
  const [error, setError] = React.useState(null);

  const load = React.useCallback(() => Promise.all([
    apiFetch('/aws/rds').then((json) => setInstances(json?.instances || [])),
    apiFetch('/aws/config').then((json) => { if (json?.rdsStorageWarnPct != null) setWarnPct(Number(json.rdsStorageWarnPct)); }).catch(() => {}),
  ]).then(() => { setLastRefreshed(new Date()); setError(null); })
    .catch(() => { setInstances([]); setError('Failed to load RDS data'); }), []);
