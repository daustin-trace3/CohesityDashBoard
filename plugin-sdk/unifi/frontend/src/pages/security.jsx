// UniFi Security — port of frontend/src/pages/unifi/UnifiSecurityPage.jsx.
// icons.jsx has no Ban/Target/ListChecks — substituted with X/Zap/ClipboardCheck
// (closest available; report loudly per contract).
import {
  PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TableControls, TablePager,
  apiFetch, fmtNum, fmtWhen, BRAND,
} from '../ui.jsx';
import { BarChart } from '../charts.jsx';
import { ShieldCheck, ShieldAlert, X, Zap, ClipboardCheck } from '../icons.jsx';
...
export default function SecurityPage() {
  ...
  const [ruleTab, setRuleTab] = React.useState('firewall');

  const load = React.useCallback(() => apiFetch('/unifi/security')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ ips: {}, rogueCounts: {}, events: [], posture: [], rules: { firewall: [], traffic: [] }, timeline: [], topDestinations: [], topOffenders: [], policyHits: [], rogueChanges: { newThisWeek: 0, flagged: [] } })), []);
  ...
  const threatChart = React.useMemo(() => ({
        ...
        <>
          ...
          <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
            {timeline.length === 0 ? (
              ...
              <BarChart data={threatChart} options={chartOpts} height={192} />
            )}
          </div>

          <div className="grid sm:grid-cols-3 gap-3 mb-4">
            <CompactList icon={X} title="Top Blocked Destinations" rows={topDestinations} labelKey="dst" countKey="count" />
            <CompactList icon={Zap} title="Repeat Offenders" rows={topOffenders} labelKey="src" countKey="count" />
            <CompactList icon={ClipboardCheck} title="Policy Hits" rows={policyHits} labelKey="policy" countKey="count" />
          </div>
