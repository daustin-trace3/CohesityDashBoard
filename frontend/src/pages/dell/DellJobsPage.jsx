...
export default function DellJobsPage() {
  ...
  const load = useCallback(() => client.get('/dell/jobs')
    .then(({ data }) => { setRows(Array.isArray(data) ? data : []); setLastRefreshed(new Date()); })
    .catch(() => { setRows([]); toast({ type: 'error', title: 'Failed to load OME jobs' }); }), [toast]);
  ...
  const list = useMemo(() => (rows || []).filter((j) => showInternal || (j.visible && !j.internal)), [rows, showInternal]);
