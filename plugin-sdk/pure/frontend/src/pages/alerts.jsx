// Ported from frontend/src/pages/pure/PureAlertsPage.jsx.
...
export default function PureAlertsPage() {
  ...
  const load = React.useCallback((force = false) => {
    return Promise.allSettled([
      apiFetch(`/pure/pure1/alerts${force ? '?refresh=1' : ''}`),
      apiFetch('/pure/pure1/status'),
    ]).then(([a, s]) => {
      if (a.status === 'fulfilled') { setAlerts(a.value || []); setLastRefreshed(new Date()); }
      else setAlerts([]);
      if (s.status === 'fulfilled') setShowHidden(!!s.value.showHiddenAlerts);
    });
  }, []);
  ...
  const counts = React.useMemo(() => visible.reduce(
    (acc, a) => {
