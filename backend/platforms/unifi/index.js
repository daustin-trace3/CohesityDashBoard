// UniFi platform manifest (ICC contract, feat/plugin-touchpoints). Manifest-
// hooks built-in (the nutanix model) — opsSummary/collectAlerts/
// searchCategories/metricsHistory/server360/server360Suggest are all wired
// via the branch's manifest hooks (backend/core/registry.js). No core.js/
// ops.js/poller.js/alertNotifier.js/search.js/server360.js edits required.
const unifiMigrations = require('../../db/migrations/unifi');
const unifiRouter = require('../../routes/unifi');
const { createUnifiPollerHandle } = require('../../services/unifiPoller');
...
module.exports = {
  ...
  apiVersion: 1,
  color: '#006FFF',
  migrations: unifiMigrations,
  createRouter() {
    return unifiRouter;
  },
  createPoller() {
    return createUnifiPollerHandle();
  },
