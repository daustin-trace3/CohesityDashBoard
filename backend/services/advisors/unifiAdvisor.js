...
function gatherNetworkHealth() {
  ...
  return {
    ...
    thresholds: { deviceCpuWarnPct: deviceCpuWarnPct(), deviceMemWarnPct: deviceMemWarnPct(), tempWarnC: tempWarnC() },
...
function gatherWanReliability() {
  ...
  const activeWanIssues = computeIssues().filter((i) => i.type === 'wan-latency' || i.type === 'wan-availability');
  ...
  return {
    ...
    activeWanIssues,
    wanIssueHistory14d,
    thresholds: { wanLatencyWarnMs: wanLatencyWarnMs(), wanAvailWarnPct: wanAvailWarnPct() },
...
function gatherSecurityPosture() {
  ...
  const securityIssues = computeIssues().filter((i) => ['ips-disabled', 'camera-offline', 'protect-breach', 'rogue-ap'].includes(i.type));
