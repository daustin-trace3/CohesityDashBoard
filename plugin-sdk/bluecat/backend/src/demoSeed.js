...
// first; counts are then derived with the SAME math as issues.js /
// poller.js (contract section 5) so numbers agree.
//
// Ported from backend/demo/generators/bluecat.js. ALL inserts here run ONLY
// behind the DASHBOARD_DEMO==='1' gate - see seedBluecatDemo() below, called
...
// Credential encryption uses coreApi.encryption.encrypt (nutanix/unifi/dell
// plugin demoSeed.js precedent) instead of requiring the host's encryption
// service directly. Issue computation goes through the pack's own ./issues
// module (threaded with coreApi) rather than requiring the host's
// services/bluecatIssues.
//
// DEVIATION FROM THE BUILT-IN's wipe strategy: the built-in generator is run
// only via backend/demo/seedDemo.js (which wipes the whole demo.db first),
// so it always INSERTs a fresh bluecat_sources row. A plugin reseeds on
// every boot, so bluecat_sources is upserted BY NAME (ON CONFLICT(name),
// the column already carries a UNIQUE constraint) so its id stays stable
// across reseeds, and only its dependent rows are wiped first - children
// before parents, scoped by source_id (bluecat_issue_history has no
// source_id column, only a source NAME column, so it is wiped by that name
// instead). Cross-hit hostnames (contract section 10): vra-prod, vra-dr,
// vrops-nyc-01, vrli-nyc-01, vrlcm-nyc-01, vrni-nyc-01 match VM names in
// the host's demo/generators/vcenter.js (and the unifi wired-client names)
// so Server 360 correlates across platforms when both plugins are installed.
...
// Children->parents, scoped by source_id (bluecat_issue_history has no
// source_id column and is wiped separately, by its source NAME column).
const DEMO_CHILD_TABLES_BY_SOURCE_ID = [
  'bluecat_addresses', 'bluecat_ranges', 'bluecat_network_overrides', 'bluecat_networks', 'bluecat_blocks',
  'bluecat_records', 'bluecat_zones', 'bluecat_views', 'bluecat_devices', 'bluecat_servers',
  'bluecat_metrics_history',
];

function seedBluecat(db, coreApi, { now, encrypt }) {
  ...
  // -- Source: upsert BY NAME so its id stays stable across reseeds -----
  const upsertSource = db.prepare(`
    ...
    ON CONFLICT(name) DO UPDATE SET
      ...
      bam_version = excluded.bam_version, configurations_json = excluded.configurations_json,
      last_poll_status = excluded.last_poll_status, last_poll_error = excluded.last_poll_error,
      last_poll_at = excluded.last_poll_at, last_enumerate_at = excluded.last_enumerate_at,
      last_enumerate_error = excluded.last_enumerate_error
  `);
  upsertSource.run({
  ...
  const sourceId = db.prepare('SELECT id FROM bluecat_sources WHERE name = ?').get(SOURCE_NAME).id;

  // Wipe this fixture's dependent rows only, children before parents - any
  // independently-registered real source is untouched (there is only ever
  // one demo source here, but the pattern stays name/id-scoped regardless).
  for (const table of DEMO_CHILD_TABLES_BY_SOURCE_ID) {
    db.prepare(`DELETE FROM ${table} WHERE source_id = ?`).run(sourceId);
  }
  db.prepare('DELETE FROM bluecat_issue_history WHERE source = ?').run(SOURCE_NAME);
  ...
  for (const v of VIEWS) {
    insertView.run(sourceId, v.id, v.configurationId, configName(v.configurationId), v.name, v.zones.length, 0);
  }
  ...
  // -- Issue history: issues.js's computeIssues() + reconcileIssueHistory()
  //    (the pack's own module, threaded with coreApi - not the host's
  //    services/bluecatIssues, which does not resolve inside a plugin
  //    bundle). Fall back to representative rows if that computation throws
  //    for any reason, same as the built-in's belt-and-suspenders design.
  let issueRules = [];
  let issueFallback = false;
  try {
    const bluecatIssues = require('./issues');
    bluecatIssues.reconcileIssueHistory(coreApi);
...
/** Demo-only entry point. Upserts the one fixture source (id stable across
 *  boots), wipes its dependent rows, and regenerates them with fresh
 *  relative timestamps, so a demo box refreshes on every boot instead of
 *  aging into a stale-looking estate. NEVER runs outside demo mode - see
 *  the DASHBOARD_DEMO gate in poller.js. */
function seedBluecatDemo(coreApi) {
  const db = coreApi.db;
  return db.transaction(() => seedBluecat(db, coreApi, { now: Date.now(), encrypt: coreApi.encryption.encrypt }))();
}

module.exports = { seedBluecat, seedBluecatDemo };
