// vCenter Overview — ported from frontend/src/pages/vcenter/VcOverviewPage.jsx.
// react-router-dom navigate/Link -> window.ReactRouterDOM; react-chartjs-2
// Line/Bar -> charts.jsx LineChart/BarChart (window.Chart backed); axios
// client -> apiFetch; useToast -> inline status text (none needed here,
// failures degrade to empty state).
import { Gauge, Server, MonitorSmartphone, Database, ShieldAlert, Boxes, Play, Power, HardDrive, Wrench, Cpu, MemoryStick } from '../icons.jsx';
import { LineChart, BarChart } from '../charts.jsx';
import {
  apiFetch, PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated, timeAgo, asDate,
  BRAND, fmtNum, fmtBytes, severityTone,
} from '../ui.jsx';
...
export default function VcOverviewPage() {
  ...
  const load = React.useCallback(() => apiFetch('/vcenter/overview')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ vcenters: [], hosts: {}, datastores: {}, issues: [] })), []);

  React.useEffect(() => { load(); }, [load]);
  React.useEffect(() => {
    apiFetch(`/vcenter/trends?days=${trendDays}`).then((json) => setTrend(json)).catch(() => setTrend([]));
  }, [trendDays]);

  // VM guest count per vCenter over time — daily last snapshot per vCenter.
  const vmTrend = React.useMemo(() => {
  ...
  const densityBar = React.useMemo(() => {
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-4 mb-4">
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          <p className="text-sm font-semibold text-ink mb-3" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Cpu size={15} className="text-brand" /> CPU Capacity</p>
          <div className="grid grid-cols-3 gap-3 text-center">
            ...
            <div>
              <p className="text-xl font-bold tnum" style={{ color: cap.cpu_overcommit > 4 ? 'var(--vc-warn)' : 'var(--vc-ink)' }}>{ratio(cap.cpu_overcommit)}</p>
              <p className="text-[11px] text-ink-faint">vCPU : pCore overcommit</p>
            </div>
          </div>
        </div>
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          <p className="text-sm font-semibold text-ink mb-3" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><MemoryStick size={15} className="text-brand" /> Memory Capacity</p>
          <div className="grid grid-cols-3 gap-3 text-center">
            <div>
            ...
            <div>
              <p className="text-xl font-bold tnum" style={{ color: cap.mem_overcommit > 1.5 ? 'var(--vc-warn)' : 'var(--vc-ink)' }}>{ratio(cap.mem_overcommit)}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        <div className="panel p-4 lg:col-span-2" style={{ borderTop: `3px solid ${BRAND}` }}>
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            <select value={trendDays} onChange={(e) => setTrendDays(Number(e.target.value))} className="vc-input" style={{ width: 'auto', cursor: 'pointer' }}>
          ...
          {trend == null ? (
            ...
            <LineChart data={vmTrend} height={240} />
          )}
        </div>
        <div className="panel p-4">
          ...
          {data == null ? (
            ...
            <BarChart data={osBar} height={Math.max(200, Math.min(12, data.osBreakdown.length) * 24 + 60)}
      ...
      <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <p className="text-sm font-semibold text-ink mr-auto" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Server size={15} className="text-brand" /> ESXi Host Density</p>
          {avgDensity != null && (
        ...
        {data == null ? (
          ...
          <BarChart data={densityBar} height={210} options={{ plugins: { legend: { display: false } } }} />
        )}
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4 mb-4">
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          <p className="text-sm font-semibold text-ink mb-3" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Boxes size={15} className="text-brand" /> vCenters</p>
          {data == null ? (
        ...
        {/* Issues feed */}
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><ShieldAlert size={15} className="text-brand" /> Issues</p>
