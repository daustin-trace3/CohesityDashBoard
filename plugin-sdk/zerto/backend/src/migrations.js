...
//
// Copied VERBATIM from backend/db/migrations/zerto.js — SAME scope id
// 'zerto', so an existing DB's zerto_* data (and its schema_migrations row)
// is adopted intact on install. The only change is v5's alert-type seed,
// which now requires the embedded ./alertTypes.js module instead of the
// host's db/reference/zertoAlertTypes.json (unreachable inside a plugin
// bundle).
module.exports = [
  {
    version: 1,
  ...
  // Per-alert-type SMTP notification toggles. Seeded from the embedded
  // ./alertTypes.js reference (229 codes, mirrors db/reference/
  // zertoAlertTypes.json — a bundled plugin cannot read that host file);
  // the poller stamps first/last_seen when a code shows up live, and inserts
  // codes the reference doesn't know. enabled=0 mutes the code in the
  // alertNotifier collector.
  {
    version: 5,
    up(db) {
      db.exec(`
      ...
      const seed = db.prepare(`
        ...
        VALUES (?, ?, ?, ?)
      `);
      for (const t of require('./alertTypes')) {
