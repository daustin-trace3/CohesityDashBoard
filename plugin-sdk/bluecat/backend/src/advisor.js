// BlueCat AI Advisor: ip_capacity/dns_hygiene/server_health fleet reports.
// Ported from backend/services/advisors/bluecatAdvisor.js. The original
...
// handed to it, so this exports a FACTORY - createBluecatAdvisor(coreApi) -
// built lazily by router.js once coreApi is known (dell/brocade advisor.js
// pattern). Per the plugin contract, coreApi.advisor is the host's
// services/platformAdvisor module (createPlatformAdvisor/linReg/parseUtcMs/
// fmtBytes), never required directly. lowFreeWarn/lowFreePct/computeIssues
// come from the pack's own ./issues module (threaded with coreApi) rather
// than a host require.
const { lowFreeWarn, lowFreePct, computeIssues } = require('./issues');

function createBluecatAdvisor(coreApi) {
  const db = coreApi.db;
  const { createPlatformAdvisor } = coreApi.advisor;

  function issueSlice(types) {
    const all = computeIssues(coreApi);
  ...
  function gatherIpCapacity() {
    ...
    return {
      ...
      issues,
      thresholds: { lowFreeWarn: lowFreeWarn(coreApi), lowFreePct: lowFreePct(coreApi) },
  ...
  // Exposed for the rehearsal script only (createPlatformAdvisor's return has no gather accessor).
  advisor.reports = REPORTS;
  return advisor;
}

module.exports = { createBluecatAdvisor };
