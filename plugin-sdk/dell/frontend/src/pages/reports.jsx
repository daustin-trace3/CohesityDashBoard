...
import {
  apiFetch, apiFetchBlob, PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TablePager,
  BRAND, fmtNum, fmtBytes, fmtWhen, healthTone,
} from '../ui.jsx';
...
export default function DellReportsPage() {
  const [active, setActive] = React.useState(REPORTS[0]);
  const [data, setData] = React.useState(null);
  const [loading, setLoading] = React.useState(false);
  const [lastRefreshed, setLastRefreshed] = React.useState(null);
  const [daysSel, setDaysSel] = React.useState(null);
  const [devices, setDevices] = React.useState(null);
  const [deviceSel, setDeviceSel] = React.useState('');
  const [statusMsg, setStatusMsg] = React.useState(null);

  const flash = (msg) => { setStatusMsg(msg); setTimeout(() => setStatusMsg((s) => (s === msg ? null : s)), 4000); };

  // Server list for the per-device reports, fetched once on demand.
  React.useEffect(() => {
    if (active.needsDevice && devices == null) {
      apiFetch('/dell/devices')
        .then((json) => setDevices((Array.isArray(json) ? json : []).filter((d) => /server/i.test(d.device_type || ''))))
        .catch(() => setDevices([]));
    }
  }, [active, devices]);
  ...
  const load = React.useCallback(() => {
    ...
    apiFetch(`/dell/reports/${active.key}${qs ? `?${qs}` : ''}`)
      .then((json) => {
  ...
  const downloadPacket = async () => {
    try {
      const blob = await apiFetchBlob(`/dell/reports/support-packet?deviceId=${encodeURIComponent(deviceSel)}`);
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      const dev = (devices || []).find((d) => String(d.id) === String(deviceSel));
      a.href = url; a.download = `support-packet-${dev?.service_tag || deviceSel}.txt`;
      a.click(); URL.revokeObjectURL(url);
    } catch { flash('Failed to build the support packet'); }
  };

  const groups = React.useMemo(() => ['Audit', 'Operations', 'Support'].map((g) => ({ g, items: REPORTS.filter((r) => r.group === g) })), []);
  const selectCls = 'dl-input';
  ...
  return (
    <div className="animate-fade-in">
      ...
      {statusMsg && <div className="panel p-3 mb-3 text-xs text-status-crit">{statusMsg}</div>}
      ...
      <div className="flex gap-4 items-start">
        ...
        <div className="flex-1 min-w-0 panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          <div className="flex items-start justify-between gap-3 mb-1 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {active.needsDevice && (
                <select value={deviceSel} onChange={(e) => setDeviceSel(e.target.value)} className={selectCls} style={{ width: 'auto', cursor: 'pointer' }} aria-label="Server">
                  <option value="">Select a server…</option>
                  {(devices || []).map((d) => <option key={d.id} value={d.id}>{d.name || d.service_tag}</option>)}
                </select>
              )}
              {active.days != null && (
                <select value={effDays} onChange={(e) => setDaysSel(Number(e.target.value))} className={selectCls} style={{ width: 'auto', cursor: 'pointer' }} aria-label="Timeframe">
