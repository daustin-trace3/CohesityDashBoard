...
//
// Ported from backend/services/proxmoxPoller.js. db/logger/createPoller now
// come from coreApi; the framework's `createPoller` is coreApi.createPoller
// (backend/core/pollerFramework.js) rather than a direct host require.
//
// Module-scoped singleton: createRouter() and createPoller() are both called
// by the host registry against the same coreApi, but createRouter runs first
// (per contract C4) and needs to reach the same poller instance for
// schedule/cancel/trigger on server CRUD. getPoller() lazily builds it if
// not yet created, and createProxmoxPoller() (the manifest.createPoller
// entry point) reuses it if routes.js got there first.
...
async function collect(server, coreApi) {
  const forbidden = new Set();

  await proxmoxApi.fetchVersion(server, coreApi); // also the connectivity check; throws on failure

  const nodes = await proxmoxApi.fetchNodes(server, coreApi);

  let clusterResources = [];
  try {
    clusterResources = await proxmoxApi.pveGet(server, coreApi, '/cluster/resources');
  } catch (err) {
  ...
  for (const n of nodes) {
    ...
    let status = null;
    try {
      status = await proxmoxApi.pveGet(server, coreApi, `/nodes/${nodeName}/status`);
    } catch (err) {
    ...
    const [qemu, lxc, storage, tasks, certs, subscription, services, network, disks] = await Promise.all([
      proxmoxApi.fetchQemu(server, coreApi, nodeName),
      proxmoxApi.fetchLxc(server, coreApi, nodeName),
      proxmoxApi.fetchNodeStorage(server, coreApi, nodeName),
      proxmoxApi.fetchTasks(server, coreApi, nodeName, 200),
      proxmoxApi.fetchCertificates(server, coreApi, nodeName),
      proxmoxApi.fetchSubscription(server, coreApi, nodeName),
      proxmoxApi.fetchNodeServices(server, coreApi, nodeName),
      proxmoxApi.fetchNodeNetwork(server, coreApi, nodeName),
      proxmoxApi.fetchDisksList(server, coreApi, nodeName),
    ]);
    ...
    nodeRows.push({
      ...
      // pveversion arrives as "pve-manager/9.1.4/<repoid>" — keep just the release.
      pveVersion: (status?.pveversion?.match(/pve-manager\/([^/]+)/)?.[1]) ?? status?.pveversion ?? null,
    ...
    let aptUpdates = [];
    try {
      aptUpdates = await proxmoxApi.fetchAptUpdates(server, coreApi, nodeName);
    } catch { /* tolerate */ }
    ...
    for (const s of storage) {
      ...
      for (const contentType of types) {
        const items = await proxmoxApi.fetchStorageContent(server, coreApi, nodeName, s.storage, contentType);
  ...
  for (const g of guestRows) {
    ...
    const [config, snaps] = await Promise.all([
      proxmoxApi.fetchGuestConfig(server, coreApi, g.node, g.type, g.vmid),
      proxmoxApi.fetchGuestSnapshots(server, coreApi, g.node, g.type, g.vmid),
    ]);
    ...
    if (config && agentEnabled(config) && g.type === 'qemu' && g.status === 'running') {
      try {
        const osinfo = await proxmoxApi.fetchAgentOsInfo(server, coreApi, g.node, g.vmid);
        g.osName = osinfo?.result?.['pretty-name'] ?? null;
        if (osinfo) g.agentRunning = 1;
      } catch { /* tolerate: non-running/no-agent 500s */ }
      try {
        const ifaces = await proxmoxApi.fetchAgentInterfaces(server, coreApi, g.node, g.vmid);
        if (ifaces?.result) {
          const ips = [];
  ...
  let backupJobs = null;
  try {
    backupJobs = (await proxmoxApi.pveGet(server, coreApi, '/cluster/backup')) || [];
  } catch (err) {
    if (err?.pveForbidden) {
      forbidden.add('cluster/backup');
      backupJobs = [];
    } else {
      coreApi.logger.warn(`[ProxmoxPoller] ${server.name}: cluster/backup fetch failed: ${safeMsg(err)}`);
    }
  }

  let clusterStatus = [];
  try {
    clusterStatus = await proxmoxApi.fetchClusterStatus(server, coreApi);
  ...
  const clusterLog = await proxmoxApi.fetchClusterLog(server, coreApi, 200);
...
function buildStore(coreApi) {
  const db = coreApi.db;
  return db.transaction((serverId, data) => {
    const {
    ...
    if (backupJobs === null) {
      coreApi.logger.warn(`[ProxmoxPoller] backup jobs fetch failed for server ${serverId}; keeping existing backup job inventory`);
    } else {
...
async function pollProxmox(server, coreApi) {
  const db = coreApi.db;
  const store = buildStore(coreApi);
  try {
    const data = await collect(server, coreApi);
    ...
    `).run(server.id);
    coreApi.logger.info(`[ProxmoxPoller] ${server.name}: ${data.nodes.length} node(s), ${data.guests.length} guest(s), ${data.storage.length} storage(s)`);
  } catch (err) {
    db.prepare(`
    ...
    try { reconcileIssueHistory(coreApi); } catch (err) {
      coreApi.logger.warn(`[ProxmoxPoller] issue-history reconcile failed: ${err.message}`);
    }
  }
}
...
function buildPoller(coreApi) {
  return coreApi.createPoller({
    id: 'proxmox',
    loadSources: () => coreApi.db.prepare('SELECT * FROM proxmox_servers').all(),
    intervalMinutes: (s) => s.polling_interval_minutes,
    poll: (source) => pollProxmox(source, coreApi),
  });
}

/** Shared singleton poller instance (schedule/cancel/trigger/init/stopAll),
...
module.exports = { createProxmoxPoller, getPoller, pollProxmox };
