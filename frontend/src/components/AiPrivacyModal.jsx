...
export default function AiPrivacyModal({ onClose }) {
      ...
      <div className="bg-cohesity-gray border border-cohesity-border rounded-lg w-full max-w-6xl max-h-[90vh] flex flex-col shadow-xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-cohesity-border flex-shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-cohesity-text font-semibold">AI Privacy Inspector</h2>
              <p className="text-[11px] text-gray-400 mt-0.5">
                Proof of anonymization — the exact payload each AI request sent vs. the name mapping that never left this server.
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex min-h-0">
          {/* Exchange list */}
          <div className="w-60 border-r border-cohesity-border overflow-y-auto flex-shrink-0">
            {exchanges === null ? (
              ...
              <p className="p-4 text-xs text-ink-muted leading-relaxed">
                No AI requests recorded since server start. Run any AI report, cluster analysis, or alert review, then reopen this window.
              </p>
            ) : exchanges.map((ex) => (
              <button
                key={ex.id}
            ...
            {!selected ? (
              ...
              <div className="flex flex-col gap-4">
                <div className="text-[11px] text-ink-muted bg-brand/5 border border-brand/20 rounded-md px-3 py-2 leading-relaxed">
                  ...
                  hostname, and IP before the request left this machine. The mapping table below exists only in this server's memory and is
