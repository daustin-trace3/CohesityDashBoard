// Zerto Analytics poller — one account-wide global cron task (the SaaS API
// returns everything in a handful of calls; there are no per-source
// connections to schedule). Current-state tables are replaced wholesale per
// poll; an account-level snapshot is appended to zerto_metrics_history for
// trends. Ported from icc-phase1, where this runs on core/pollerFramework's
// createGlobalTask — here the small task wrapper is inlined.
...
// Minimal global-task wrapper (same surface as core/pollerFramework's
// createGlobalTask on icc-phase1: reschedule/trigger/stop/isRunning).
let task = null;

async function runWrapped() {
  pollerStatus.markStart('zerto', 0);
  try {
    await refreshAll();
    pollerStatus.markEnd('zerto', 0, 'success');
  } catch (err) {
    logger.error('[ZertoPoller] Poll failed:', err?.message || err);
    pollerStatus.markEnd('zerto', 0, 'error');
  }
}
...
const zertoTask = {
  reschedule() {
    this.stop();
    const interval = Math.max(5, Number(getSetting('zerto_poll_interval_minutes')) || 15);
    task = cron.schedule(`*/${interval} * * * *`, runWrapped);
    return task;
  },
  trigger: () => runWrapped(),
  stop() {
