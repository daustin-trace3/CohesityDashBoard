// Port of frontend/src/pages/unifi/UnifiOverviewPage.jsx onto the plugin ui/charts kit.
import {
  PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated,
  apiFetch, apiFetchBlob, fmtNum, fmtWhen, BRAND,
} from '../ui.jsx';
import {
  Gauge, Router, Users, Globe, ShieldAlert, Activity, Server, Zap, Cable, Shield, Wifi,
  RotateCcw, UserPlus, Clock, ArrowDownUp, Thermometer, Cctv,
} from '../icons.jsx';
...
// Snapshot <img> can't carry auth headers, so frames are fetched as blobs
// through apiFetchBlob and swapped in as object URLs.
...
function CameraSnapshot({ cameraId, state }) {
  ...
  return <img src={url} alt="camera snapshot" className="aspect-video rounded-lg object-cover w-full" />;
}
            ...
            <div className="flex flex-col gap-0.5">
              {topTalkers.slice(0, 4).map((t) => (
                <Line11 key={t.label + t.ip}>
                  <span className="text-ink">{t.label}</span> · {fmtBytesShort(t.bytes)} · {t.is_wired ? 'wired' : 'wifi'}
                </Line11>
              ))}
            </div>
        ...
        <InsightCard icon={Zap} title="PoE Power" onClick={() => navigate('/unifi/ports')}>
          ...
          {(poe?.trend || []).length > 1 && (
            <div style={{ marginTop: 6 }}>
              <LineChart data={poeTrendData} options={{ plugins: { legend: { display: false } }, scales: { x: { display: false }, y: { display: false } } }} height={48} />
            </div>
          )}
        </InsightCard>
...
export default function UnifiOverviewPage() {
  ...
  const load = React.useCallback(() => Promise.all([
    apiFetch('/unifi/overview')
      .then((json) => { setData(json); setLastRefreshed(new Date()); })
      .catch(() => setData({ sources: [], deviceCounts: {}, clientCounts: {}, wan: null, health: [], issueCounts: {}, spark: [] })),
    apiFetch('/unifi/insights')
      .then((json) => setInsights(json))
      .catch(() => setInsights(null)),
    apiFetch('/unifi/protect')
      .then((json) => setCameras((json?.cameras || []).filter((c) => c.model_key !== 'chime')))
      .catch(() => setCameras([])),
  ]), []);
  ...
  const clientsTrend = React.useMemo(() => ({
    labels: spark.map((s) => fmtWhen(s.capturedAt).split(',')[0]),
    datasets: [
      { label: 'Clients', data: spark.map((s) => s.clientsTotal), borderColor: BRAND, backgroundColor: 'rgba(0,111,255,0.15)', pointRadius: 0, borderWidth: 2, tension: 0.25, fill: true },
    ],
  }), [spark]);
  ...
  const latencyTrend = React.useMemo(() => ({
    labels: spark.map((s) => fmtWhen(s.capturedAt).split(',')[0]),
    datasets: [
      { label: 'WAN Latency (ms)', data: spark.map((s) => s.wanLatencyMs), borderColor: '#D4A24E', backgroundColor: 'rgba(212,162,78,0.12)', pointRadius: 0, borderWidth: 2, tension: 0.25, fill: true },
    ],
  }), [spark]);
  ...
  return (
    <div className="uf-root animate-fade-in">
      ...
      {insights && <InsightsBand insights={insights} navigate={navigate} features={data?.features} />}

      {cameras.length > 0 && (
        <div className="mb-4">
          <p className="text-sm font-semibold text-ink mb-3 flex items-center gap-2">
            <Cctv size={15} className="text-brand" /> Cameras
            <ReactRouterDOM.Link to="/unifi/protect" className="text-[11px] text-brand font-normal underline ml-auto">View all</ReactRouterDOM.Link>
          </p>
      ...
      {spark.length > 1 && (
        <div className="grid lg:grid-cols-2 gap-4 mb-4">
          <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
            ...
            <LineChart data={clientsTrend} height={192} />
          </div>
          <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
            ...
            <LineChart data={latencyTrend} height={192} />
          </div>
        </div>
      )}
