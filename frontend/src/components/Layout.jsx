...
export default function Layout() {
  ...
  const isNutanix = pathname.startsWith('/nutanix');
  const isOps = pathname.startsWith('/ops');
  const isPlatform = isPure || isNetapp || isZerto || isVcenter || isDell || isAria || isAriaOps || isNetbackup || isAws || isNutanix;
  const platformKey = isPure ? 'pure' : isNetapp ? 'netapp' : isZerto ? 'zerto' : isVcenter ? 'vcenter' : isDell ? 'dell' : isAria ? 'aria' : isAriaOps ? 'ariaops' : isNetbackup ? 'netbackup' : isAws ? 'aws' : isNutanix ? 'nutanix' : null;
  const platformLabel = isPure ? 'Pure Array' : isNetapp ? 'NetApp Cluster' : isZerto ? 'Zerto Site' : isVcenter ? 'ESX Host' : isDell ? 'Device' : isAria ? 'Deployment' : isAriaOps ? 'Resource' : isNetbackup ? 'Server' : isAws ? 'Instance' : isNutanix ? 'VM' : '';
  ...
  // Drops nav items tagged with a `feature` flag (Move/Mine) when that
  // feature isn't configured on any Nutanix source yet.
  const filterFeatureItems = (groups, features) => groups
    .map(group => ({ ...group, items: group.items.filter(item => !item.feature || features[item.feature]) }))
    .filter(group => group.items.length > 0);
  const nutanixNavGroups = filterFeatureItems(getPlatform('nutanix')?.navGroups || [], nutanixFeatures);
  ...
  useEffect(() => {
    ...
    const nutanix = platformKey === 'nutanix';
    ...
    const overviewUrl = pureFleet ? '/pure1/overview' : zerto ? '/zerto/sites' : vcenter ? '/vcenter/hosts' : dell ? '/dell/devices' : aria ? '/aria/deployments' : ariaops ? '/ariaops/resources' : netbackup ? '/netbackup/overview' : aws ? '/aws/overview' : nutanix ? '/nutanix/overview' : `/${platformKey}/overview`;
    const alertsUrl = pureFleet ? '/pure1/alerts' : (vcenter || dell) ? `/${platformKey}/overview` : aria ? '/aria/issues' : ariaops ? '/ariaops/alerts' : netbackup ? '/netbackup/issues' : aws ? '/aws/issues' : nutanix ? '/nutanix/issues' : `/${platformKey}/alerts`;

    const loadAlertList = () => client.get(alertsUrl)
      .then(r => {
        if (cancelled) return;
        if (vcenter || dell || nutanix) {
          const rows = (r.data?.issues || []).filter(i => i.severity !== 'info');
          setPlatformAlertList(rows.map((i, idx) => ({
            id: idx,
            cluster_name: nutanix ? (i.source || '—') : (i.vcenter || i.ome || '—'),
    ...
    const loadPlatform = () => client.get(overviewUrl)
      .then(r => {
        ...
        // bare entity arrays).
        const rows = netbackup ? (r.data?.sources || []) : nutanix ? (r.data?.clusters || []) : (r.data || []);
        ...
        if (zerto) {
          ...
          // Overview is a rollup object; "healthy" is binary — no open
          // critical issues across the estate.
          setPlatformHealthy((r.data?.totals?.criticalAlerts || 0) === 0 ? (r.data?.totals?.clusters ?? rows.length) : 0);
        } else {
  ...
  useEffect(() => {
    const loadPlatforms = () => client.get('/settings')
      ...
      .then(r => setEnabledPlatformIds([
        ...
        ...(r.data.platformNutanixEnabled ? ['nutanix'] : []),
  ...
  const baseNavGroups = isOps ? opsNavGroups
    : isPure ? pureNavGroups : isNetapp ? netappNavGroups : isZerto ? zertoNavGroups : isVcenter ? vcenterNavGroups : isDell ? dellNavGroups : isAria ? ariaNavGroups : isAriaOps ? ariaopsNavGroups : isNetbackup ? netbackupNavGroups : isAws ? awsNavGroups : isNutanix ? nutanixNavGroups
  ...
  const noun = isNetapp ? 'cluster' : isZerto ? 'site' : isVcenter ? 'host' : isDell ? 'device' : isAria ? 'instance' : isAriaOps ? 'resource' : isNetbackup ? 'server' : isAws ? 'instance' : isNutanix ? 'cluster' : 'array';
  ...
  return (
    <div className="h-screen flex flex-row bg-transparent overflow-hidden">
      ...
      <aside className={`${collapsed ? 'w-[60px]' : 'w-[218px]'} bg-surface-base/80 border-r border-cohesity-border flex flex-col flex-shrink-0 transition-all duration-200`}>
        <BrandMark
          collapsed={collapsed}
          label={isOps ? 'Operations' : isPure ? 'Pure' : isNetapp ? 'NetApp' : isZerto ? 'Zerto' : isVcenter ? 'vCenter' : isDell ? 'Dell' : isAria ? 'Aria' : isAriaOps ? 'Aria Ops' : isNetbackup ? 'NetBackup' : isAws ? 'AWS' : isNutanix ? 'Nutanix' : activePluginPlatform ? activePluginPlatform.label : 'Cohesity'}
          accent={isPure ? '#FF6B00' : isNetapp ? '#0067C5' : isZerto ? '#EE3124' : isVcenter ? '#0091DA' : isDell ? '#007DB8' : isAria ? '#00A2C7' : isAriaOps ? '#78BE20' : isNetbackup ? '#B1181E' : isAws ? '#FF9900' : isNutanix ? '#7855FA' : activePluginPlatform ? activePluginPlatform.color : undefined}
        />
