...
// every upstream response shape in services/ariaopsApi.js is UNVERIFIED —
// the probe route (routes/ariaops.js) exists to see real shapes against a
// live instance. Resources/alerts are replaced per instance each poll (a
// full-fidelity dataset each cycle keeps stale rows from lingering);
// ariaops_metrics_history accumulates snapshots for trends.
module.exports = [
  {
    version: 1,
    up(db) {
  ...
  },
  {
    ...
    up(db) {
      db.exec(`
        CREATE TABLE IF NOT EXISTS ariaops_ai_reports (
