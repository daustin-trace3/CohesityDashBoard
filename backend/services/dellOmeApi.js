...
function baseClient(ome, headers = {}) {
  return axios.create({
    ...
    timeout: 60000,
    headers,
    // The login body and the X-Auth-Token ride this client: never follow a
    // redirect to another host with them.
    maxRedirects: 0,
...
async function testConnection(candidate) {
  ...
  try {
    ...
    sessions.delete(probe.id);
    logger.debug(`[dellOmeApi] connection test failed for ${candidate.host}: ${err.message}`);
