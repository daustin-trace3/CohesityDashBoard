...
async function hwlogPage(ome, coreApi, path, skip, top) {
  try {
    const d = await oGet(ome, coreApi, `${path}?$top=${top}&$skip=${skip}`);
    return { rows: d?.value || [], count: d?.['@odata.count'] ?? null, top };
  } catch (err) {
    if (top > 100) return hwlogPage(ome, coreApi, path, skip, 100);
    if (skip === 0) {
      const d = await oGet(ome, coreApi, path);
...
async function fetchHardwareLogsSince(ome, coreApi, deviceId, lastSeq = null, { top = 500, maxPages = 8, backlogMaxPages = 40 } = {}) {
  const path = `/api/DeviceService/Devices(${deviceId})/HardwareLogs`;
  const fullWalk = () => fetchHardwareLogs(ome, coreApi, deviceId, backlogMaxPages);
  if (lastSeq == null) return fullWalk();

  const first = await hwlogPage(ome, coreApi, path, 0, top);
  ...
  if (newestFirst) {
    ...
    for (let p = 1; p < maxPages && page.rows.length === size; p++) {
      ...
      page = await hwlogPage(ome, coreApi, path, p * size, size);
      out.push(...page.rows);
    }
  ...
  for (let p = 0; p < maxPages; p++) {
    const page = skip === 0 ? first : await hwlogPage(ome, coreApi, path, skip, size);
    out.push(...page.rows);
    const min = Math.min(...page.rows.map((r) => seqOf(r) ?? Infinity));
