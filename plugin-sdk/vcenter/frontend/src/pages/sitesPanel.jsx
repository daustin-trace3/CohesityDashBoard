...
export function SitesSection({ flash }) {
  ...
  const autoCreate = async () => {
    ...
    try {
      ...
      await load();
      flash?.('success', `Mapped ${r.mapped} cluster(s)`, `${r.created} vCenter site(s) created.`);
    } catch (err) {
      ...
      {sites == null ? <LoadingPanel label="Loading sites…" height={100} /> : (
        <>
          <div className="flex flex-wrap items-end gap-2 mb-4">
            ...
            <button onClick={autoCreate} disabled={saving || unmappedCount === 0} title="Every unmapped cluster is mapped to a site named after its vCenter (created if missing)" className="vc-btn-ghost">
