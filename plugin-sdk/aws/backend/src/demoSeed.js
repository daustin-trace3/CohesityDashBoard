...
//
// Ported from backend/demo/generators/aws.js. ALL inserts here run ONLY
// behind the DASHBOARD_DEMO==='1' gate — see seedAwsDemo() below, called
...
// service directly.
//
// DEVIATION FROM THE BUILT-IN's wipe strategy: aws_accounts itself is NEVER
// wiped/deleted (it is the user-facing connection table — an admin could
// register a real AWS account on a demo instance and that must survive a
// reseed). Instead the one fixture account is upserted by name so its id
// stays stable across boots, and only ITS dependent rows (scoped by
// account_id) are wiped before reseeding — any independently-registered real
// account's data is untouched. aws_health_events carries no account_id (it's
// a global status-feed cache, not user data) so it is wiped in full each
// reseed — same treatment as a poller-derived cache table.
...
// Children->parents, scoped by account_id — aws_accounts itself is NEVER
// wiped (see module header).
const DEMO_CHILD_TABLES = [
  'aws_optimizer_recommendations', 'aws_cost_instance_type_daily', 'aws_cost_usage_daily',
  'aws_rds_storage_history', 'aws_s3_size_history', 'aws_issue_history',
  'aws_metrics_history', 'aws_cost_daily', 'aws_bedrock_usage',
  'aws_subnets', 'aws_vpcs', 'aws_ecr_repos', 'aws_dynamo_tables', 'aws_lambda_functions',
  'aws_rds_instances', 'aws_s3_buckets', 'aws_ecs_services', 'aws_ecs_clusters',
  'aws_lightsail_instances', 'aws_ebs_volumes', 'aws_ec2_instances',
];
...
function seedAws(db, { now, encrypt }) {
  ...
  // Upsert the fixture account by name so its id stays stable across
  // reseeds (aws_accounts is never deleted — see module header).
  db.prepare(`
    ...
    ON CONFLICT(name) DO UPDATE SET
      access_key_id = excluded.access_key_id, encrypted_credentials = excluded.encrypted_credentials,
      last_poll_status = excluded.last_poll_status, last_poll_error = excluded.last_poll_error,
      last_poll_at = excluded.last_poll_at, last_cost_capture_at = excluded.last_cost_capture_at,
      last_s3_capture_at = excluded.last_s3_capture_at, last_optimizer_capture_at = excluded.last_optimizer_capture_at,
      co_enrollment = excluded.co_enrollment, updated_at = datetime('now')
  `).run(
  ...
  // independently-registered real account's data is untouched.
  for (const table of DEMO_CHILD_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE account_id = ?`).run(accountId);
  }
  // aws_health_events carries no account_id (global status-feed cache) —
  // wipe in full, same as any other poller-derived cache table.
  db.prepare('DELETE FROM aws_health_events').run();
  ...
  // ── Cost instance-type breakdown done above; AWS health events: one <24h old, one 3 days old ──
  ...
  return {
    accountId, dayBeforeTotal, yesterdayTotal, subnetRows, costRows, bedrockRows, metricsRows,
    s3HistoryRows, rdsHistoryRows, costUsageRows, costInstanceTypeRows, healthRows, optimizerRows,
  };
}

/** Demo-only entry point. Upserts the one fixture account (id stable across
 ...
 *  aging into a stale-looking estate. Reconciles issue history through
 *  ./issues (threaded coreApi) so the Overview issues panel demos every
 *  computed rule. NEVER runs outside demo mode — see the DASHBOARD_DEMO gate
 *  in poller.js. */
function seedAwsDemo(coreApi) {
  const db = coreApi.db;
  const result = db.transaction(() => seedAws(db, { now: Date.now(), encrypt: coreApi.encryption.encrypt }))();
  ...
  try {
    ...
    coreApi.logger.warn(`[demoSeed] aws issue reconcile failed: ${err.message}`);
  }
  ...
  return {
    ...
    vpcs: VPCS.length,
    subnets: result.subnetRows,
    costRows: result.costRows,
    bedrockRows: result.bedrockRows,
    metricsRows: result.metricsRows,
    issueRows,
    s3HistoryRows: result.s3HistoryRows,
    rdsHistoryRows: result.rdsHistoryRows,
    costUsageRows: result.costUsageRows,
    costInstanceTypeRows: result.costInstanceTypeRows,
    healthRows: result.healthRows,
    optimizerRows: result.optimizerRows,
  };
}

module.exports = { seedAws, seedAwsDemo };
