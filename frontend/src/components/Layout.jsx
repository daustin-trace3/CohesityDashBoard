...
import {
  Bell, Server, HardDrive, PanelLeftClose, PanelLeftOpen, Hexagon, ShieldCheck, Settings, LogOut, Activity, Crosshair, Waypoints, LayoutGrid, ChevronDown, HelpCircle, HeartPulse,
} from 'lucide-react';
...
export default function Layout() {
  ...
  const isPlatform = isPure || isNetapp || isZerto || isVcenter || isDell || isAria || isAriaOps || isAws || isUnifi || isBrocade || isBluecat;
  const platformKey = isPure ? 'pure' : isNetapp ? 'netapp' : isZerto ? 'zerto' : isVcenter ? 'vcenter' : isDell ? 'dell' : isAria ? 'aria' : isAriaOps ? 'ariaops' : isAws ? 'aws' : isUnifi ? 'unifi' : isBrocade ? 'brocade' : isBluecat ? 'bluecat' : null;
  const platformLabel = isPure ? 'Pure Array' : isNetapp ? 'NetApp Cluster' : isZerto ? 'Zerto Site' : isVcenter ? 'ESX Host' : isDell ? 'Device' : isAria ? 'Deployment' : isAriaOps ? 'Resource' : isAws ? 'Instance' : isUnifi ? 'Device' : isBrocade ? 'Fabric' : isBluecat ? 'Network' : '';
  ...
  useEffect(() => {
    ...
    const overviewUrl = pureFleet ? '/pure1/overview' : zerto ? '/zerto/sites' : vcenter ? '/vcenter/hosts' : dell ? '/dell/devices' : aria ? '/aria/deployments' : ariaops ? '/ariaops/resources' : aws ? '/aws/overview' : unifi ? '/unifi/overview' : brocade ? '/brocade/overview' : bluecat ? '/bluecat/overview' : `/${platformKey}/overview`;
    const alertsUrl = pureFleet ? '/pure1/alerts' : (vcenter || dell) ? `/${platformKey}/overview` : aria ? '/aria/issues' : ariaops ? '/ariaops/alerts' : aws ? '/aws/issues' : unifi ? '/unifi/issues' : brocade ? '/brocade/issues' : bluecat ? '/bluecat/issues' : `/${platformKey}/alerts`;
  ...
  const baseNavGroups = isOps ? opsNavGroups
    : isPure ? pureNavGroups : isNetapp ? netappNavGroups : isZerto ? zertoNavGroups : isVcenter ? vcenterNavGroups : isDell ? dellNavGroups : isAria ? ariaNavGroups : isAriaOps ? ariaopsNavGroups : isAws ? awsNavGroups : isUnifi ? unifiNavGroups : isBrocade ? brocadeNavGroups : isBluecat ? bluecatNavGroups
  ...
  const noun = isNetapp ? 'cluster' : isZerto ? 'site' : isVcenter ? 'host' : isDell ? 'device' : isAria ? 'instance' : isAriaOps ? 'resource' : isAws ? 'instance' : isUnifi ? 'device' : isBrocade ? 'fabric' : isBluecat ? 'network' : 'array';
  ...
  return (
    <div className="h-screen flex flex-row bg-transparent overflow-hidden">
      ...
      <aside className={`${collapsed ? 'w-[60px]' : 'w-[218px]'} bg-surface-base/80 border-r border-cohesity-border flex flex-col flex-shrink-0 transition-all duration-200`}>
        <BrandMark
          collapsed={collapsed}
          label={isOps ? 'Operations' : isPure ? 'Pure' : isNetapp ? 'NetApp' : isZerto ? 'Zerto' : isVcenter ? 'vCenter' : isDell ? 'Dell' : isAria ? 'Aria' : isAriaOps ? 'Aria Ops' : isAws ? 'AWS' : isUnifi ? 'Ubiquiti UniFi' : isBrocade ? 'Brocade SAN' : isBluecat ? 'BlueCat' : activePluginPlatform ? activePluginPlatform.label : 'Cohesity'}
          accent={isPure ? '#FF6B00' : isNetapp ? '#0067C5' : isZerto ? '#EE3124' : isVcenter ? '#0091DA' : isDell ? '#007DB8' : isAria ? '#00A2C7' : isAriaOps ? '#78BE20' : isAws ? '#FF9900' : isUnifi ? '#006FFF' : isBrocade ? '#CC092F' : isBluecat ? '#0057B8' : activePluginPlatform ? activePluginPlatform.color : undefined}
        />
