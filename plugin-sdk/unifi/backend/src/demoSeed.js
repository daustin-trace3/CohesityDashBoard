...
// EVERY rule in issues.js: an offline switch, a PoE fault, a port with
// climbing rx_errors in the trailing 24h, a flapping port, a gateway at 93%
// cpu, an overheating switch, WAN latency/availability breaches on
// Lakehouse, a rogue (evil-twin) AP, IPS disabled on Lakehouse, 4
// low-satisfaction wireless clients, and an upgradable AP. Wired client
// names vra-prod/vra-dr/vrops-nyc-01/vrli-nyc-01 mirror the vcenter demo VMs
// for cross-platform Server 360 hits.
//
// Ported from backend/demo/generators/unifi.js. ALL inserts here run ONLY
// behind the DASHBOARD_DEMO==='1' gate — see seedUnifiDemo() below, called
...
// backend/services/unifiIssues; only the seeded-random helpers were copied
// from the host's demo/generators/core.js (./demoRng.js) — no seedCore
// requires. Credential encryption uses coreApi.encryption.encrypt (nutanix
...
function seedUnifi(db, { now, encrypt, coreApi }) {
  // Port fix (WP1): LOW_SATISFACTION is a module-level const mutated via
  // `_used` flags during client generation. Left unreset, a second seed call
  // in the same process (demoSeed's own reseed-on-boot loop, or this smoke
  // test) would find every entry already "used" and silently drop the
  // wifi-experience issue — breaking the idempotency the contract requires.
  // Not present as a bug in the built-in generator only because it's loaded
  // once per process there; reset here defensively.
  LOW_SATISFACTION.forEach((l) => { delete l._used; });
  ...
  SOURCES.forEach((s) => {
    ...
    // health_json.ips.enabled is the additive column issues.js reads for
  ...
  Object.values(devicesByKey).forEach((d) => {
    ...
    if (d.portCount) {
      const ports = d.isGateway ? buildGatewayPorts(rng, d.portCount) : buildSwitchPorts(rng, d.portCount, offline);
  ...
  try {
    ...
    console.error(`[demoSeed] unifi issue history reconcile skipped: ${err.message}`);
  }
...
// Demo-only entry point. Wipes the unifi_* estate (children before parents)
...
const DEMO_TABLES = [
  'unifi_issue_history', 'unifi_client_seen', 'unifi_metrics_history', 'unifi_cameras',
  'unifi_topology', 'unifi_wan', 'unifi_firewall_rules', 'unifi_rogue_aps', 'unifi_events',
  'unifi_wlans', 'unifi_networks', 'unifi_port_history', 'unifi_ports', 'unifi_clients',
  'unifi_devices', 'unifi_sources',
];

function seedUnifiDemo(coreApi) {
  const db = coreApi.db;
  return db.transaction(() => {
    ...
    return seedUnifi(db, { now: Date.now(), encrypt: coreApi.encryption.encrypt, coreApi });
  })();
}

module.exports = { seedUnifi, seedUnifiDemo };
