// BlueCat Address Manager plugin manifest.
//
// Migrations copied VERBATIM (same scope id 'bluecat') so an existing local
// DB's bluecat_* data (and its schema_migrations row) is adopted intact on
// install.
//
...
// server360Suggest - from backend/platforms/bluecat/index.js.
const migrations = require('./migrations');
const { createRouter } = require('./router');
const { createBluecatPoller } = require('./poller');
...
module.exports = {
  ...
  createPoller(coreApi) {
    return createBluecatPoller(coreApi);
  },
