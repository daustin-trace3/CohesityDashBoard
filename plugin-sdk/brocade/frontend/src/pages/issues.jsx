...
// IssueAlertsPage (frontend/src/components/IssueAlertsPage.jsx) is a host
// component and not importable into the plugin sandbox — per contract §WP2.6
// this is a minimal read-only issues + history view against
// /api/brocade/issues + /api/brocade/issue-history (dell alerts.jsx pattern).
...
export default function BrocadeIssuesPage() {
  ...
  const load = React.useCallback(() => Promise.all([
    client.get('/brocade/issues').then(({ data }) => setIssues(Array.isArray(data) ? data : data?.issues || [])),
    client.get('/brocade/issue-history', { params: { days } }).then(({ data }) => setHistory(data)).catch(() => setHistory([])),
  ]).then(() => setLastRefreshed(new Date()))
    .catch(() => { setIssues([]); toast({ type: 'error', title: 'Failed to load alerts' }); }), [toast, days]);

  React.useEffect(() => { load(); }, [load]);

  return (
    <div className="animate-fade-in">
      <PageHeader icon={ClipboardCheck} title="Issues & Alerts" description="Computed fabric issues across all registered SANnav sources, with open/resolved history">
