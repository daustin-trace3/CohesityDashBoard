// UniFi plugin chart kit — renders via the host's Chart.js instance
// (window.Chart, injected by esbuild define; see plugin-sdk/build.mjs).
// Unlike plugin-sdk/nutanix's charts.jsx (which takes an abstracted
// `series` prop), the pinned contract for this plugin passes Chart.js
// `data`/`options` straight through — pages build the same chart.js
// dataset shapes the built-in unifi pages already build for react-chartjs-2.
// Dark theme defaults are deep-merged underneath whatever options a page
// supplies, so pages can omit scales/plugins entirely and still look right.
