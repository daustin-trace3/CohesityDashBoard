...
// Portaled to <body> WHEN the host exposes createPortal (portalOrInline
// falls back to an inline fixed overlay otherwise — see ui.jsx). The page
// wrapper's fade-in animation leaves a transform applied (fill-mode: both),
// which would re-anchor position:fixed to the page div and cut off the
// modal top on scrolled/short pages.
function ProbeModal({ instance, onClose }) {
  ...
  React.useEffect(() => {
    setResult(null);
    setError(false);
    apiFetch(`/ariaops/instances/${instance.id}/probe`)
...
export default function AriaOpsSettingsPage() {
  ...
  const [form, setForm] = React.useState({ name: '', host: '', username: '', password: '', authSource: '', sslVerify: false, pollingIntervalMinutes: 15 });
  ...
  const loadInstances = () => apiFetch('/ariaops/instances')
  ...
  const test = async () => {
    ...
    setTestResult(null);
    try {
      const json = await apiFetch('/ariaops/instances/test', {
        method: 'POST',
        body: {
  ...
  const add = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await apiFetch(`/ariaops/instances/${editingId}`, { method: 'PUT', body });
        flash('success', 'Aria Operations instance updated', form.password ? 'Credentials replaced — next poll uses them.' : 'Saved. Stored password unchanged.');
      } else {
        body.password = form.password;
        await apiFetch('/ariaops/instances', { method: 'POST', body });
        flash('success', 'Aria Operations instance registered', 'First poll started — data appears shortly.');
      }
      setEditingId(null);
      blankForm();
      await loadInstances();
  ...
  const remove = async (o) => {
    ...
    try {
      await apiFetch(`/ariaops/instances/${o.id}`, { method: 'DELETE' });
      await loadInstances();
  ...
  const refresh = async (o) => {
    ...
    try {
      await apiFetch(`/ariaops/instances/${o.id}/refresh`, { method: 'POST', body: {} });
      await loadInstances();
