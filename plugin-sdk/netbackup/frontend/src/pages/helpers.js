// Ported verbatim from frontend/src/pages/netbackup/helpers.js — pure
// functions, no host imports, so no adaptation needed.
...
/* ────────────────────────────────────────────────────────────────────────
 * Plugin-sandbox API helpers (not in the host original — proxmox pattern).
 * Every mutating call spreads window.__ICC_CSRF_TOKEN__ as x-csrf-token.
 * ────────────────────────────────────────────────────────────────────── */
export function apiGet(path, params) {
  const qs = params ? `?${new URLSearchParams(params)}` : '';
  return fetch(`/api/netbackup${path}${qs}`, { credentials: 'include' }).then((res) => {
    if (!res.ok) return res.json().catch(() => ({})).then((body) => { throw Object.assign(new Error(body.error || `request failed: ${res.status}`), { body }); });
    return res.json();
  });
}

export function apiSend(path, method, body, opts) {
  const csrf = typeof window !== 'undefined' ? window.__ICC_CSRF_TOKEN__ : null;
  return fetch(`/api/netbackup${path}`, {
    method,
    credentials: 'include',
