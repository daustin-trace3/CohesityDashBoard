// Ported from frontend/src/pages/pure/PureEstatePage.jsx.
import { LayoutList, Search, Download } from '../icons.jsx';
import {
  apiFetch, PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated,
  BRAND, fmtBytes, fmtNum, fmtRatio, severityTone,
} from '../ui.jsx';
...
export default function PureEstatePage() {
  ...
  const estate = React.useMemo(() => (rows || []).map((r) => {
  ...
  const models = React.useMemo(() => [...new Set(estate.map((r) => r.model).filter(Boolean))].sort(), [estate]);
    ...
    <div className="animate-fade-in">
      <PageHeader icon={LayoutList} title="Pure Estate Overview" description="Every Pure array — serials, versions, capacity, health — on one filterable page">
        <div className="flex items-center gap-2">
          ...
          <button onClick={exportCsv} disabled={filtered.length === 0} className="pu-btn-ghost">
      ...
      <div className="panel p-3 mb-4 flex flex-wrap items-center gap-2" style={{ borderTop: `3px solid ${BRAND}` }}>
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="pu-input" style={{ width: 'auto' }}>
        ...
        {models.length > 1 && (
          <select value={modelFilter} onChange={(e) => setModelFilter(e.target.value)} className="pu-input" style={{ width: 'auto' }}>
