...
function ProbeModal({ source, onClose }) {
  const [section, setSection] = React.useState('fabrics');
  ...
  React.useEffect(() => { runProbe(section); }, [source.id]); // eslint-disable-line react-hooks/exhaustive-deps
...
function FosOverridesPanel({ sourceId, switches, toast }) {
  ...
  const [form, setForm] = React.useState(blankOverride());
  const [saving, setSaving] = React.useState(false);

  const load = React.useCallback(() => client.get(`/brocade/sources/${sourceId}/fos-overrides`)
...
export default function BrocadeSettingsPage() {
  ...
  const [testingFos, setTestingFos] = React.useState(false);
  const [fosTestResult, setFosTestResult] = React.useState(null);
  const [switches, setSwitches] = React.useState([]);

  const [config, setConfig] = React.useState(null);
  const [savingConfig, setSavingConfig] = React.useState(false);

  const loadSources = React.useCallback(() => client.get('/brocade/sources')
