  ...
  const item = (s) => {
    ...
    return (
      <button key={s.key} onClick={() => onSelect(s.key)} aria-current={on ? 'page' : undefined}
        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 8, textAlign: 'left', fontSize: 12, fontWeight: on ? 600 : 400, border: 'none', cursor: 'pointer', background: on ? 'var(--px-surface-overlay)' : 'transparent', color: on ? 'var(--px-ink)' : 'var(--px-ink-muted)' }}>
        {Icon && <Icon size={13} style={{ color: on ? 'var(--px-brand)' : 'var(--px-ink-faint)' }} />}
        {s.label}
      </button>
    );
  };
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-start' }}>
      <nav aria-label={label ? label + ' settings sections' : 'Settings sections'}
        style={{ width: 224, flexShrink: 0, padding: 8, borderRadius: 12, background: 'var(--px-surface)', border: '1px solid var(--px-border)', borderTop: '3px solid ' + brand }}>
        {groups.map((g) => (
          <div key={g || 'all'} style={{ marginBottom: grouped ? 8 : 0 }}>
            {g && <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--px-ink-faint)', padding: '4px 8px 0', margin: '0 0 4px' }}>{g}</p>}
