// BlueCat plugin style kit. No Tailwind build step is available inside a
// plugin bundle, so this installs a hand-rolled utility stylesheet that
// mirrors the class names the built-in frontend/src/pages/bluecat/* pages
...
// plugin-sdk/brocade/frontend/src/ui.jsx, renamed bc- -> blc- (bc-root is
// already taken by the Brocade pack), rebranded to BlueCat navy (#0057B8).
// ToastHost/useToast ported the same way (the built-in BlueCat pages call
// useToast() throughout; the plugin sandbox has no host toast system).
//
...
const STYLE_ID = 'blc-plugin-styles';
...
const CSS = `
:root {
  --blc-surface-base: #0B1015;
  --blc-surface: #131B23;
  --blc-surface-raised: #18222C;
  --blc-surface-overlay: #1E2A36;
  --blc-border: #1F2B37;
  --blc-gray: #131B23;
  --blc-ink: #E8EDF2;
  --blc-ink-muted: #94A3B3;
  --blc-ink-faint: #5F7081;
  --blc-brand: ${BRAND};
  --blc-brand-dark: #003D85;
  --blc-ok: #34D399;
  --blc-warn: #FBBF24;
  --blc-crit: #F87171;
  --blc-info: #60A5FA;
...
.blc-root { font-family: inherit; color: var(--blc-ink); }

/* component classes used by ported page bodies */
.panel { background: var(--blc-surface); border: 1px solid var(--blc-border); border-radius: 12px; box-shadow: 0 1px 2px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.02); }
...
.animate-fade-in { animation: blc-fade-in 220ms ease-out both; }
.animate-spin { animation: blc-spin 0.8s linear infinite; }
.animate-pulse { animation: blc-pulse 1.4s ease-in-out infinite; }
@keyframes blc-fade-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
@keyframes blc-spin { to { transform: rotate(360deg); } }
@keyframes blc-pulse { 0%, 100% { opacity: 1; } 50% { opacity: .4; } }

/* colors */
.text-ink { color: var(--blc-ink); }
.text-ink-muted { color: var(--blc-ink-muted); }
.text-ink-faint { color: var(--blc-ink-faint); }
.text-brand { color: var(--blc-brand); }
.text-status-ok { color: var(--blc-ok); }
.text-status-warn { color: var(--blc-warn); }
.text-status-crit { color: var(--blc-crit); }
.text-status-bad { color: var(--blc-crit); }
.text-status-info { color: var(--blc-info); }
...
.bg-brand { background: var(--blc-brand); }
.bg-brand\\/10 { background: rgba(0,87,184,.1); }
.bg-brand\\/15 { background: rgba(0,87,184,.15); }
.bg-brand\\/20 { background: rgba(0,87,184,.2); }
.bg-cohesity-gray { background: var(--blc-gray); }
.bg-surface-overlay { background: var(--blc-surface-overlay); }
.bg-surface { background: var(--blc-surface); }
.bg-white { background: #fff; }
${statusColorCss()}

/* borders */
.border { border: 1px solid var(--blc-border); }
.border-b { border-bottom: 1px solid var(--blc-border); }
.border-t { border-top: 1px solid var(--blc-border); }
.border-l { border-left: 1px solid var(--blc-border); }
.border-transparent { border-color: transparent; }
.border-cohesity-border { border-color: var(--blc-border); }
.border-cohesity-border\\/40 { border-color: rgba(31,43,55,.4); }
.border-cohesity-border\\/50 { border-color: rgba(31,43,55,.5); }
.border-brand\\/20 { border-color: rgba(0,87,184,.2); }
.border-brand\\/25 { border-color: rgba(0,87,184,.25); }
.border-brand\\/30 { border-color: rgba(0,87,184,.3); }
.border-brand\\/40 { border-color: rgba(0,87,184,.4); }
.border-brand\\/50 { border-color: rgba(0,87,184,.5); }
.border-brand\\/60 { border-color: rgba(0,87,184,.6); }
...
.shadow-panel { box-shadow: 0 1px 2px rgba(0,0,0,.4); }
...
.accent-brand { accent-color: var(--blc-brand); }
.accent-status-crit { accent-color: var(--blc-crit); }
.opacity-40 { opacity: .4; }
.opacity-50 { opacity: .5; }
.ring-1 { box-shadow: 0 0 0 1px var(--blc-ring, transparent); }
.ring-2 { box-shadow: 0 0 0 2px var(--blc-ring, transparent); }
.ring-brand\\/30 { --blc-ring: rgba(0,87,184,.3); }
.ring-brand\\/60 { --blc-ring: rgba(0,87,184,.6); }
.ring-brand\\/70 { --blc-ring: rgba(0,87,184,.7); }
.ring-cohesity-border { --blc-ring: var(--blc-border); }

/* hover / focus / disabled */
.hover\\:bg-surface-overlay:hover { background: var(--blc-surface-overlay); }
.hover\\:bg-brand\\/10:hover { background: rgba(0,87,184,.1); }
.hover\\:bg-brand\\/20:hover { background: rgba(0,87,184,.2); }
.hover\\:border-brand\\/40:hover { border-color: rgba(0,87,184,.4); }
.hover\\:border-brand\\/50:hover { border-color: rgba(0,87,184,.5); }
.hover\\:text-ink:hover { color: var(--blc-ink); }
.hover\\:text-white:hover { color: #fff; }
.hover\\:text-brand:hover { color: var(--blc-brand); }
.hover\\:text-status-crit:hover { color: var(--blc-crit); }
.hover\\:text-status-ok:hover { color: var(--blc-ok); }
.hover\\:underline:hover { text-decoration: underline; }
.hover\\:opacity-90:hover { opacity: .9; }
.hover\\:ring-1:hover { box-shadow: 0 0 0 1px var(--blc-ring, transparent); }
.hover\\:ring-brand\\/30:hover { --blc-ring: rgba(0,87,184,.3); }
.hover\\:-translate-y-0\\.5:hover { transform: translateY(-2px); }
.focus\\:border-brand\\/60:focus { border-color: rgba(0,87,184,.6); }
.disabled\\:opacity-50:disabled { opacity: .5; }
.disabled\\:opacity-40:disabled { opacity: .4; }
.disabled\\:cursor-default:disabled { cursor: default; }
.disabled\\:cursor-not-allowed:disabled { cursor: not-allowed; }

.blc-scroll::-webkit-scrollbar { width: 8px; height: 8px; }
.blc-scroll::-webkit-scrollbar-track { background: transparent; }
.blc-scroll::-webkit-scrollbar-thumb { background: #2A3845; border-radius: 4px; border: 2px solid var(--blc-surface-base); }
.blc-scroll::-webkit-scrollbar-thumb:hover { background: #3B4D5E; }

.blc-input {
  width: 100%;
  background: var(--blc-surface-overlay);
...
.blc-input:focus { border-color: rgba(0,87,184,0.6); }
...
.blc-btn-ghost {
  ...
  color: var(--blc-ink-muted);
  cursor: pointer;
  transition: color 150ms, border-color 150ms;
}
.blc-btn-ghost:hover { color: var(--blc-ink); border-color: rgba(0,87,184,0.4); }
.blc-btn-ghost:disabled { opacity: 0.5; cursor: default; }
...
/* Scope every rule under .blc-root so the utility vocabulary (w-full, flex,
 * panel, ...) cannot leak into HOST pages. Unscoped, this stylesheet loads
 * after the host's Tailwind CSS and its .w-full beats the host's own
 * responsive menu widths - which blanked the host Global Settings pages
 * (the unifi 1.0.2 bug). :root var declarations stay global (blc- prefixed,
  ...
  return parts.map((part, i) => {
    ...
    return part.replace(/(^|\{|\})(\s*)([^@{}]+?)(\s*\{)/g, (m, brace, ws, sel, open) => {
      const scoped = sel.split(',').map((s) => {
        const t = s.trim();
        if (!t || t === ':root' || t.startsWith('.blc-root')) return t;
        return `.blc-root ${t}`;
      }).join(', ');
...
/* ----------------------------------------------------------------------
 * Formatting helpers - ported from frontend/src/pages/bluecat/helpers.js,
 * merged here so ui.jsx is the single source (matches the dell/brocade
...
/* ----------------------------------------------------------------------
 * Inline-SVG loader glyph (icons.jsx owns the named icon set)
 * ---------------------------------------------------------------------- */
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
        {IconComp && (
          <div style={{ marginTop: 2, display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(0,87,184,0.1)', border: '1px solid rgba(0,87,184,0.2)', flexShrink: 0 }}>
            <IconComp size={18} style={{ color: 'var(--blc-brand)' }} />
          </div>
        )}
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--blc-ink)', lineHeight: 1.2, margin: 0 }}>{title}</h1>
          {description && <p style={{ fontSize: 12, color: 'var(--blc-ink-muted)', margin: '2px 0 0' }}>{description}</p>}
        </div>
      </div>
...
const TONE = {
  default: { icon: 'var(--blc-ink-muted)', iconBg: 'var(--blc-surface-overlay)', iconBorder: 'var(--blc-border)' },
  neutral: { icon: 'var(--blc-ink-muted)', iconBg: 'var(--blc-surface-overlay)', iconBorder: 'var(--blc-border)' },
  brand: { icon: 'var(--blc-brand)', iconBg: 'rgba(0,87,184,0.1)', iconBorder: 'rgba(0,87,184,0.2)' },
  ok: { icon: 'var(--blc-ok)', iconBg: 'rgba(52,211,153,0.1)', iconBorder: 'rgba(52,211,153,0.2)' },
  warn: { icon: 'var(--blc-warn)', iconBg: 'rgba(251,191,36,0.1)', iconBorder: 'rgba(251,191,36,0.2)' },
  crit: { icon: 'var(--blc-crit)', iconBg: 'rgba(248,113,113,0.1)', iconBorder: 'rgba(248,113,113,0.2)' },
  info: { icon: 'var(--blc-info)', iconBg: 'rgba(96,165,250,0.1)', iconBorder: 'rgba(96,165,250,0.2)' },
};
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--blc-ink-faint)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</p>
        {loading ? (
          <div style={{ height: 24, width: 80, marginTop: 4, borderRadius: 6, background: 'var(--blc-surface-overlay)' }} />
        ) : (
          <p className="tnum" style={{ fontSize: 20, fontWeight: 700, color: 'var(--blc-ink)', lineHeight: 1.2, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</p>
        )}
        {sub && !loading && (
          <p style={{ fontSize: 11, color: 'var(--blc-ink-muted)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sub}</p>
        )}
      </div>
    </Tag>
  );
...
const BADGE_TONES = {
  ok: { bg: 'rgba(52,211,153,0.1)', color: 'var(--blc-ok)', border: 'rgba(52,211,153,0.25)' },
  warn: { bg: 'rgba(251,191,36,0.1)', color: 'var(--blc-warn)', border: 'rgba(251,191,36,0.25)' },
  crit: { bg: 'rgba(248,113,113,0.1)', color: 'var(--blc-crit)', border: 'rgba(248,113,113,0.25)' },
  info: { bg: 'rgba(96,165,250,0.1)', color: 'var(--blc-info)', border: 'rgba(96,165,250,0.25)' },
  brand: { bg: 'rgba(0,87,184,0.1)', color: 'var(--blc-brand)', border: 'rgba(0,87,184,0.25)' },
  neutral: { bg: 'var(--blc-surface-overlay)', color: 'var(--blc-ink-muted)', border: 'var(--blc-border)' },
};

export function Badge({ tone = 'neutral', children, style, className }) {
  const t = BADGE_TONES[tone] || BADGE_TONES.neutral;
  return (
    <span className={cls('blc-chip', className)} style={{ background: t.bg, color: t.color, borderColor: t.border, ...style }}>
      {children}
    </span>
  );
}
...
export function Spinner({ size = 16, style }) {
  return <LoaderGlyph size={size} style={{ color: 'var(--blc-brand)', animation: 'blc-spin 0.8s linear infinite', ...style }} />;
}
...
export function LoadingPanel({ label = 'Loading data...', height = 200 }) {
  return (
    <div role="status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <Spinner size={22} />
      <p style={{ fontSize: 12, color: 'var(--blc-ink-muted)', margin: 0 }}>{label}</p>
    </div>
  );
}

export function ErrorPanel({ label = 'Failed to load data.', onRetry, height = 200 }) {
  return (
    <div role="alert" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <p style={{ fontSize: 12, color: 'var(--blc-crit)', margin: 0, textAlign: 'center' }}>{label}</p>
      {onRetry && (
        <button onClick={onRetry} className="blc-btn-ghost">Retry</button>
      )}
    </div>
  );
}
...
export function RefreshButton({ onClick, refreshing, label = 'Refresh' }) {
  return (
    <button onClick={onClick} disabled={refreshing} className="blc-btn-ghost">
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={refreshing ? { animation: 'blc-spin 0.8s linear infinite' } : undefined}>
...
export function LastUpdated({ date, prefix = 'Updated' }) {
  ...
  return (
    <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--blc-ink-faint)' }}>
...
export function Fact({ label, value }) {
  return (
    <div>
      <p style={{ margin: 0, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--blc-ink-faint)' }}>{label}</p>
      <p className="tnum" style={{ margin: 0, fontSize: 13, color: 'var(--blc-ink)' }}>{value ?? '-'}</p>
    </div>
  );
}
 ...
 * only) on older hosts, which has NO createPortal - calling it unguarded
 * crashes the page. The inline fixed overlay fallback renders identically
 * except on transformed ancestors.
 * ---------------------------------------------------------------------- */
export function portalOrInline(node) {
  // Portaled content lands outside the .blc-root wrapper, so re-wrap it -
  // otherwise the scoped stylesheet (see scopeCss) no longer applies.
  const wrapped = <div className="blc-root">{node}</div>;
  ...
  return portalOrInline(
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} role="dialog" aria-modal="true">
      ...
      <div className="panel" style={{ position: 'relative', width: maxWidth, maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderTop: `3px solid ${BRAND}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '16px 16px 12px', borderBottom: '1px solid var(--blc-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {IconComp && <IconComp size={17} style={{ color: 'var(--blc-brand)', flexShrink: 0 }} />}
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--blc-ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</p>
              {subtitle && <p style={{ margin: 0, fontSize: 11, color: 'var(--blc-ink-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</p>}
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            {headerExtra}
            <button onClick={onClose} aria-label="Close" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 28, width: 28, borderRadius: 8, border: 'none', background: 'transparent', color: 'var(--blc-ink-faint)', cursor: 'pointer' }}>
        ...
        <div className="blc-scroll" style={{ padding: 16, overflowY: 'auto' }}>{children}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <th className={cls('py-2 pr-3', align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left')} style={style}>
      <button
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'transparent', color: active ? 'var(--blc-ink)' : 'var(--blc-ink-muted)', padding: 0 }}
      >
        {label}
        {active && (
...
export function TableSearch({ ctl, placeholder = 'Search...', className }) {
  return (
    <div className={className || 'w-full max-w-md'} style={{ position: 'relative' }}>
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--blc-ink-faint)', pointerEvents: 'none' }}>
        <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
      </svg>
      <input value={ctl.q} onChange={(e) => ctl.setQ(e.target.value)} placeholder={placeholder} className="blc-input" style={{ paddingLeft: 32 }} />
    </div>
  );
}
...
export function FilterSelect({ ctl, k, rows, label }) {
  ...
  return (
    <select value={ctl.filters[k] || ''} onChange={(e) => ctl.setFilter(k, e.target.value)} className="blc-input" style={{ width: 'auto', cursor: 'pointer' }}>
...
export function TableControls({ ctl, rows, searchPlaceholder, filters = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      <span className="tnum" style={{ fontSize: 11, color: 'var(--blc-ink-faint)', marginLeft: 'auto' }}>
...
const pagerBtnStyle = { fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--blc-border)', background: 'transparent', color: 'var(--blc-ink-muted)', cursor: 'pointer' };
...
export function TablePager({ ctl, sizes = PAGE_SIZES }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, marginTop: 4, borderTop: '1px solid var(--blc-border)' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--blc-ink-faint)' }}>
        Rows per page
        <select value={String(ctl.pageSize)} onChange={(e) => ctl.setPageSize(e.target.value === 'all' ? 'all' : Number(e.target.value))} className="blc-input" style={{ width: 'auto', padding: '4px 8px', cursor: 'pointer' }}>
          {sizes.map((s) => <option key={s} value={String(s)}>{s === 'all' ? 'All' : s}</option>)}
        </select>
      </label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="tnum" style={{ fontSize: 11, color: 'var(--blc-ink-faint)' }}>{start}-{end} of {total}</span>
        {!all && ctl.pageCount > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span className="tnum" style={{ fontSize: 11, color: 'var(--blc-ink-faint)', padding: '0 4px' }}>{ctl.page + 1} / {ctl.pageCount}</span>
 ...
 * needed). Ported from plugin-sdk/brocade/frontend/src/ui.jsx (itself
 * ported from cohesity; dell has no toast system, but the built-in BlueCat
 * pages call useToast() throughout). <ToastHost/> is mounted once per
 * routed page by index.jsx's rooted() wrapper. API mirrors host
    ...
    <div aria-live="polite" style={{ position: 'fixed', bottom: 20, right: 20, zIndex: 90, display: 'flex', flexDirection: 'column', gap: 8, width: 340, maxWidth: 'calc(100vw - 2rem)' }}>
      {items.map((t) => (
        <div key={t.id} role="status" className="panel animate-fade-in" style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 14px', background: 'rgba(30,42,54,0.95)', backdropFilter: 'blur(6px)' }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--blc-ink)', margin: 0, lineHeight: 1.35 }}>{t.title}</p>
            {t.message && <p style={{ fontSize: 11, color: 'var(--blc-ink-muted)', margin: '2px 0 0', lineHeight: 1.5 }}>{t.message}</p>}
          </div>
          <button onClick={() => toastDismiss(t.id)} aria-label="Dismiss notification" style={{ flexShrink: 0, border: 'none', background: 'transparent', color: 'var(--blc-ink-faint)', cursor: 'pointer', padding: 0 }}>
