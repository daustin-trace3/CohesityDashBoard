...
function seedProxmox(db, { now, encrypt, coreApi }) {
  ...
  // ── Computed issue reconcile ────────────────────────────────────────────
  // Was `require('../../services/proxmoxIssues')` while this lived in the host
  // demo tree; inside the plugin that host path does not resolve, which
  // silently dropped every computed rule and left only the two hand-seeded
  // ones below. Now calls the plugin's own issues module with coreApi.
  let reconcileProxmoxIssues = null;
  try {
    ({ reconcileIssueHistory: reconcileProxmoxIssues } = require('./issues'));
  } catch (err) {
    console.error(`[demoSeed] proxmox issues module unavailable, skipping computed-issue reconcile: ${err.message}`);
  }
  if (reconcileProxmoxIssues && coreApi) {
    reconcileProxmoxIssues(coreApi);
  } else {
...
// Demo-only entry point. Wipes the proxmox_* estate (children before parents,
// FK order) and regenerates it with fresh relative timestamps, so a demo box
...
function seedProxmoxDemo(coreApi) {
  const db = coreApi.db;
  return db.transaction(() => {
    ...
    return seedProxmox(db, { now: new Date(), encrypt: coreApi.encryption.encrypt, coreApi });
  })();
}

module.exports = { seedProxmox, seedProxmoxDemo };
