// Dell plugin style kit. No Tailwind build step is available inside a
// plugin bundle, so this installs a hand-rolled utility stylesheet that
// mirrors the class *names* the built-in frontend/src/pages/dell/* pages
...
// plugin-sdk/unifi/frontend/src/ui.jsx exactly, renamed uf- -> dl- and
// rebranded to Dell blue (#007DB8 = rgb(0,125,184)).
//
...
const STYLE_ID = 'dl-plugin-styles';
...
const CSS = `
:root {
  --dl-surface-base: #0B1015;
  --dl-surface: #131B23;
  --dl-surface-raised: #18222C;
  --dl-surface-overlay: #1E2A36;
  --dl-border: #1F2B37;
  --dl-gray: #131B23;
  --dl-ink: #E8EDF2;
  --dl-ink-muted: #94A3B3;
...
.dl-root { font-family: inherit; color: var(--dl-ink); }

/* component classes used by ported page bodies */
.panel { background: var(--dl-surface); border: 1px solid var(--dl-border); border-radius: 12px; box-shadow: 0 1px 2px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.02); }
.tnum { font-variant-numeric: tabular-nums; }
.animate-fade-in { animation: dl-fade-in 220ms ease-out both; }
.animate-spin { animation: dl-spin 0.8s linear infinite; }
@keyframes dl-fade-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
@keyframes dl-spin { to { transform: rotate(360deg); } }

/* colors */
.text-ink { color: var(--dl-ink); }
.text-ink-muted { color: var(--dl-ink-muted); }
.text-ink-faint { color: var(--dl-ink-faint); }
.text-brand { color: var(--dl-brand); }
.text-status-ok { color: var(--dl-ok); }
.text-status-warn { color: var(--dl-warn); }
.text-status-crit { color: var(--dl-crit); }
.text-status-bad { color: var(--dl-crit); }
.text-status-info { color: var(--dl-info); }
.text-white { color: #fff; }
.text-white\\/80 { color: rgba(255,255,255,.8); }
.bg-black\\/40 { background: rgba(0,0,0,.4); }
.bg-black\\/50 { background: rgba(0,0,0,.5); }
.bg-black\\/60 { background: rgba(0,0,0,.6); }
.bg-brand\\/10 { background: rgba(0,125,184,.1); }
.bg-brand\\/20 { background: rgba(0,125,184,.2); }
...
.bg-cohesity-gray { background: var(--dl-gray); }
.bg-surface-overlay { background: var(--dl-surface-overlay); }
.bg-white { background: #fff; }

/* borders */
.border { border: 1px solid var(--dl-border); }
.border-b { border-bottom: 1px solid var(--dl-border); }
.border-t { border-top: 1px solid var(--dl-border); }
.border-l { border-left: 1px solid var(--dl-border); }
.border-transparent { border-color: transparent; }
.border-cohesity-border { border-color: var(--dl-border); }
.border-cohesity-border\\/40 { border-color: rgba(31,43,55,.4); }
.border-cohesity-border\\/50 { border-color: rgba(31,43,55,.5); }
.border-brand\\/20 { border-color: rgba(0,125,184,.2); }
.border-brand\\/30 { border-color: rgba(0,125,184,.3); }
.border-brand\\/40 { border-color: rgba(0,125,184,.4); }
.border-brand\\/50 { border-color: rgba(0,125,184,.5); }
...
.accent-brand { accent-color: var(--dl-brand); }
.accent-\\[\\#007DB8\\] { accent-color: #007DB8; }
.opacity-50 { opacity: .5; }
.ring-1 { box-shadow: 0 0 0 1px var(--dl-ring, transparent); }
.ring-2 { box-shadow: 0 0 0 2px var(--dl-ring, transparent); }
.ring-brand\\/30 { --dl-ring: rgba(0,125,184,.3); }
.ring-brand\\/70 { --dl-ring: rgba(0,125,184,.7); }

/* hover / focus / disabled */
.hover\\:bg-surface-overlay:hover { background: var(--dl-surface-overlay); }
.hover\\:bg-brand\\/10:hover { background: rgba(0,125,184,.1); }
.hover\\:bg-brand\\/20:hover { background: rgba(0,125,184,.2); }
.hover\\:border-brand\\/40:hover { border-color: rgba(0,125,184,.4); }
.hover\\:border-brand\\/50:hover { border-color: rgba(0,125,184,.5); }
.hover\\:border-status-crit\\/50:hover { border-color: rgba(248,113,113,.5); }
.hover\\:text-ink:hover { color: var(--dl-ink); }
.hover\\:text-white:hover { color: #fff; }
.hover\\:text-status-crit:hover { color: var(--dl-crit); }
.hover\\:underline:hover { text-decoration: underline; }
.hover\\:ring-1:hover { box-shadow: 0 0 0 1px var(--dl-ring, transparent); }
.hover\\:ring-brand\\/30:hover { --dl-ring: rgba(0,125,184,.3); }
.hover\\:-translate-y-0\\.5:hover { transform: translateY(-2px); }
.focus\\:border-brand\\/60:focus { border-color: rgba(0,125,184,.6); }
.disabled\\:opacity-50:disabled { opacity: .5; }
.disabled\\:cursor-default:disabled { cursor: default; }

.dl-scroll::-webkit-scrollbar { width: 8px; height: 8px; }
.dl-scroll::-webkit-scrollbar-track { background: transparent; }
.dl-scroll::-webkit-scrollbar-thumb { background: #2A3845; border-radius: 4px; border: 2px solid var(--dl-surface-base); }
.dl-scroll::-webkit-scrollbar-thumb:hover { background: #3B4D5E; }
...
.dl-input:focus { border-color: rgba(0,125,184,0.6); }
...
.dl-btn-ghost:hover { color: var(--dl-ink); border-color: rgba(0,125,184,0.4); }
.dl-btn-ghost:disabled { opacity: 0.5; cursor: default; }
...
/* Scope every rule under .dl-root so the utility vocabulary (w-full, flex,
 ...
 * (the unifi 1.0.2 bug). :root var declarations stay global (dl- prefixed,
  ...
  return parts.map((part, i) => {
    ...
    return part.replace(/(^|\{|\})(\s*)([^@{}]+?)(\s*\{)/g, (m, brace, ws, sel, open) => {
      const scoped = sel.split(',').map((s) => {
        const t = s.trim();
        if (!t || t === ':root' || t.startsWith('.dl-root')) return t;
        return `.dl-root ${t}`;
      }).join(', ');
  ...
  const item = (s) => {
    ...
    return (
      <button key={s.key} onClick={() => onSelect(s.key)} aria-current={on ? 'page' : undefined}
        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 8, textAlign: 'left', fontSize: 12, fontWeight: on ? 600 : 400, border: 'none', cursor: 'pointer', background: on ? 'var(--dl-surface-overlay)' : 'transparent', color: on ? 'var(--dl-ink)' : 'var(--dl-ink-muted)' }}>
        {Icon && <Icon size={13} style={{ color: on ? 'var(--dl-brand)' : 'var(--dl-ink-faint)' }} />}
        {s.label}
      </button>
    );
  };
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-start' }}>
      <nav aria-label={label ? label + ' settings sections' : 'Settings sections'}
        style={{ width: 224, flexShrink: 0, padding: 8, borderRadius: 12, background: 'var(--dl-surface)', border: '1px solid var(--dl-border)', borderTop: '3px solid ' + brand }}>
        {groups.map((g) => (
          <div key={g || 'all'} style={{ marginBottom: grouped ? 8 : 0 }}>
            {g && <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--dl-ink-faint)', padding: '4px 8px 0', margin: '0 0 4px' }}>{g}</p>}
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
        {IconComp && (
          <div style={{ marginTop: 2, display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(0,125,184,0.1)', border: '1px solid rgba(0,125,184,0.2)', flexShrink: 0 }}>
            <IconComp size={18} style={{ color: 'var(--dl-brand)' }} />
          </div>
        )}
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--dl-ink)', lineHeight: 1.2, margin: 0 }}>{title}</h1>
          {description && <p style={{ fontSize: 12, color: 'var(--dl-ink-muted)', margin: '2px 0 0' }}>{description}</p>}
        </div>
      </div>
...
const TONE = {
  default: { icon: 'var(--dl-ink-muted)', iconBg: 'var(--dl-surface-overlay)', iconBorder: 'var(--dl-border)' },
  neutral: { icon: 'var(--dl-ink-muted)', iconBg: 'var(--dl-surface-overlay)', iconBorder: 'var(--dl-border)' },
  brand: { icon: 'var(--dl-brand)', iconBg: 'rgba(0,125,184,0.1)', iconBorder: 'rgba(0,125,184,0.2)' },
  ok: { icon: 'var(--dl-ok)', iconBg: 'rgba(52,211,153,0.1)', iconBorder: 'rgba(52,211,153,0.2)' },
  warn: { icon: 'var(--dl-warn)', iconBg: 'rgba(251,191,36,0.1)', iconBorder: 'rgba(251,191,36,0.2)' },
  crit: { icon: 'var(--dl-crit)', iconBg: 'rgba(248,113,113,0.1)', iconBorder: 'rgba(248,113,113,0.2)' },
  info: { icon: 'var(--dl-info)', iconBg: 'rgba(96,165,250,0.1)', iconBorder: 'rgba(96,165,250,0.2)' },
};
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--dl-ink-faint)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</p>
        {loading ? (
          <div style={{ height: 24, width: 80, marginTop: 4, borderRadius: 6, background: 'var(--dl-surface-overlay)' }} />
        ) : (
          <p className="tnum" style={{ fontSize: 20, fontWeight: 700, color: 'var(--dl-ink)', lineHeight: 1.2, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</p>
        )}
        {sub && !loading && (
          <p style={{ fontSize: 11, color: 'var(--dl-ink-muted)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sub}</p>
        )}
      </div>
    </Tag>
  );
...
const BADGE_TONES = {
  ok: { bg: 'rgba(52,211,153,0.1)', color: 'var(--dl-ok)', border: 'rgba(52,211,153,0.25)' },
  warn: { bg: 'rgba(251,191,36,0.1)', color: 'var(--dl-warn)', border: 'rgba(251,191,36,0.25)' },
  crit: { bg: 'rgba(248,113,113,0.1)', color: 'var(--dl-crit)', border: 'rgba(248,113,113,0.25)' },
  info: { bg: 'rgba(96,165,250,0.1)', color: 'var(--dl-info)', border: 'rgba(96,165,250,0.25)' },
  brand: { bg: 'rgba(0,125,184,0.1)', color: 'var(--dl-brand)', border: 'rgba(0,125,184,0.25)' },
  neutral: { bg: 'var(--dl-surface-overlay)', color: 'var(--dl-ink-muted)', border: 'var(--dl-border)' },
};

export function Badge({ tone = 'neutral', children, style, className }) {
  const t = BADGE_TONES[tone] || BADGE_TONES.neutral;
  return (
    <span className={cls('dl-chip', className)} style={{ background: t.bg, color: t.color, borderColor: t.border, ...style }}>
      {children}
    </span>
  );
}
...
export function Spinner({ size = 16, style }) {
  return <LoaderGlyph size={size} style={{ color: 'var(--dl-brand)', animation: 'dl-spin 0.8s linear infinite', ...style }} />;
}
...
export function LoadingPanel({ label = 'Loading data…', height = 200 }) {
  return (
    <div role="status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <Spinner size={22} />
      <p style={{ fontSize: 12, color: 'var(--dl-ink-muted)', margin: 0 }}>{label}</p>
    </div>
  );
}
...
export function RefreshButton({ onClick, refreshing, label = 'Refresh' }) {
  return (
    <button onClick={onClick} disabled={refreshing} className="dl-btn-ghost">
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={refreshing ? { animation: 'dl-spin 0.8s linear infinite' } : undefined}>
...
export function LastUpdated({ date, prefix = 'Updated' }) {
  ...
  return (
    <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--dl-ink-faint)' }}>
...
export function Fact({ label, value }) {
  return (
    <div>
      <p style={{ margin: 0, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--dl-ink-faint)' }}>{label}</p>
      <p className="tnum" style={{ margin: 0, fontSize: 13, color: 'var(--dl-ink)' }}>{value ?? '—'}</p>
    </div>
  );
}
...
export function portalOrInline(node) {
  // Portaled content lands outside the .dl-root wrapper, so re-wrap it —
  // otherwise the scoped stylesheet (see scopeCss) no longer applies.
  const wrapped = <div className="dl-root">{node}</div>;
  ...
  return portalOrInline(
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} role="dialog" aria-modal="true">
      ...
      <div className="panel" style={{ position: 'relative', width: maxWidth, maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderTop: `3px solid ${BRAND}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '16px 16px 12px', borderBottom: '1px solid var(--dl-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {IconComp && <IconComp size={17} style={{ color: 'var(--dl-brand)', flexShrink: 0 }} />}
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--dl-ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</p>
              {subtitle && <p style={{ margin: 0, fontSize: 11, color: 'var(--dl-ink-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</p>}
            </div>
          </div>
          <button onClick={onClose} aria-label="Close" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 28, width: 28, borderRadius: 8, border: 'none', background: 'transparent', color: 'var(--dl-ink-faint)', cursor: 'pointer', flexShrink: 0 }}>
        ...
        <div className="dl-scroll" style={{ padding: 16, overflowY: 'auto' }}>{children}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <th className={cls('py-2 pr-3', align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left')} style={style}>
      <button
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'transparent', color: active ? 'var(--dl-ink)' : 'var(--dl-ink-muted)', padding: 0 }}
      >
        {label}
        {active && (
...
export function TableSearch({ ctl, placeholder = 'Search…', className }) {
  return (
    <div className={className || 'w-full max-w-md'} style={{ position: 'relative' }}>
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--dl-ink-faint)', pointerEvents: 'none' }}>
        <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
      </svg>
      <input value={ctl.q} onChange={(e) => ctl.setQ(e.target.value)} placeholder={placeholder} className="dl-input" style={{ paddingLeft: 32 }} />
    </div>
  );
}
...
export function FilterSelect({ ctl, k, rows, label }) {
  ...
  return (
    <select value={ctl.filters[k] || ''} onChange={(e) => ctl.setFilter(k, e.target.value)} className="dl-input" style={{ width: 'auto', cursor: 'pointer' }}>
...
export function TableControls({ ctl, rows, searchPlaceholder, filters = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      <span className="tnum" style={{ fontSize: 11, color: 'var(--dl-ink-faint)', marginLeft: 'auto' }}>
...
const pagerBtnStyle = { fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--dl-border)', background: 'transparent', color: 'var(--dl-ink-muted)', cursor: 'pointer' };
...
export function TablePager({ ctl, sizes = PAGE_SIZES }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, marginTop: 4, borderTop: '1px solid var(--dl-border)' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--dl-ink-faint)' }}>
        Rows per page
        <select value={String(ctl.pageSize)} onChange={(e) => ctl.setPageSize(e.target.value === 'all' ? 'all' : Number(e.target.value))} className="dl-input" style={{ width: 'auto', padding: '4px 8px', cursor: 'pointer' }}>
          {sizes.map((s) => <option key={s} value={String(s)}>{s === 'all' ? 'All' : s}</option>)}
        </select>
      </label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="tnum" style={{ fontSize: 11, color: 'var(--dl-ink-faint)' }}>{start}–{end} of {total}</span>
        {!all && ctl.pageCount > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span className="tnum" style={{ fontSize: 11, color: 'var(--dl-ink-faint)', padding: '0 4px' }}>{ctl.page + 1} / {ctl.pageCount}</span>
