// Computed Brocade SAN issues (shared by router.js and the manifest's
// opsSummary/collectAlerts hooks) plus their lifecycle history — ported from
// backend/services/brocadeIssues.js (unifiIssues.js model). Issue identity
// is `type|source|target` — stable across polls even as the message text
// changes. 14 rules. db/getSetting now come from coreApi rather than direct
// host requires.
...
const healthWarnScore = (coreApi) => clampedInt(coreApi, 'brocade_health_warn_score', 70, 1, 100);
const healthCritScore = (coreApi) => clampedInt(coreApi, 'brocade_health_crit_score', 50, 1, 100);
const certWarnDays = (coreApi) => clampedInt(coreApi, 'brocade_cert_warn_days', 60, 1, 365);
const eventStormCount = (coreApi) => clampedInt(coreApi, 'brocade_event_storm_count', 10, 1, 1000);
const eventRetentionDays = (coreApi) => clampedInt(coreApi, 'brocade_event_retention_days', 30, 1, 365);
...
function computeIssues(coreApi) {
  ...
  const warnScore = healthWarnScore(coreApi);
  const critScore = healthCritScore(coreApi);
  ...
  const warnDays = certWarnDays(coreApi);
  ...
  const stormCount = eventStormCount(coreApi);
