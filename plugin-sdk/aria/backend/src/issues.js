// Computed Aria issues (shared by router.js and poller.js) plus their
// lifecycle history — unifi/nutanix issues.js reconcile pattern (aria HAS an
// issue-history table, unlike dell). Ported from backend/services/
// ariaIssues.js — db/getSetting now come from coreApi rather than direct host
// requires.
function leaseWarnDays(coreApi) {
  const n = Number(coreApi.settings.getSetting('aria_lease_warn_days'));
  return Number.isFinite(n) && n >= 1 && n <= 60 ? Math.round(n) : 7;
}

function certWarnDays(coreApi) {
  const n = Number(coreApi.settings.getSetting('aria_cert_warn_days'));
  return Number.isFinite(n) && n >= 1 && n <= 365 ? Math.round(n) : 30;
}

function requestFailLookbackHours(coreApi) {
  const n = Number(coreApi.settings.getSetting('aria_request_fail_lookback_hours'));
...
function computeIssues(coreApi) {
  ...
  `).all();
  const leaseWarn = leaseWarnDays(coreApi);
  ...
  const lookbackHours = requestFailLookbackHours(coreApi);
