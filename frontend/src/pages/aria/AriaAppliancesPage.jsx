...
export default function AriaAppliancesPage() {
  ...
  const load = useCallback(() => client.get('/aria/appliances')
    .then(({ data }) => { setData(data); setLastRefreshed(new Date()); })
    .catch(() => { setData({ instances: [], suiteVms: [], vcenterConfigured: false }); toast({ type: 'error', title: 'Failed to load appliance data' }); }), [toast]);
