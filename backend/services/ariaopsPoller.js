...
const {
  fetchVersion, fetchNodeStatus, fetchResourcesByKind, fetchAlerts, fetchLatestStats, getToken,
} = require('./ariaopsApi');
...
async function safe(label, row, fn) {
  try {
    ...
    logger.debug(`[AriaOpsPoller] ${row.name}: ${label} failed (skipping): ${safeMsg(err)}`);
    return null;
  }
}
...
async function collect(row) {
  ...
  const version = await safe('version', row, () => fetchVersion(row));
  const nodeStatus = await safe('node status', row, () => fetchNodeStatus(row));
  const alerts = await safe('alerts', row, () => fetchAlerts(row));

  const resourcesByKind = {};
  for (const kind of RESOURCE_KINDS) {
    resourcesByKind[kind] = await safe(`resources:${kind}`, row, () => fetchResourcesByKind(row, kind));
  }
  ...
  if (vms) {
    ...
    statsById = ids.length ? await safe('latest stats', row, () => fetchLatestStats(row, ids)) : new Map();
  }
...
async function pollAriaOps(row) {
  try {
    ...
    logger.info(`[AriaOpsPoller] ${row.name}: ${vmCount} VM resource(s), ${(data.alerts || []).length} active alert(s)`);
  } catch (err) {
    db.prepare(`
...
const ariaopsPoller = createPoller({
  id: 'ariaops',
  loadSources: () => db.prepare('SELECT * FROM ariaops_instances').all(),
  intervalMinutes: (row) => row.polling_interval_minutes,
  poll: pollAriaOps,
});

function initAriaOpsPoller() {
  const sources = ariaopsPoller.init();
  logger.info(`[AriaOpsPoller] Initialized ${sources.length} Aria Operations instance(s)`);
  return ariaopsPoller;
}

module.exports = { initAriaOpsPoller, ariaopsPoller, pollAriaOps };
