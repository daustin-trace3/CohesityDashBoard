import { Grid3x3, X, Search, HeartPulse, LayoutGrid, Waypoints, Server, AlertTriangle, ShieldAlert } from '../icons.jsx';
import client from '../api.js';
import {
  useToast, PageHeader, Badge, StatCard, LoadingPanel, RefreshButton, LastUpdated, portalOrInline,
  BRAND, fmtWhen, statusTone, fmtStatus, severityTone, scoreTone, parseJsonArr,
} from '../ui.jsx';
...
function PortDetails({ port, peerInfo, onSelectPeer }) {
  ...
  const speed = port.speed ? `${port.speed}${Number(port.speedType) === 2 ? 'G' : ''}` : '—';
  ...
  return (
    <div className="space-y-2.5">
      ...
      <p className="text-xs text-ink-muted">Speed: <span className="text-ink tnum">{speed}</span></p>
...
function TooltipBody({ port, pos }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-ink-faint">Speed: {speed}</p>
      {device ? (
...
function Tooltip({ port, pos }) {
  ...
  return portalOrInline(<TooltipBody port={port} pos={pos} />);
}
...
function IslTable({ ports }) {
  const eports = React.useMemo(() => ports.filter((p) => !p.placeholder && isEPort(p)), [ports]);
              ...
              {groups.map((g) => (
                <tr key={g.name} className="border-t border-cohesity-border">
                  <td className="py-1.5 pr-3">
                    <ReactRouterDOM.Link to={`/brocade/devices?search=${encodeURIComponent(g.name)}`} className="text-brand hover:underline truncate block max-w-[220px]">{g.name}</ReactRouterDOM.Link>
                  </td>
...
function EventsPanel({ events }) {
  return (
    <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      ...
      <ReactRouterDOM.Link to="/brocade/events" className="text-[11px] text-brand underline mt-3 inline-block">View all →</ReactRouterDOM.Link>
    </div>
  );
}
...
export default function BrocadePortMapPage() {
  ...
  const [switches, setSwitches] = React.useState(null);
  const [switchId, setSwitchId] = React.useState(searchParams.get('switch') || '');
  ...
  const [switchDetail, setSwitchDetail] = React.useState(null);
  ...
  const loadPortmap = React.useCallback((id) => {
    if (!id) return;
  ...
  const switchName = React.useMemo(() => {
  ...
  const filteredCount = React.useMemo(() => grouped.reduce((n, [, list]) => n + list.length, 0), [grouped]);

  const sw = detail && detail.switch;
  const ports = (detail && detail.ports) || [];
  const slots = React.useMemo(() => groupBySlot(ports, sw?.maxPort), [ports, sw]);

  const zoneIndex = React.useMemo(() => {
    const m = new Map();
    for (const p of ports) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      {detailsOpen && selectedPort && portalOrInline(
        <div
