...
//
// Migration bodies copied VERBATIM from backend/db/migrations/bluecat.js,
// same version numbers, same scope id 'bluecat' - an existing local DB's
// bluecat_* data (and its schema_migrations row) is adopted intact on
// install.
module.exports = [
  {
    version: 1,
  ...
  // Migration: cached AI Advisor report content, one row per report key.
  {
    ...
    up(db) {
      db.exec(`
        CREATE TABLE IF NOT EXISTS bluecat_ai_reports (
