...
import { BRAND, fmtWhen, counterDelta } from './helpers';
...
export default function PortHistoryModal({ mac, portIdx, portLabel, deviceName, onClose }) {
  ...
  useEffect(() => {
    setRows(null);
    client.get(`/unifi/devices/${mac}/port-history`, { params: { port: portIdx, hours } })
      .then(({ data }) => setRows(Array.isArray(data) ? data : []))
      .catch(() => setRows([]));
  }, [mac, portIdx, hours]);

  const labels = useMemo(() => (rows || []).map((r) => fmtWhen(r.captured_at)), [rows]);

  const upData = useMemo(() => (rows || []).map((r) => (r.up ? 1 : 0)), [rows]);

  const errorDeltas = useMemo(() => {
    if (!rows) return [];
    return rows.map((r, i) => {
      if (i === 0) return 0;
      const prev = rows[i - 1];
  ...
  const poeWatts = useMemo(() => (rows || []).map((r) => (r.poe_power != null ? Number(r.poe_power) : null)), [rows]);
  const poeVolts = useMemo(() => (rows || []).map((r) => (r.poe_voltage != null ? Number(r.poe_voltage) : null)), [rows]);
  const hasPoe = poeWatts.some((v) => v != null);
  ...
  const poeChart = {
    labels,
    datasets: [
      { label: 'Watts', data: poeWatts, borderColor: '#D4A24E', pointRadius: 0, borderWidth: 2, tension: 0.2, yAxisID: 'y' },
      { label: 'Volts', data: poeVolts, borderColor: '#8FA3B0', pointRadius: 0, borderWidth: 1.5, tension: 0.2, yAxisID: 'y1' },
    ],
  };
  const poeOpts = {
    ...chartOpts,
  ...
  const stateOpts = { ...chartOpts, scales: { x: chartOpts.scales.x, y: { ...chartOpts.scales.y, min: -0.1, max: 1.1, ticks: { ...chartOpts.scales.y.ticks, callback: (v) => (v === 1 ? 'Up' : v === 0 ? 'Down' : '') } } } };
          ...
          {rows == null ? (
            ...
            <div className="flex flex-col gap-4">
              <div>
                ...
                <div className="h-24"><Line data={stateChart} options={stateOpts} /></div>
              </div>
              <div>
                <p className="text-xs font-semibold text-ink mb-2">Error Growth</p>
                <div className="h-32"><Line data={errChart} options={chartOpts} /></div>
              </div>
              <div>
                <p className="text-xs font-semibold text-ink mb-2">Traffic</p>
                <div className="h-32"><Line data={trafficChart} options={chartOpts} /></div>
              </div>
              {hasPoe && (
                <div>
                  <p className="text-xs font-semibold text-ink mb-2">PoE</p>
                  <div className="h-32"><Line data={poeChart} options={poeOpts} /></div>
                </div>
              )}
            </div>
          )}
