...
router.get('/overview', (req, res, next) => {
  try {
    ...
    // services/dellVcenterUtil), and their quickstats stand in for the plugin.
    let vcUtil = null;
    const vcenterUtil = () => {
      if (vcUtil === null) {
...
/** GET /api/dell/export?include=cpu,memory,...&deviceId=&layout= and
 *  POST /api/dell/export { ids, include, layout } - CSV inventory export.
 *  The POST form carries the device ids the Devices page currently shows
 *  (search and dropdown filters applied) so the file matches the table.
 *  Base columns (always): device identity, IP, health and support contract. */
const EXPORT_LAYOUTS = ['devices', 'components'];
function sendDellExport(req, res, next, { ids, include, layout }) {
  try {
...
router.get('/export', [
  ...
  query('deviceId').optional().isInt().toInt(),
  query('layout').optional().isIn(EXPORT_LAYOUTS),
], validate, (req, res, next) => sendDellExport(req, res, next, {
  ids: req.query.deviceId ? [req.query.deviceId] : null, include: req.query.include, layout: req.query.layout,
}));
router.post('/export', [
  body('ids').optional({ nullable: true }).isArray({ max: 50000 }),
  body('ids.*').isInt().toInt(),
  body('include').optional().isString().trim(),
  body('layout').optional().isIn(EXPORT_LAYOUTS),
], validate, (req, res, next) => sendDellExport(req, res, next, {
  ids: Array.isArray(req.body.ids) ? req.body.ids : null, include: req.body.include, layout: req.body.layout,
}));
