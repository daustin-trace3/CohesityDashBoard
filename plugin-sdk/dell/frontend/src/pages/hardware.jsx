...
export default function DellHardwarePage() {
  ...
  const load = React.useCallback(() => apiFetch('/dell/governance')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ failing: [], firmware: [], disconnected: [] })), []);
