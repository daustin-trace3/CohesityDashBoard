// BlueCat Overview - ported from the built-in BluecatOverviewPage.jsx.
// Uses the plugin's own LineChart (Chart.js) wrapper instead of
// react-chartjs-2 (not resolvable inside the plugin sandbox).
import { Gauge, Network, Globe, HardDrive, Server, ShieldAlert, Layers, FileText } from '../icons.jsx';
import { LineChart } from '../charts.jsx';
import client from '../api.js';
import {
  useToast, PageHeader, StatCard, Badge, LoadingPanel, ErrorPanel, RefreshButton, LastUpdated,
  BRAND, fmtNum, fmtPct, fmtWhen, freeStaticTone,
} from '../ui.jsx';
...
export default function BluecatOverviewPage() {
  ...
  const load = React.useCallback(() => {
    setFailed(false);
    return client.get('/bluecat/overview')
  ...
  const lowSpaceTrend = React.useMemo(() => ({
  ...
  if (failed && !data) {
    return (
      <div className="animate-fade-in">
        ...
        <ErrorPanel label="Failed to load BlueCat overview." onRetry={load} height={240} />
      </div>
    );
  }
        ...
        {trends.length > 1 && (
          <div>
            ...
            <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
              <LineChart data={lowSpaceTrend} height={192} />
            </div>
          </div>
        )}
      </div>
