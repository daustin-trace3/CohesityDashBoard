// vCenter Network — ported from frontend/src/pages/vcenter/VcNetworkPage.jsx.
import { Network, Cable, Share2, EthernetPort, Waypoints } from '../icons.jsx';
import {
...
function PnicSection({ rows }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><EthernetPort size={15} className="text-brand" /> Physical NICs</p>
...
function SwitchSection({ vswitches, dvswitches }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Share2 size={15} className="text-brand" /> Virtual Switches</p>
...
function PortgroupSection({ portgroups, dvportgroups, onShowVms }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Waypoints size={15} className="text-brand" /> Port Groups</p>
...
function VmkernelSection({ rows }) {
  ...
  return (
    <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><Cable size={15} className="text-brand" /> VMkernel Interfaces</p>
...
export default function VcNetworkPage() {
  ...
  const [vmModal, setVmModal] = React.useState(null);   // { title, subtitle, filter }
  const [detailVmId, setDetailVmId] = React.useState(null);

  const load = React.useCallback(() => apiFetch('/vcenter/network')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ pnics: [], vswitches: [], portgroups: [], vmkernels: [], dvswitches: [], dvportgroups: [] })), []);
