import { BadgeCheck, ShieldX, ShieldAlert, Shield, ShieldCheck, FileStack, Download } from '../icons.jsx';
import {
  apiFetch, PageHeader, LoadingPanel, RefreshButton, LastUpdated,
...
function Tile({ bucket, count, active, onClick }) {
  ...
  return (
    <button onClick={onClick}
      className="panel p-4 text-left transition-all cursor-pointer hover:-translate-y-0.5"
      style={{ borderTop: `3px solid ${toneColor}`, boxShadow: active ? `0 0 0 2px ${toneColor}` : undefined }}>
...
export default function DellSupportPage() {
  ...
  const [bucketKey, setBucketKey] = React.useState('all');
  const [lastRefreshed, setLastRefreshed] = React.useState(null);

  const load = React.useCallback(() => apiFetch('/dell/warranty')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ warnDays: 90, rows: [] })), []);

  React.useEffect(() => { load(); }, [load]);

  const warnDays = data?.warnDays ?? 90;
  const rows = data?.rows || [];
  const buckets = React.useMemo(() => BUCKETS(warnDays), [warnDays]);
