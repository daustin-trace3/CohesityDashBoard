...
export default function AwsVpcPage() {
  ...
  const load = useCallback(() => client.get('/aws/vpc')
    .then(({ data }) => { setVpcs(data?.vpcs || []); setSubnets(data?.subnets || []); setLastRefreshed(new Date()); })
    .catch(() => { setVpcs([]); setSubnets([]); toast({ type: 'error', title: 'Failed to load VPC data' }); }), [toast]);
