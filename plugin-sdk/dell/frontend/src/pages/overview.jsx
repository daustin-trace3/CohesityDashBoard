import { Gauge, Server, ShieldAlert, Boxes, AlertTriangle, HardDrive, Cpu, MemoryStick, Zap, BadgeCheck, Thermometer, Unplug, ClipboardCheck } from '../icons.jsx';
import { DoughnutChart, BarChart, LineChart } from '../charts.jsx';
import {
  apiFetch, PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated, timeAgo,
  BRAND, fmtNum, fmtBytes, severityTone, asDate,
} from '../ui.jsx';
...
export default function DellOverviewPage() {
  ...
  const [utilShow, setUtilShow] = React.useState({ cpu: true, mem: true });

  const load = React.useCallback(() => apiFetch('/dell/overview')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ instances: [], devices: {}, issues: [] })), []);
    ...
    <div className="animate-fade-in">
      ...
      {data != null && instances.length === 0 && (
        <div className="panel p-4 mb-4 text-sm text-ink-muted">
          ...
          <ReactRouterDOM.Link to="/dell/settings" className="text-brand hover:underline">Settings</ReactRouterDOM.Link>.
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-3 gap-4 mb-4">
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {data == null ? <LoadingPanel label="Loading…" height={180} /> : (
            <div className="h-[200px] flex items-center justify-center" style={{ height: 200 }}>
              <DoughnutChart height={200} data={healthDonut} options={{
                cutout: '62%',
        ...
        <div className="panel p-4 lg:col-span-2" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {data == null ? <LoadingPanel label="Loading…" height={180} /> : models.length === 0 ? (
            ...
            <BarChart height={200} data={modelBar} options={{
      ...
      <div className={`grid ${chartCols} gap-4 mb-4`}>
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {data == null ? <LoadingPanel label="Loading…" height={170} /> : alertDays.length === 0 ? (
            ...
            <BarChart height={190} data={alertsBar} options={{
        ...
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {data == null ? <LoadingPanel label="Loading…" height={170} /> : powerLine.datasets.length === 0 ? (
            ...
            <LineChart height={190} data={powerLine} options={{
              plugins: { legend: { position: 'bottom', labels: { color: '#E5E5E5', boxWidth: 10, font: { size: 10 } } } },
        ...
        <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {data == null ? <LoadingPanel label="Loading…" height={170} /> : topUtil.length === 0 ? (
            ...
            <BarChart height={190} data={utilBar} options={{
              indexAxis: 'y',
              plugins: { legend: { position: 'bottom', labels: { color: '#E5E5E5', boxWidth: 10, font: { size: 10 } },
                onClick: (e, item) => {
