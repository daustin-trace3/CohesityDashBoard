...
// trouble exercising EVERY rule in services/brocadeIssues.js: a critical
// switch (BAD_PWR), an unreachable switch, a switch with other nonzero
// management_state bits, a marginal fabric, a sub-50 + sub-70 health score,
// 2 fenced + 1 blocked port, a cert expiring in 20 days + one already
// expired (chassis), an EOS switch, firmware drift within a fabric, a
// fabric with default_zone_access=1, a zone change inside the last 24h, a
// switch in maintenance mode, an event storm (>=12 critical/alert events in
// the last hour on one source), and a failed poll on the DR source.
// A few enclosure host_name values mirror the vcenter demo VM/host names
// (aria suite VMs + nyc-esx-0101.icc.demo) for cross-platform Server 360 hits.
...
const CROSS_HIT_HOSTS = ['vra-prod', 'vra-dr', 'vrops-nyc-01', 'vrli-nyc-01', 'nyc-esx-0101.icc.demo'];
...
function seedBrocade(db, { now, encrypt }) {
  ...
  ENCLOSURE_PLAN.forEach((e, i) => {
    ...
    const sourceId = sourceIds[fabric.source];
    const rng = rngFor(`brocade-enclosure-${e.name}`);
    const guid = `enc-guid-${i}`;
    const info = insertEnclosure.run({
      source_id: sourceId, sannav_id: 3000 + i, guid, name: e.name, type: e.type,
    ...
    enclosures.push({ id: info.lastInsertRowid, guid, name: e.name, type: e.type, fabric: e.fabric, sourceId, hostName: e.hostName });
    enclosureTotal++;
  });
