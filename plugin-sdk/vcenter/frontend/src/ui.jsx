// vCenter plugin style kit. No Tailwind build step is available inside a
// plugin bundle, so this installs a hand-rolled utility stylesheet that
// mirrors the class *names* the built-in frontend/src/pages/vcenter/* pages
...
// plugin-sdk/dell/frontend/src/ui.jsx exactly, renamed dl- -> vc- and
// rebranded to vSphere blue (#0091DA = rgb(0,145,218)).
//
...
export const BRAND = '#0091DA';

const STYLE_ID = 'vc-plugin-styles';
...
const CSS = `
:root {
  --vc-surface-base: #0B1015;
  --vc-surface: #131B23;
  --vc-surface-raised: #18222C;
  --vc-surface-overlay: #1E2A36;
  --vc-border: #1F2B37;
  --vc-gray: #131B23;
  --vc-ink: #E8EDF2;
  --vc-ink-muted: #94A3B3;
...
.vc-root { font-family: inherit; color: var(--vc-ink); }

/* component classes used by ported page bodies */
.panel { background: var(--vc-surface); border: 1px solid var(--vc-border); border-radius: 12px; box-shadow: 0 1px 2px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.02); }
.tnum { font-variant-numeric: tabular-nums; }
.animate-fade-in { animation: vc-fade-in 220ms ease-out both; }
.animate-spin { animation: vc-spin 0.8s linear infinite; }
@keyframes vc-fade-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: none; } }
@keyframes vc-spin { to { transform: rotate(360deg); } }

/* colors */
.text-ink { color: var(--vc-ink); }
.text-ink-muted { color: var(--vc-ink-muted); }
.text-ink-faint { color: var(--vc-ink-faint); }
.text-brand { color: var(--vc-brand); }
.text-status-ok { color: var(--vc-ok); }
.text-status-warn { color: var(--vc-warn); }
.text-status-crit { color: var(--vc-crit); }
.text-status-bad { color: var(--vc-crit); }
.text-status-info { color: var(--vc-info); }
.text-white { color: #fff; }
.text-white\\/80 { color: rgba(255,255,255,.8); }
.text-cohesity-black { color: #0B1015; }
.bg-black\\/40 { background: rgba(0,0,0,.4); }
.bg-black\\/50 { background: rgba(0,0,0,.5); }
.bg-black\\/60 { background: rgba(0,0,0,.6); }
.bg-brand\\/10 { background: rgba(0,145,218,.1); }
.bg-brand\\/20 { background: rgba(0,145,218,.2); }
.bg-brand { background: var(--vc-brand); }
.hover\\:opacity-90:hover { opacity: .9; }
.ml-auto { margin-left: auto; }
.mr-auto { margin-right: auto; }
.bg-status-ok\\/10 { background: rgba(52,211,153,.1); }
.bg-status-ok\\/5 { background: rgba(52,211,153,.05); }
...
.bg-cohesity-gray { background: var(--vc-gray); }
.bg-surface-overlay { background: var(--vc-surface-overlay); }
.bg-surface-overlay\\/30 { background: rgba(30, 42, 54, 0.3); }
.bg-surface-overlay\\/50 { background: rgba(30, 42, 54, 0.5); }
.bg-white { background: #fff; }

/* borders */
.border { border: 1px solid var(--vc-border); }
.border-b { border-bottom: 1px solid var(--vc-border); }
.border-t { border-top: 1px solid var(--vc-border); }
.border-l { border-left: 1px solid var(--vc-border); }
.border-l-2 { border-left: 2px solid var(--vc-border); }
.border-transparent { border-color: transparent; }
.border-cohesity-border { border-color: var(--vc-border); }
.border-cohesity-border\\/30 { border-color: rgba(31,43,55,.3); }
.border-cohesity-border\\/40 { border-color: rgba(31,43,55,.4); }
.border-cohesity-border\\/50 { border-color: rgba(31,43,55,.5); }
.border-brand\\/20 { border-color: rgba(0,145,218,.2); }
.border-brand\\/30 { border-color: rgba(0,145,218,.3); }
.border-brand\\/40 { border-color: rgba(0,145,218,.4); }
.border-brand\\/50 { border-color: rgba(0,145,218,.5); }
.border-status-warn { border-color: var(--vc-warn); }
.border-status-warn\\/40 { border-color: rgba(251,191,36,.4); }
.border-status-crit { border-color: var(--vc-crit); }
...
.accent-brand { accent-color: var(--vc-brand); }
.accent-\\[\\#0091DA\\] { accent-color: #0091DA; }
.opacity-50 { opacity: .5; }
.ring-1 { box-shadow: 0 0 0 1px var(--vc-ring, transparent); }
.ring-2 { box-shadow: 0 0 0 2px var(--vc-ring, transparent); }
.ring-brand\\/30 { --vc-ring: rgba(0,145,218,.3); }
.ring-brand\\/70 { --vc-ring: rgba(0,145,218,.7); }

/* hover / focus / disabled */
.hover\\:bg-surface-overlay:hover { background: var(--vc-surface-overlay); }
.hover\\:bg-brand\\/10:hover { background: rgba(0,145,218,.1); }
.hover\\:bg-brand\\/20:hover { background: rgba(0,145,218,.2); }
.hover\\:border-brand\\/40:hover { border-color: rgba(0,145,218,.4); }
.hover\\:border-brand\\/50:hover { border-color: rgba(0,145,218,.5); }
.hover\\:border-status-crit\\/50:hover { border-color: rgba(248,113,113,.5); }
.hover\\:text-ink:hover { color: var(--vc-ink); }
.hover\\:text-white:hover { color: #fff; }
.hover\\:text-status-crit:hover { color: var(--vc-crit); }
.hover\\:underline:hover { text-decoration: underline; }
.hover\\:ring-1:hover { box-shadow: 0 0 0 1px var(--vc-ring, transparent); }
.hover\\:ring-brand\\/30:hover { --vc-ring: rgba(0,145,218,.3); }
.hover\\:-translate-y-0\\.5:hover { transform: translateY(-2px); }
.focus\\:border-brand\\/60:focus { border-color: rgba(0,145,218,.6); }
...
.vc-scroll::-webkit-scrollbar-thumb { background: #2A3845; border-radius: 4px; border: 2px solid var(--vc-surface-base); }
.vc-scroll::-webkit-scrollbar-thumb:hover { background: #3B4D5E; }
...
.vc-input:focus { border-color: rgba(0,145,218,0.6); }
...
.vc-btn-ghost:hover { color: var(--vc-ink); border-color: rgba(0,145,218,0.4); }
.vc-btn-ghost:disabled { opacity: 0.5; cursor: default; }
...
/* Scope every rule under .vc-root so the utility vocabulary (w-full, flex,
 ...
 * (the unifi 1.0.2 bug). :root var declarations stay global (vc- prefixed,
  ...
  return parts.map((part, i) => {
    ...
    return part.replace(/(^|\{|\})(\s*)([^@{}]+?)(\s*\{)/g, (m, brace, ws, sel, open) => {
      const scoped = sel.split(',').map((s) => {
        const t = s.trim();
        if (!t || t === ':root' || t.startsWith('.vc-root')) return t;
        return `.vc-root ${t}`;
      }).join(', ');
  ...
  const item = (s) => {
    ...
    return (
      <button key={s.key} onClick={() => onSelect(s.key)} aria-current={on ? 'page' : undefined}
        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 8, textAlign: 'left', fontSize: 12, fontWeight: on ? 600 : 400, border: 'none', cursor: 'pointer', background: on ? 'var(--vc-surface-overlay)' : 'transparent', color: on ? 'var(--vc-ink)' : 'var(--vc-ink-muted)' }}>
        {Icon && <Icon size={13} style={{ color: on ? 'var(--vc-brand)' : 'var(--vc-ink-faint)' }} />}
        {s.label}
      </button>
    );
  };
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-start' }}>
      <nav aria-label={label ? label + ' settings sections' : 'Settings sections'}
        style={{ width: 224, flexShrink: 0, padding: 8, borderRadius: 12, background: 'var(--vc-surface)', border: '1px solid var(--vc-border)', borderTop: '3px solid ' + brand }}>
        {groups.map((g) => (
          <div key={g || 'all'} style={{ marginBottom: grouped ? 8 : 0 }}>
            {g && <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--vc-ink-faint)', padding: '4px 8px 0', margin: '0 0 4px' }}>{g}</p>}
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
        {IconComp && (
          <div style={{ marginTop: 2, display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(0,145,218,0.1)', border: '1px solid rgba(0,145,218,0.2)', flexShrink: 0 }}>
            <IconComp size={18} style={{ color: 'var(--vc-brand)' }} />
          </div>
        )}
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--vc-ink)', lineHeight: 1.2, margin: 0 }}>{title}</h1>
          {description && <p style={{ fontSize: 12, color: 'var(--vc-ink-muted)', margin: '2px 0 0' }}>{description}</p>}
        </div>
      </div>
...
const TONE = {
  default: { icon: 'var(--vc-ink-muted)', iconBg: 'var(--vc-surface-overlay)', iconBorder: 'var(--vc-border)' },
  neutral: { icon: 'var(--vc-ink-muted)', iconBg: 'var(--vc-surface-overlay)', iconBorder: 'var(--vc-border)' },
  brand: { icon: 'var(--vc-brand)', iconBg: 'rgba(0,145,218,0.1)', iconBorder: 'rgba(0,145,218,0.2)' },
  ok: { icon: 'var(--vc-ok)', iconBg: 'rgba(52,211,153,0.1)', iconBorder: 'rgba(52,211,153,0.2)' },
  warn: { icon: 'var(--vc-warn)', iconBg: 'rgba(251,191,36,0.1)', iconBorder: 'rgba(251,191,36,0.2)' },
  crit: { icon: 'var(--vc-crit)', iconBg: 'rgba(248,113,113,0.1)', iconBorder: 'rgba(248,113,113,0.2)' },
  info: { icon: 'var(--vc-info)', iconBg: 'rgba(96,165,250,0.1)', iconBorder: 'rgba(96,165,250,0.2)' },
};
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--vc-ink-faint)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</p>
        {loading ? (
          <div style={{ height: 24, width: 80, marginTop: 4, borderRadius: 6, background: 'var(--vc-surface-overlay)' }} />
        ) : (
          <p className="tnum" style={{ fontSize: 20, fontWeight: 700, color: 'var(--vc-ink)', lineHeight: 1.2, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</p>
        )}
        {sub && !loading && (
          <p style={{ fontSize: 11, color: 'var(--vc-ink-muted)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sub}</p>
        )}
      </div>
    </Tag>
  );
...
const BADGE_TONES = {
  ok: { bg: 'rgba(52,211,153,0.1)', color: 'var(--vc-ok)', border: 'rgba(52,211,153,0.25)' },
  warn: { bg: 'rgba(251,191,36,0.1)', color: 'var(--vc-warn)', border: 'rgba(251,191,36,0.25)' },
  crit: { bg: 'rgba(248,113,113,0.1)', color: 'var(--vc-crit)', border: 'rgba(248,113,113,0.25)' },
  info: { bg: 'rgba(96,165,250,0.1)', color: 'var(--vc-info)', border: 'rgba(96,165,250,0.25)' },
  brand: { bg: 'rgba(0,145,218,0.1)', color: 'var(--vc-brand)', border: 'rgba(0,145,218,0.25)' },
  neutral: { bg: 'var(--vc-surface-overlay)', color: 'var(--vc-ink-muted)', border: 'var(--vc-border)' },
};

export function Badge({ tone = 'neutral', children, style, className }) {
  const t = BADGE_TONES[tone] || BADGE_TONES.neutral;
  return (
    <span className={cls('vc-chip', className)} style={{ background: t.bg, color: t.color, borderColor: t.border, ...style }}>
      {children}
    </span>
  );
}
...
export function Spinner({ size = 16, style }) {
  return <LoaderGlyph size={size} style={{ color: 'var(--vc-brand)', animation: 'vc-spin 0.8s linear infinite', ...style }} />;
}
...
export function LoadingPanel({ label = 'Loading data…', height = 200 }) {
  return (
    <div role="status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <Spinner size={22} />
      <p style={{ fontSize: 12, color: 'var(--vc-ink-muted)', margin: 0 }}>{label}</p>
    </div>
  );
}
...
export function RefreshButton({ onClick, refreshing, label = 'Refresh' }) {
  return (
    <button onClick={onClick} disabled={refreshing} className="vc-btn-ghost">
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={refreshing ? { animation: 'vc-spin 0.8s linear infinite' } : undefined}>
...
export function LastUpdated({ date, prefix = 'Updated' }) {
  ...
  return (
    <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--vc-ink-faint)' }}>
...
export function Fact({ label, value }) {
  return (
    <div>
      <p style={{ margin: 0, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--vc-ink-faint)' }}>{label}</p>
      <p className="tnum" style={{ margin: 0, fontSize: 13, color: 'var(--vc-ink)' }}>{value ?? '—'}</p>
    </div>
  );
}
...
export function portalOrInline(node) {
  // Portaled content lands outside the .vc-root wrapper, so re-wrap it —
  // otherwise the scoped stylesheet (see scopeCss) no longer applies.
  const wrapped = <div className="vc-root">{node}</div>;
  ...
  return portalOrInline(
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} role="dialog" aria-modal="true">
      ...
      <div className="panel" style={{ position: 'relative', width: maxWidth, maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderTop: `3px solid ${BRAND}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '16px 16px 12px', borderBottom: '1px solid var(--vc-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {IconComp && <IconComp size={17} style={{ color: 'var(--vc-brand)', flexShrink: 0 }} />}
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--vc-ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</p>
              {subtitle && <p style={{ margin: 0, fontSize: 11, color: 'var(--vc-ink-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</p>}
            </div>
          </div>
          <button onClick={onClose} aria-label="Close" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 28, width: 28, borderRadius: 8, border: 'none', background: 'transparent', color: 'var(--vc-ink-faint)', cursor: 'pointer', flexShrink: 0 }}>
        ...
        <div className="vc-scroll" style={{ padding: 16, overflowY: 'auto' }}>{children}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <th className={cls('py-2 pr-3', align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left')} style={style}>
      <button
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'transparent', color: active ? 'var(--vc-ink)' : 'var(--vc-ink-muted)', padding: 0 }}
      >
        {label}
        {active && (
...
export function TableSearch({ ctl, placeholder = 'Search…', className }) {
  return (
    <div className={className || 'w-full max-w-md'} style={{ position: 'relative' }}>
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--vc-ink-faint)', pointerEvents: 'none' }}>
        <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
      </svg>
      <input value={ctl.q} onChange={(e) => ctl.setQ(e.target.value)} placeholder={placeholder} className="vc-input" style={{ paddingLeft: 32 }} />
    </div>
  );
}
...
export function FilterSelect({ ctl, k, rows, label }) {
  ...
  return (
    <select value={ctl.filters[k] || ''} onChange={(e) => ctl.setFilter(k, e.target.value)} className="vc-input" style={{ width: 'auto', cursor: 'pointer' }}>
...
export function TableControls({ ctl, rows, searchPlaceholder, filters = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      <span className="tnum" style={{ fontSize: 11, color: 'var(--vc-ink-faint)', marginLeft: 'auto' }}>
...
const pagerBtnStyle = { fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--vc-border)', background: 'transparent', color: 'var(--vc-ink-muted)', cursor: 'pointer' };
...
export function TablePager({ ctl, sizes = PAGE_SIZES }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, marginTop: 4, borderTop: '1px solid var(--vc-border)' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--vc-ink-faint)' }}>
        Rows per page
        <select value={String(ctl.pageSize)} onChange={(e) => ctl.setPageSize(e.target.value === 'all' ? 'all' : Number(e.target.value))} className="vc-input" style={{ width: 'auto', padding: '4px 8px', cursor: 'pointer' }}>
          {sizes.map((s) => <option key={s} value={String(s)}>{s === 'all' ? 'All' : s}</option>)}
        </select>
      </label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="tnum" style={{ fontSize: 11, color: 'var(--vc-ink-faint)' }}>{start}–{end} of {total}</span>
        {!all && ctl.pageCount > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span className="tnum" style={{ fontSize: 11, color: 'var(--vc-ink-faint)', padding: '0 4px' }}>{ctl.page + 1} / {ctl.pageCount}</span>
