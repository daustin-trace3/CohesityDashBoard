// Cohesity demo data, ported from backend/demo/generators/cohesity.js (725
// lines). Seeds clusters, metrics, alerts, protection/replication runs,
// policies, source registrations, licensing tables, cohesity_views,
// workload_history, cohesity_objects, cohesity_agents, gflags — the FULL
// built-in generator, even though several of these tables are read by
// WP-B's (features-plane) routes. Demo seeding is one unit regardless of
// the route-plane split (dell/nutanix/proxmox precedent): WP-A owns
// demoSeed.js in full.
//
// DEVIATION FROM THE BUILT-IN's wipe strategy (icc-platform-to-plugin skill,
// "demoSeed refinement" — dell wave): `clusters` is the USER-CONNECTION
// table (contract: real admin-added clusters must survive a demo reseed) —
// it is NEVER wiped/deleted. The 24 fixture clusters are upserted BY NAME so
// their ids stay stable across reboots/reseeds, and only THEIR dependent
// rows are wiped (children before parents) before regenerating.
//
// cohesity_objects is special-cased: the vcenter plugin's demo generator
// also INSERTs a handful of rows into it (global_id LIKE 'demo:s360:%',
// scoped to one of THIS plugin's fixture cluster ids, for Server 360 demo
// coherence) — see plugin-sdk/vcenter/backend/src/demoSeed.js. This
// module's wipe of cohesity_objects excludes that global_id namespace so a
// reseed here never destroys vcenter's rows regardless of poll ordering.
...
// Dependent tables scoped by cluster_id, wiped children-before-parents for
// the fixture cluster ids only — `clusters` itself is never wiped.
const CLUSTER_SCOPED_TABLES = [
  'gflag_changes', 'cluster_gflags',
  'cohesity_agents',
  'workload_history',
  'source_registrations', 'policies',
  'replication_runs', 'protection_runs',
  'alerts', 'metrics_history',
];

// license_usage/license_meter_usage/license_view_detail/consumption_breakdown
// key on system_id (TEXT — the cluster id stringified), not cluster_id.
const SYSTEM_ID_SCOPED_TABLES = [
  'consumption_breakdown', 'license_view_detail', 'license_meter_usage', 'license_usage',
];
...
function seedCohesity(db, { now, encrypt }) {
  ...
  // ── Upsert fixture clusters by name (stable id across reseeds) ─────────
  const upsertCluster = db.prepare(`
    ...
    ON CONFLICT(name) DO UPDATE SET
      vip = excluded.vip, encrypted_credentials = excluded.encrypted_credentials,
      tags = excluded.tags, updated_at = excluded.updated_at
  `);
  const getClusterId = db.prepare('SELECT id FROM clusters WHERE name = ?');
  ...
  clusterDefs.forEach((def, idx) => {
    ...
    upsertCluster.run(def.name, vip, credentials, tags, nowIso, nowIso);
    const clusterId = getClusterId.get(def.name).id;
    clusters.push({ ...def, id: clusterId, rng, drTarget: drTargetFor(def) });
  });
  const clusterByName = new Map(clusters.map((c) => [c.name, c]));

  // ── Wipe this fixture's dependent rows only, children before parents ───
  const fixtureIds = clusters.map((c) => c.id);
  const placeholders = fixtureIds.map(() => '?').join(',');
  for (const table of CLUSTER_SCOPED_TABLES) {
    db.prepare(`DELETE FROM ${table} WHERE cluster_id IN (${placeholders})`).run(...fixtureIds);
  }
  {
    const systemIdPlaceholders = fixtureIds.map(() => '?').join(',');
    const systemIds = fixtureIds.map(String);
    for (const table of SYSTEM_ID_SCOPED_TABLES) {
      db.prepare(`DELETE FROM ${table} WHERE system_id IN (${systemIdPlaceholders})`).run(...systemIds);
    }
  }
  // cohesity_objects: exclude the vcenter plugin's Server 360 demo rows
  // (global_id 'demo:s360:%') so a reseed here never destroys them.
  db.prepare(`
    DELETE FROM cohesity_objects
    WHERE cluster_id IN (${placeholders})
      AND (global_id IS NULL OR global_id NOT LIKE 'demo:s360:%')
  `).run(...fixtureIds);
  // cohesity_views / license_type_usage are keyed by system_name / global,
  // not cluster_id — scope by the fixture cluster names, and license_type_usage
  // (one global row per license type, fixture-only content) wholesale.
  {
    const namePlaceholders = clusters.map(() => '?').join(',');
    const names = clusters.map((c) => c.name);
    db.prepare(`DELETE FROM cohesity_views WHERE system_name IN (${namePlaceholders})`).run(...names);
    db.prepare(`DELETE FROM replication_status_cache WHERE cluster_name IN (${namePlaceholders})`).run(...names);
  }
  ...
  function idxSoftwareVersion(rng) {
  ...
  for (const cluster of clusters) {
    ...
    const softwareVersion = idxSoftwareVersion(rng);
  ...
  for (const cluster of clusters) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      insertAlert.run(
        cluster.id, `demo-${cluster.id}-${i}`, severity, alertType,
        `${alertType} on ${cluster.name}`, resolved, firstSeen, lastUpdated
      );
      alertsCount++;
    }
  }
      ...
      for (let day = 0; day < 14; day++) {
        for (let run = 0; run < 4; run++) {
          ...
          const info = insertProtRun.run(
            cluster.id, jobIdx, jobName, status,
            startTime.toISOString(), endTime ? endTime.toISOString() : null,
            errorCode, errorMessage, logicalBytes, startTime.toISOString()
          );
          protRunCount++;
          rows.push({ id: info.lastInsertRowid, jobName, status, startTime, logicalBytes });
        }
      }
    }
  ...
  for (const cluster of clusters) {
    ...
    for (const run of runs) {
      ...
      insertReplRun.run(
        run.id, cluster.id, cluster.drTarget, targetCluster ? targetCluster.id : null,
        status, run.logicalBytes, startTime.toISOString(), endTime ? endTime.toISOString() : null,
        lagSeconds, startTime.toISOString()
      );
      replRunCount++;
      replList.push({
        jobName: run.jobName, protectionGroupId: run.id, runId: run.id,
        runStartTimeUsecs: startTime.getTime() * 1000, localBackupStatus: 'kSuccess',
  ...
  for (const cluster of clusters) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const replicationTargets = hasTarget ? JSON.stringify([cluster.drTarget]) : '[]';
  ...
  for (const cluster of clusters) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      insertSource.run(
        cluster.id, 1000 + cluster.id * 10 + i, `${cluster.name}-source-${i}`,
        env, protectedCount, unprotectedCount, protectedBytes, unprotectedBytes, nowIso
      );
      sourceCount++;
    }
  }
  ...
  outer: for (const cluster of clusters) {
    ...
    for (let i = 0; i < viewsPerCluster; i++) {
      ...
      insertViewDetail.run(
        String(cluster.id), cluster.name, viewName, isReadOnly, createdMs,
        physicalBytes, logicalBytes, Math.round(logicalBytes * 0.5), nowIso
      );
      ...
      insertCohesityView.run(
        String(cluster.id), cluster.name, 1000 + viewCount, viewName,
        ...
        logicalBytes, physicalBytes, logicalBytes, Math.round(logicalBytes * 0.5),
        randInt(rng, 100, 50000), createdMs, nowIso
      );
      viewCount++;
    }
  }
  ...
  for (const cluster of clusters) {
    ...
    const baseByEnv = envs.map((environment, i) => ({
      ...
      growth: randFloat(rng, 0.03, 0.14, 3),
    }));
    ...
    for (let d = WORKLOAD_DAYS; d >= 0; d--) {
      ...
      for (const base of baseByEnv) {
        ...
        insertWorkload.run(
          cluster.id, base.environment, Math.round(base.protectedCount * scale), base.unprotectedCount,
          protectedBytes, base.jobCount, logicalBytes, Math.round(logicalBytes / base.reduction), capturedAt
        );
        workloadRows++;
      }
...
/** Demo-only entry point (DASHBOARD_DEMO==='1' — see poller.js). Wraps the
 *  whole seed in one transaction so a reseed is atomic. */
function seedCohesityDemo(coreApi) {
  const db = coreApi.db;
  return db.transaction(() => seedCohesity(db, { now: Date.now(), encrypt: coreApi.encryption.encrypt }))();
}

module.exports = { seedCohesity, seedCohesityDemo, buildClusterList, drTargetFor };
