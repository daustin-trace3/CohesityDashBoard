...
//
// Ported from backend/demo/generators/vcenter.js. ALL inserts here run ONLY
// behind the DASHBOARD_DEMO==='1' gate — see seedVcenterDemo() below, called
...
// service directly.
//
// DEVIATION FROM THE BUILT-IN's wipe strategy: vcenter_vcenters itself is
// NEVER wiped/deleted (it is the user-facing connection table — an admin
// could register a real vCenter on a demo instance and that must survive a
// reseed). Instead each fixture vCenter is upserted by name so its id stays
// stable across boots, and only ITS dependent rows are wiped before
// reseeding, children before parents — any independently-registered real
// vCenter's data is untouched. issue-history rows are scoped by vcenter name.
...
function seedVcenter(coreApi, { now, encrypt }) {
  const db = coreApi.db;
  ...
  const upsertVc = db.prepare(`
    ...
    ON CONFLICT(name) DO UPDATE SET
      ...
      last_poll_at = excluded.last_poll_at, version = excluded.version, build = excluded.build,
      product_name = excluded.product_name, updated_at = datetime('now')
  `);
  const getVcId = db.prepare('SELECT id FROM vcenter_vcenters WHERE name = ?');
  ...
  let hostTotal = 0, vmTotal = 0, dsTotal = 0, clusterTotal = 0, orphanTotal = 0, eventTotal = 0;

  // Demo-seeded Server 360 coherence rows are marked with a stable global_id
  // prefix so a reseed can clear and reinsert them idempotently.
  try {
    db.prepare("DELETE FROM cohesity_objects WHERE global_id LIKE 'demo:s360:%'").run();
  } catch { /* built-in cohesity tables not present in a bare rehearsal DB */ }
  ...
  SITES.forEach((site, vcIdx) => {
    ...
    const vcId = getVcId.get(vcName).id;
    ...
    // independently-registered real vCenter's data is untouched.
    for (const table of DEMO_CHILD_TABLES) {
      db.prepare(`DELETE FROM ${table} WHERE vcenter_id = ?`).run(vcId);
    }
    db.prepare('DELETE FROM vcenter_issue_history WHERE vcenter = ?').run(vcName);
    ...
    for (let c = 1; c <= clusterCount; c++) {
      ...
      for (let h = 1; h <= hostCount; h++) {
        ...
        // nyc-esx-0102 is the SAN-boot-path-down incident host shared with the
        // Dell pack and the host's Brocade demo data: down so the reconcile
        // below opens its host-down issue on every demo boot.
        ...
        for (let v = 1; v <= hostVms; v++) {
          ...
          // mounts), mostly healthy, a few at warning or critical, some growing
          // fast enough to show days to full. Virtual disks sized to the volumes
          // they carry; every VM has them even when Tools is off.
    ...
    // Native vSphere events: ~40-60 for this vCenter over the last 48h.
    const eventRng = rngFor(`${vcName}-events`);
    const vms = db.prepare('SELECT name, host_name FROM vcenter_vms WHERE vcenter_id = ?').all(vcId);
    const hostNames = db.prepare('SELECT name FROM vcenter_hosts WHERE vcenter_id = ?').all(vcId).map((h) => h.name);
    const count = randInt(eventRng, 40, 60);
    let eventKey = 100000 + vcIdx * 100000;
    for (let i = 0; i < count; i++) {
      const at = new Date(now - randInt(eventRng, 2, 48 * 60) * 60000).toISOString();
      const vm = pick(eventRng, vms);
      const otherHost = pick(eventRng, hostNames);
      const roll = eventRng();
      let type, severity, message, entity, user = null;
      if (unreachable && roll < 0.25) {
        type = 'HostConnectionLostEvent'; severity = 'error'; entity = pick(eventRng, hostNames);
        ...
        type = 'HostCnxFailedTimeoutEvent'; severity = 'warning'; entity = pick(eventRng, hostNames);
        message = `Host ${entity} heartbeat delayed — connection retried successfully`;
      } else if (roll < 0.5) {
        type = chance(eventRng, 0.6) ? 'DrsVmMigratedEvent' : 'VmMigratedEvent'; severity = 'info'; entity = vm.name;
        ...
        type = chance(eventRng, 0.5) ? 'VmCreatedEvent' : 'VmRemovedEvent'; severity = 'info'; entity = vm.name;
        ...
        const entering = chance(eventRng, 0.5);
        type = entering ? 'EnteredMaintenanceModeEvent' : 'ExitMaintenanceModeEvent'; severity = 'info';
        entity = pick(eventRng, hostNames); user = 'ICC\\vsphere.admin';
        message = `Host ${entity} has ${entering ? 'entered' : 'exited'} maintenance mode`;
      }
      insertEvent.run(vcId, eventKey++, type, severity, message, user, entity, at, nowIso);
      eventTotal++;
    }
  });
...
/** Demo-only entry point. Upserts the eight fixture vCenters (id stable
