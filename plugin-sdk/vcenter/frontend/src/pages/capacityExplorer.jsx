// Failover Explorer — pick VMs (or a whole site) and move them to the other
// site client-side; the impact panel shows before → after against each
// site's N+1 usable. Nothing is written back. Table uses the same
// filter/sort/pager kit as datastores.jsx.
const { ArrowLeftRight, Layers, Building2 } = require('../icons.jsx');
const {
...
const { fmtMhz, fmtPctInt, pctColor, SiteDot, SiteBadge, BigStat, PanelTitle, NoSitesState } = require('./capacityShared.jsx');
...
function ImpactRow({ label, before, after, fmt, usable }) {
  ...
  return (
    <div className="text-xs py-1 border-b border-cohesity-border/30" style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '2px 8px' }}>
      ...
      {usable > 0 ? (
        <span className="tnum text-right" style={{ color: pctAfter > 80 ? pctColor(pctAfter) : 'var(--vc-ink-faint)' }}>{fmtPctInt(pctAfter)} of usable</span>
      ) : <span />}
      <span className="tnum" style={{ gridColumn: '1 / -1', whiteSpace: 'nowrap' }}>
...
export default function VcCapacityExplorerPage() {
  ...
  const [scenario, setScenario] = React.useState(new Map()); // vmId -> targetSiteId
  const [selected, setSelected] = React.useState(new Set());
  const [preset, setPreset] = React.useState({ from: '', to: '' });

  const load = React.useCallback(() => apiFetch('/vcenter/capacity/explorer')
    .then((j) => { setData(j); setScenario(new Map()); setSelected(new Set()); setLastRefreshed(new Date()); })
    .catch(() => setData({ sites: [], vms: [] })), []);
  React.useEffect(() => { load(); }, [load]);

  const sites = data?.sites || [];
  const siteById = new Map(sites.map((s) => [s.id, s]));
  const rows = React.useMemo(() => (data?.vms || []).map((v) => ({
    ...v,
  ...
  })), [data, scenario]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const impact = React.useMemo(() => {
    ...
    return out;
  }, [data, sites, scenario]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const impactSites = sites.length > 4 ? touched : sites;
  const manySites = sites.length > 4;

  return (
    <div className="animate-fade-in">
      <PageHeader icon={ArrowLeftRight} title="Failover Explorer" description="Move VMs — or a whole site — to the other site and see the resulting demand against N+1 usable. Nothing is changed in vCenter.">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-3">
                ...
                {selected.size > 0 && (
                  <>
                    ...
                    {sites.map((s) => <button key={s.id} onClick={() => moveVms([...selected], s.id)} className="vc-btn-ghost"><SiteDot site={s} size={8} /> {s.name}</button>)}
                  </>
                )}
              </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead><tr className="text-left text-[11px] uppercase tracking-wide text-ink-faint border-b border-cohesity-border">
                      <th className="py-2 pr-2" style={{ width: 24 }}><input type="checkbox" checked={allPageSelected} onChange={togglePage} className="accent-brand cursor-pointer" title="Select this page" /></th>
                        ...
                        return (
                          <tr key={vm.id} className="border-b border-cohesity-border/50" style={moved ? { background: 'rgba(0,145,218,0.06)' } : undefined}>
                            ...
                            <td className="py-2 pr-3">
                              {moved ? (
                                <span className="text-xs text-brand" style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>→ <SiteDot site={siteById.get(target)} size={8} /> {vm.targetName}</span>
                ...
                <div className="flex flex-col gap-2">
                  {manySites ? (
                    <div className="flex flex-wrap items-center gap-2">
                      <select value={preset.from} onChange={(e) => setPreset((p) => ({ ...p, from: e.target.value }))} className="vc-input" style={{ width: 'auto', cursor: 'pointer', flex: '1 1 120px' }}>
                        <option value="">From site…</option>
                        {sites.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                      </select>
                      <span className="text-ink-faint">→</span>
                      <select value={preset.to} onChange={(e) => setPreset((p) => ({ ...p, to: e.target.value }))} className="vc-input" style={{ width: 'auto', cursor: 'pointer', flex: '1 1 120px' }}>
                        <option value="">To site…</option>
                        {sites.filter((s) => String(s.id) !== preset.from).map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                      </select>
                      <button onClick={() => moveAll(Number(preset.from), Number(preset.to))} disabled={!preset.from || !preset.to} className="vc-btn-ghost">Move all</button>
                    </div>
                  ) : sites.flatMap((from) => sites.filter((to) => to.id !== from.id).map((to) => (
                    <button key={`${from.id}-${to.id}`} onClick={() => moveAll(from.id, to.id)} className="vc-btn-ghost" style={{ justifyContent: 'flex-start' }}>
                      <SiteDot site={from} size={8} /> {from.name} <span className="text-ink-faint">→</span> <SiteDot site={to} size={8} /> {to.name}
                    </button>
                  )))}
                  {movedCount > 0 && (
                    <button onClick={() => { setScenario(new Map()); setSelected(new Set()); }} className="vc-btn-ghost" style={{ justifyContent: 'center', color: 'var(--vc-crit)' }}>Reset scenario</button>
                  )}
                </div>
              </div>
              ...
              <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                ...
                <div className="flex flex-col gap-3">
                  {impactSites.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="rounded-lg px-3 py-2" style={{ background: 'var(--vc-surface-overlay)', border: `1px solid ${over ? 'rgba(248,113,113,0.4)' : 'var(--vc-border)'}` }}>
