// Brocade SAN AI Advisor: fabric health/switch lifecycle/zoning review/port
// health reports. Ported from backend/services/advisors/brocadeAdvisor.js.
// The original module eagerly required the host's db + services/
// platformAdvisor at require-time; a bundled plugin has neither available
// until coreApi is handed to it, so this exports a FACTORY —
// createBrocadeAdvisor(coreApi) — built lazily by router.js once coreApi is
// known (dell pack advisor.js pattern). Per the plugin contract, coreApi.
// advisor is the host's services/platformAdvisor module
// (createPlatformAdvisor/linReg/parseUtcMs/fmtBytes), never required
// directly. Lifecycle status uses the same FOS_LIFECYCLE table as issues.js
// (duplicated here rather than required, to keep this file self-contained
// with no cross-module coupling beyond coreApi).
const FOS_LIFECYCLE = {
  '7.4': { eos: '2020-02-22' },
  '8.0': { eos: '2020-11-30' },
  '8.1': { eos: '2022-02-28' },
...
function createBrocadeAdvisor(coreApi) {
  const db = coreApi.db;
  const { createPlatformAdvisor } = coreApi.advisor;
  const { computeIssues, healthWarnScore, healthCritScore } = require('./issues');
  ...
  function gatherFabricHealth() {
    ...
    return {
      ...
      issues: {
        ...
        countsByType: byType,
        top: topIssues,
        thresholds: { healthWarnScore: healthWarnScore(coreApi), healthCritScore: healthCritScore(coreApi) },
      },
  ...
  function gatherZoningReview() {
    ...
    const issues = computeIssues(coreApi).filter((i) => i.type === 'zone_default_access' || i.type === 'zone_drift')
...
module.exports = { createBrocadeAdvisor };
