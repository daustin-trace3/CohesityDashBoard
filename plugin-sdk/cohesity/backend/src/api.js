// Cohesity API client, ported VERBATIM from backend/services/cohesityApi.js
// (SPECIAL DISPENSATION per the eng review: axios@1.7.2 is installed as a
// plugin-sdk devDep, like AWS's @aws-sdk precedent, so this bundles rather
// than being rewritten onto node https). Only change from the built-in:
// every function now takes `coreApi` as an explicit parameter (dell api.js
// convention) instead of requiring the host's './encryption' and
// './settings' directly — used for coreApi.encryption.decrypt and
// coreApi.settings.getHeliosApiKey.
//
// Also includes backend/services/helios.js (heliosAllClusters) — 45 lines,
// small enough to fold into this module rather than a separate file.
 ...
 */
async function getAuthenticatedClient(cluster, coreApi) {
  const credentials = JSON.parse(coreApi.encryption.decrypt(cluster.encrypted_credentials));

  if (cluster.connection_type === 'helios') {
    const apiKey = credentials.apiKey || coreApi.settings.getHeliosApiKey();
 ...
 */
async function testClusterConnection(config, coreApi) {
 ...
 */
async function fetchClusterInfo(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const { data } = await client.get('/irisservices/api/v1/public/cluster?fetchStats=true');
  return data;
}
 ...
 */
async function fetchNodes(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
 ...
 * Fetch active alerts.
 */
async function fetchAlerts(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
...
async function fetchChassis(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  try {
 ...
 */
async function fetchNodesV2(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  try {
 ...
 */
async function fetchClusterStatus(cluster, coreApi) {
  return fetchClusterInfo(cluster, coreApi);
}

/**
 * Fetch protection runs (v1 public/protectionRuns endpoint).
 */
async function fetchProtectionRuns(cluster, coreApi, numRuns = 100, startTimeUsecs = null, endTimeUsecs = null, jobId = null) {
  const client = await getAuthenticatedClient(cluster, coreApi);
...
async function fetchProtectionJobs(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
 ...
 */
async function listProtectionGroupsV2(cluster, coreApi, options = {}) {
  const client = await getAuthenticatedClient(cluster, coreApi);
 ...
 */
async function getProtectionGroupRunsV2(cluster, coreApi, protectionGroupId, options = {}) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const params = new URLSearchParams();
 ...
 */
async function fetchProtectionPolicies(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  try {
 ...
 */
async function fetchSourceRegistrations(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const { data } = await client.get(
 ...
 * @param {string} resolutionText  free-text note stored on the resolution
 */
async function resolveAlerts(cluster, coreApi, alertIdList, resolutionText = 'Resolved from ICC') {
  const client = await getAuthenticatedClient(cluster, coreApi);
 ...
 */
async function fetchGflags(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
 ...
 */
async function fetchSearchObjects(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const objects = [];
  let cookie = null;
  let pages = 0;
 ...
 */
async function fetchProtectedObjectTimes(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const times = new Map();
  let cookie = null;
  let pages = 0;
 ...
 */
async function fetchPhysicalAgents(cluster, coreApi) {
  const client = await getAuthenticatedClient(cluster, coreApi);
  const { data } = await client.get(
...
/**
 * backend/services/helios.js port (heliosAllClusters only — the sole export
 * any route/service actually calls; heliosClusters/heliosClustersV2/
 * buildHeliosClient are unreferenced elsewhere and omitted).
 */
