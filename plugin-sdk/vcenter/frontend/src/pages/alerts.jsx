// vCenter Alerts — ported from the shared frontend/src/components/IssueAlertsPage.jsx
// (platform="vcenter" config), which isn't importable in the plugin sandbox.
// Same two panels: open issues (recomputed live) and issue history with a
// day-range picker, against the vCenter plugin's own /issues + /issue-history.
...
function CurrentIssuesPanel({ rows }) {
  ...
  const ctl = useTableControls(list, {
    searchKeys: ['message', 'type', 'target', 'vcenter'],
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      ...
      <TableControls ctl={ctl} rows={list} searchPlaceholder="Filter by message, type or target…"
        filters={[{ k: 'severity', label: 'Severities' }, { k: 'type', label: 'Types' }, { k: 'vcenter', label: 'vCenters' }]} />
      {rows == null ? (
            ...
            <tbody>
              {ctl.pageRows.map((i, idx) => (
                <tr key={`${i.type}|${i.target}|${idx}`} className="border-b border-cohesity-border/50">
                  ...
                  <td className="py-2 pr-3 text-ink-muted whitespace-nowrap">{i.vcenter || '—'}</td>
...
export default function VcAlertsPage() {
  ...
  const load = React.useCallback(() => Promise.all([
    apiFetch('/vcenter/issues').then((json) => setIssues(Array.isArray(json) ? json : [])),
    apiFetch(`/vcenter/issue-history?days=${days}`).then((json) => setHistory(Array.isArray(json) ? json : [])).catch(() => setHistory([])),
  ]).then(() => setLastRefreshed(new Date()))
    .catch(() => setIssues((i) => i || [])), [days]);

  React.useEffect(() => { load(); }, [load]);

  return (
    <div className="animate-fade-in">
      <PageHeader icon={Bell} title="Alerts" description="Computed issues across all registered vCenters, with open/resolved history">
