// vCenter Events — ported from frontend/src/pages/vcenter/VcEventsPage.jsx.
import { History, ShieldAlert, ScrollText } from '../icons.jsx';
import {
  apiFetch, PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TableControls, TablePager,
  BRAND, severityTone, fmtWhen, asDate,
} from '../ui.jsx';
...
function IssueTimelineSection({ rows }) {
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <p className="text-sm font-semibold text-ink mb-1" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><ShieldAlert size={15} className="text-brand" /> Issue Timeline</p>
          ...
          {ctl.rows.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {ctl.pageRows.map((r) => (
                    <tr key={r.id} className={`border-b border-cohesity-border/50 ${r.status === 'resolved' ? 'opacity-50' : ''}`}>
  ...
  return (
    <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <div className="flex flex-wrap items-center gap-2 mb-1">
        <p className="text-sm font-semibold text-ink mr-auto" style={{ display: 'flex', alignItems: 'center', gap: 8 }}><ScrollText size={15} className="text-brand" /> vSphere Events</p>
...
export default function VcEventsPage() {
  ...
  const load = React.useCallback(() => Promise.all([
    apiFetch('/vcenter/issue-history').then((json) => setIssues(Array.isArray(json) ? json : [])),
    apiFetch(`/vcenter/events?days=${days}`).then((json) => setEvents(Array.isArray(json) ? json : [])),
  ]).then(() => setLastRefreshed(new Date()))
    .catch(() => { setIssues((i) => i || []); setEvents((e) => e || []); }), [days]);
