...
import { DiscAlbum, AlertTriangle } from 'lucide-react';
...
export default function AriaImagesAuditPage() {
  ...
  const load = useCallback(() => client.get('/aria/image-usage')
    .then(({ data }) => { setUsage(data); setLastRefreshed(new Date()); })
    .catch(() => { setUsage({ mappings: [], fabricImages: [], blueprintCount: 0 }); toast({ type: 'error', title: 'Failed to load image usage' }); }), [toast]);
