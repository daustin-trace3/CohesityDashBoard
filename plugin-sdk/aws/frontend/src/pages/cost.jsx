// Ported from frontend/src/pages/aws/AwsCostPage.jsx — react-chartjs-2 <Bar>
// replaced with charts.jsx BarChart (window.Chart), client.get replaced with
// apiFetch. The external HTML tooltip renderer is kept as-is (it manipulates
// the DOM directly, no host imports involved) but scoped classnames to avoid
// leaking into host pages.
import { DollarSign } from '../icons.jsx';
import { apiFetch, PageHeader, StatCard, LoadingPanel, RefreshButton, LastUpdated, BRAND, fmtUsd } from '../ui.jsx';
...
function renderCostTooltip(context) {
  ...
  let el = parent.querySelector('.aw-cost-tt');
  if (!el) {
    el = document.createElement('div');
    el.className = 'aw-cost-tt';
...
export default function AwsCostPage() {
  ...
  const [usageTypes, setUsageTypes] = React.useState(null);
  const [instanceTypes, setInstanceTypes] = React.useState(null);
  ...
  React.useEffect(() => {
    apiFetch('/aws/accounts').then((json) => setAccounts(Array.isArray(json) ? json : json?.accounts || [])).catch(() => setAccounts([]));
  }, []);
  ...
  const load = React.useCallback(() => {
    ...
    return Promise.all([
      apiFetch('/aws/costs', { params }).then((json) => setData(json)),
      apiFetch('/aws/costs/usage-types', { params: usageParams }).then((json) => setUsageTypes(json?.rows || [])).catch(() => setUsageTypes([])),
      apiFetch('/aws/costs/instance-types', { params: base }).then((json) => setInstanceTypes(json?.rows || [])).catch(() => setInstanceTypes([])),
    ]).then(() => { setLastRefreshed(new Date()); setError(null); })
      .catch(() => { setData({ days: [], byService: [] }); setError('Failed to load AWS costs'); });
  }, [days, accountId, selectedDay, month]);
  ...
  // Bar-click day-select handled via the chart's built-in onClick, wired
  // through a ref to the live Chart.js instance rendered by BarChart below.
  const chartRef = React.useRef(null);
  React.useEffect(() => {
    const canvas = canvasWrapRef.current?.querySelector('canvas');
    if (!canvas || !window.Chart) return undefined;
    const inst = window.Chart.getChart(canvas);
    chartRef.current = inst || null;
    if (!inst) return undefined;
    const handler = (evt) => {
      if (isMonthly) return;
      const points = inst.getElementsAtEventForMode(evt, 'index', { intersect: false }, false);
      if (points?.length) {
        const day = byDay[points[0].index]?.day;
        if (day) setSelectedDay((cur) => (cur === day ? '' : day));
      }
    };
    canvas.addEventListener('click', handler);
    return () => canvas.removeEventListener('click', handler);
  }, [dailyBar, isMonthly, byDay]);
    ...
    <div className="animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <StatCard icon={DollarSign} label="Day-over-Day" value={month ? '—' : (data?.deltaPct != null ? `${data.deltaPct > 0 ? '+' : ''}${data.deltaPct.toFixed(1)}%` : '—')}
          sub="vs previous day" tone={!month && data?.deltaPct > 0 ? 'warn' : 'default'} />
      </div>
      ...
      <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
        ...
        {data == null ? (
          ...
          <div className="h-64" ref={canvasWrapRef}>
            <BarChartCanvas data={dailyBar} options={chartOpts} />
          </div>
        )}
      </div>
...
// Thin local wrapper around the shared BarChart so this file doesn't need a
// top-level import cycle with charts.jsx while still using window.Chart.
function BarChartCanvas({ data, options }) {
  const canvasRef = React.useRef(null);
  const chartRef = React.useRef(null);

  React.useEffect(() => {
    if (!window.Chart || !canvasRef.current) return undefined;
    chartRef.current = new window.Chart(canvasRef.current, { type: 'bar', data, options });
