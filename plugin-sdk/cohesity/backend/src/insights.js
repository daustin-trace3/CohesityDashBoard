// Rule-based prioritized insights, ported from backend/services/insights.js.
// db via coreApi. Contract file for snapshot.js's optional './insights'
// require — exports computeInsights(coreApi) with that exact signature.
function linReg(pts) {
  const n = pts.length;
  if (n < 2) return null;
...
function computeInsights(coreApi) {
  const db = coreApi.db;
  const insights = [];
  const now = Date.now();
...
module.exports = { computeInsights, FAILURE_STATUSES, fmtBytes, linReg, parseUtcMs };
