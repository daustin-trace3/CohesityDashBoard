// Aria Alerts — ported from the built-in shared components/IssueAlertsPage.jsx
// (not importable in a plugin sandbox), narrowed to the aria-only config
// entry. Computed issues with an open/resolved lifecycle rather than
// vendor-raised alert rows — /aria/issues + /aria/issue-history.
...
function CurrentIssuesPanel({ rows }) {
  ...
  const ctl = useTableControls(list, {
    searchKeys: ['message', 'type', 'target', 'instance'],
  ...
  const csvColumns = [
    ...
    { label: 'Instance', get: (i) => i.instance },
  ...
  return (
    <div className="panel p-4 mb-4" style={{ borderTop: `3px solid ${BRAND}` }}>
      <div className="flex items-center justify-between gap-2 mb-2">
        ...
        <CsvExportButton filename="aria-issues" rows={ctl.rows} columns={csvColumns} />
      </div>
      <TableControls ctl={ctl} rows={list} searchPlaceholder="Filter by message, type or target…"
        filters={[{ k: 'severity', label: 'Severities' }, { k: 'type', label: 'Types' }, { k: 'instance', label: 'Instances' }]} />
      {rows == null ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="text-left text-[11px] uppercase tracking-wide text-ink-faint border-b border-cohesity-border">
              ...
              <SortTh k="instance" label="Instance" ctl={ctl} />
            ...
            <tbody>
              {ctl.pageRows.map((i, idx) => (
                <tr key={`${i.type}|${i.target}|${idx}`} className="border-b border-cohesity-border/50">
                  ...
                  <td className="py-2 pr-3 text-ink-muted whitespace-nowrap">{i.instance || '—'}</td>
...
export default function AriaAlertsPage() {
  ...
  const load = React.useCallback(() => Promise.all([
    apiFetch('/aria/issues').then((json) => setIssues(Array.isArray(json) ? json : json?.issues || [])),
    apiFetch(`/aria/issue-history?days=${days}`).then((json) => setHistory(json)).catch(() => setHistory([])),
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader icon={Bell} title="Alerts" description="Computed issues across all registered Aria Automation instances, with open/resolved history">
