// UniFi routes, ported from backend/routes/unifi.js. Mounted by the host
// dispatcher at /api/unifi — paths below are relative.
//
...
// next) function (plugin-sdk/nutanix/backend/src/router.js pattern). This
// file hand-matches req.method/req.path against a route table and
// re-implements the validation express-validator did inline, preserving the
// same status codes (400 invalid params, 404 missing, 409 duplicate, 502
// upstream/test-connection/snapshot failure) and JSON response shapes
// EXACTLY — including the unwrapped array/object shapes the built-in
// returns (unlike nutanix's `{sources: [...]}` convention, UniFi's GET
// /sources returns a bare array, POST/PUT /sources return a bare object,
// etc. — preserved verbatim here).
...
function publicSource(coreApi, row) {
  return {
    ...
    deviceCount: coreApi.db.prepare('SELECT COUNT(*) n FROM unifi_devices WHERE source_id = ?').get(row.id).n,
  };
}
...
function handlePostSources(req, res, coreApi) {
  ...
  const dup = db.prepare('SELECT id FROM unifi_sources WHERE name = ?').get(name);
  ...
  const info = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(name, b.host.trim(), b.port ? parseIntStrict(b.port) : 443,
    coreApi.encryption.encrypt(JSON.stringify({ apiKey: b.apiKey })),
    toBool(b.sslVerify) ? 1 : 0, b.pollingIntervalMinutes ? parseIntStrict(b.pollingIntervalMinutes) : 10);
  ...
  res.status(201).json(publicSource(coreApi, row));
}
...
function handlePutSource(req, res, coreApi) {
  ...
  `).run(
    ...
    b.apiKey ? coreApi.encryption.encrypt(JSON.stringify({ apiKey: b.apiKey })) : row.encrypted_credentials,
  ...
  res.json(publicSource(coreApi, updated));
}
...
async function handlePostSourcesTest(req, res, coreApi) {
  ...
  if (id) {
    ...
    candidate = { host: String(host).trim(), apiKey, port: port ? parseIntStrict(port) : 443, ssl_verify: toBool(sslVerify) ? 1 : 0 };
  }
...
async function handlePostSourcePoll(req, res, coreApi) {
  const id = requireIdParam(req, res);
  if (id === null) return;
  const row = coreApi.db.prepare('SELECT * FROM unifi_sources WHERE id = ?').get(id);
  if (!row) return res.status(404).json({ error: 'UniFi source not found.' });
...
async function handleGetSourceProbe(req, res, coreApi) {
  ...
  await run('info', () => api.fetchInfo(row, coreApi));
  let sites = [];
  await run('sites', async () => { sites = await api.fetchSites(row, coreApi); return sites; });
  const site = sites[0]?.internalReference || 'default';

  await run('devicesFirst', async () => (await api.fetchDevices(row, coreApi, site))[0] || null);
  await run('clientFirst', async () => (await api.fetchClients(row, coreApi, site))[0] || null);
  await run('health', () => api.fetchHealth(row, coreApi, site));
  await run('wlans', () => api.fetchWlans(row, coreApi, site));
  await run('rogueCount', async () => (await api.fetchRogueAps(row, coreApi, site)).length);
  await run('topologyCounts', async () => {
    const t = await api.fetchTopology(row, coreApi, site);
    return { vertices: t.vertices.length, edges: t.edges.length };
  });
  await run('systemLogFirst', async () => (await api.fetchSystemLog(row, coreApi, site))[0] || null);
...
const currentFeatures = (coreApi) => ({
  protect: featureEnabled(coreApi, 'protect'),
  wifi: featureEnabled(coreApi, 'wifi'),
  security: featureEnabled(coreApi, 'security'),
});
...
function handleGetOverview(req, res, coreApi) {
  ...
  res.json({
    ...
    health,
    issueCounts,
    features: currentFeatures(coreApi),
    spark,
  });
}
...
function handleGetDeviceByMac(req, res, coreApi) {
  if (!isNonEmptyString(req.params.mac)) return badRequest(res, [fail('mac')]);
  const db = coreApi.db;
...
function handleGetPortHistory(req, res, coreApi) {
  if (!isNonEmptyString(req.params.mac)) return badRequest(res, [fail('mac')]);
  const port = parseQueryInt(req.query.port, 0, 1000);
  if (!port.ok || port.value === undefined) return badRequest(res, [fail('port')]);
  const hours = parseQueryInt(req.query.hours, 1, 720);
  if (!hours.ok) return badRequest(res, [fail('hours')]);
  res.json(coreApi.db.prepare(`
    SELECT * FROM unifi_port_history
    WHERE device_mac = ? AND port_idx = ? AND captured_at >= datetime('now', ?)
    ORDER BY captured_at ASC
  `).all(req.params.mac, port.value, `-${hours.value || 168} hours`));
}
...
function handleGetClients(req, res, coreApi) {
  res.json(coreApi.db.prepare(`
...
async function cachedHourlyReport(coreApi, source, site, scope, hours) {
  ...
  const data = await api.fetchHourlyReport(source, coreApi, site, scope, hours);
  hourlyReportCache.set(key, { at: Date.now(), data });
  return data;
}

async function handleGetWifi(req, res, coreApi) {
  const hoursQ = parseQueryInt(req.query.hours, 1, 168);
  if (!hoursQ.ok) return badRequest(res, [fail('hours')]);
  const db = coreApi.db;
  if (!featureEnabled(coreApi, 'wifi')) return res.json({ disabled: true, wlans: [], radios: [], rogues: [], signalBuckets: {}, signalByAp: [], roaming: [], history: { site: [], aps: [] } });
  ...
  for (const s of historySources) {
    ...
    try {
      const rows = await cachedHourlyReport(coreApi, s, site, 'site', hours);
      for (const r of rows) {
        siteHistory.push({
    ...
    try {
      const rows = await cachedHourlyReport(coreApi, s, site, 'ap', hours);
      for (const r of rows) {
...
function handleGetSecurity(req, res, coreApi) {
  const db = coreApi.db;
  if (!featureEnabled(coreApi, 'security')) return res.json({ disabled: true, ips: null, rogueCounts: { total: 0, flagged: 0 }, events: [], posture: [], rules: { firewall: [], traffic: [] }, timeline: [], topDestinations: [], topOffenders: [], policyHits: [], rogueChanges: { newThisWeek: 0, flagged: [] } });
...
function handleGetEvents(req, res, coreApi) {
  if (req.query.category !== undefined && !isNonEmptyString(req.query.category, 60)) return badRequest(res, [fail('category')]);
  const limitQ = parseQueryInt(req.query.limit, 1, 500);
...
function handleGetTopology(req, res, coreApi) {
  const db = coreApi.db;
...
function handleGetWan(req, res, coreApi) {
  const db = coreApi.db;
  ...
  const latHot = wanLatencyWarnMs(coreApi) * 2;
  const outages = [];
...
function handleGetProtect(req, res, coreApi) {
  const db = coreApi.db;
  if (!featureEnabled(coreApi, 'protect')) return res.json({ disabled: true, cameras: [], nvrs: [] });
...
async function handleGetSnapshot(req, res, coreApi) {
  if (!isNonEmptyString(req.params.id)) return badRequest(res, [fail('id')]);
  const row = coreApi.db.prepare('SELECT c.*, s.*, c.camera_id AS cam_id FROM unifi_cameras c JOIN unifi_sources s ON s.id = c.source_id WHERE c.camera_id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'unknown camera' });
  try {
    const snap = await api.fetchCameraSnapshot(row, coreApi, row.cam_id);
...
function handleGetFeatures(req, res, coreApi) {
  res.json({ features: currentFeatures(coreApi) });
}

function handlePutFeatures(req, res, coreApi) {
  const b = req.body || {};
  const errors = [];
  for (const k of ['protect', 'wifi', 'security']) {
    if (b[k] !== undefined && !isBooleanish(b[k])) errors.push(fail(k));
  }
  if (errors.length) return badRequest(res, errors);
  for (const k of ['protect', 'wifi', 'security']) {
    if (b[k] !== undefined) coreApi.settings.setSetting(`unifi_feature_${k}`, toBool(b[k]) ? '1' : '0');
  }
  res.json({ features: currentFeatures(coreApi) });
}

function configThresholds(coreApi) {
  return {
    unifiWanLatencyWarnMs: wanLatencyWarnMs(coreApi),
    unifiWanAvailWarnPct: wanAvailWarnPct(coreApi),
    unifiPortErrDeltaWarn: portErrDeltaWarn(coreApi),
    unifiPortFlapWarn: portFlapWarn(coreApi),
    unifiDeviceCpuWarnPct: deviceCpuWarnPct(coreApi),
    unifiDeviceMemWarnPct: deviceMemWarnPct(coreApi),
    unifiTempWarnC: tempWarnC(coreApi),
    unifiSatisfactionWarn: satisfactionWarn(coreApi),
    unifiNewDeviceDays: newDeviceDays(coreApi),
  };
}
...
const CONFIG_MAP = {
  unifiWanLatencyWarnMs: { key: 'unifi_wan_latency_warn_ms', min: 1, max: 5000 },
  unifiWanAvailWarnPct: { key: 'unifi_wan_avail_warn_pct', min: 0, max: 100 },
  unifiPortErrDeltaWarn: { key: 'unifi_port_err_delta_warn', min: 1, max: 1000000 },
  unifiPortFlapWarn: { key: 'unifi_port_flap_warn', min: 1, max: 100 },
  unifiDeviceCpuWarnPct: { key: 'unifi_device_cpu_warn_pct', min: 1, max: 100 },
  unifiDeviceMemWarnPct: { key: 'unifi_device_mem_warn_pct', min: 1, max: 100 },
  unifiTempWarnC: { key: 'unifi_temp_warn_c', min: 1, max: 200 },
  unifiSatisfactionWarn: { key: 'unifi_satisfaction_warn', min: 1, max: 100 },
  unifiNewDeviceDays: { key: 'unifi_new_device_days', min: 1, max: 30 },
};

function handlePutConfig(req, res, coreApi) {
  const b = req.body || {};
  const errors = [];
  for (const [k, { min, max }] of Object.entries(CONFIG_MAP)) {
    if (b[k] === undefined) continue;
    const n = parseIntStrict(b[k]);
    if (!Number.isInteger(n) || n < min || n > max) errors.push(fail(k));
  }
  if (errors.length) return badRequest(res, errors);
  for (const [k, { key }] of Object.entries(CONFIG_MAP)) {
    if (b[k] !== undefined) coreApi.settings.setSetting(key, String(parseIntStrict(b[k])));
  }
  res.json({ thresholds: configThresholds(coreApi) });
}
...
const ROUTES = [
  ...
  { method: 'GET', ...compile('/devices/:mac'), handler: handleGetDeviceByMac },
  { method: 'GET', ...compile('/devices/:mac/port-history'), handler: handleGetPortHistory },
  { method: 'GET', ...compile('/ports'), handler: handleGetPorts },
  { method: 'GET', ...compile('/clients'), handler: handleGetClients },
  { method: 'GET', ...compile('/wifi'), handler: handleGetWifi },
  { method: 'GET', ...compile('/security'), handler: handleGetSecurity },
  { method: 'GET', ...compile('/events'), handler: handleGetEvents },
  { method: 'GET', ...compile('/topology'), handler: handleGetTopology },
  { method: 'GET', ...compile('/wan'), handler: handleGetWan },
  ...
  { method: 'GET', ...compile('/protect'), handler: handleGetProtect },
  { method: 'GET', ...compile('/protect/cameras/:id/snapshot'), handler: handleGetSnapshot },
  { method: 'GET', ...compile('/features'), handler: handleGetFeatures },
...
function createRouter(coreApi) {
  return function unifiRouter(req, res, next) {
