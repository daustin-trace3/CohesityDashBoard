// Veritas NetBackup plugin style kit ("nb-" prefix) — cloned from the
// Proxmox VE plugin's ui.jsx (itself cloned from the host dashboard's design
// system), brand color swapped to Veritas red (#B1181E), the same color the
// host frontend/src/platforms/netbackup/index.jsx and pages/netbackup/helpers.js
// already use. No Tailwind classnames are available inside a plugin bundle
// (the host's CSS purge only scans host source), so this kit installs real
// CSS via a single injected <style> tag and exposes className-based
// components that consume it.

const STYLE_ID = 'nb-plugin-styles';

const CSS = `
:root {
  --nb-surface-base: #0B1015;
  --nb-surface: #131B23;
  --nb-surface-raised: #18222C;
  --nb-surface-overlay: #1E2A36;
  --nb-border: #1F2B37;
  --nb-ink: #E8EDF2;
  --nb-ink-muted: #94A3B3;
  --nb-ink-faint: #5F7081;
  --nb-brand: #B1181E;
  --nb-brand-dark: #8A1218;
  --nb-ok: #34D399;
  --nb-warn: #FBBF24;
  --nb-crit: #F87171;
  --nb-info: #60A5FA;
...
.nb-root { font-family: inherit; color: var(--nb-ink); }
...
.nb-panel-hover:hover {
  border-color: rgba(177,24,30,0.4);
  box-shadow: 0 4px 16px rgba(0,0,0,.45), 0 0 0 1px rgba(177,24,30,.18);
}
.nb-panel-title {
  font-size: 11px;
  font-weight: 600;
...
.nb-tnum { font-variant-numeric: tabular-nums; }

.nb-skeleton {
  background: linear-gradient(90deg, #18222C 25%, #1E2A36 37%, #18222C 63%);
  background-size: 400px 100%;
  animation: nb-shimmer 1.6s linear infinite;
  border-radius: 6px;
}
...
.nb-fade-in { animation: nb-fade-in 220ms ease-out both; }
@keyframes nb-fade-in {
  from { opacity: 0; transform: translateY(4px); }
  to { opacity: 1; transform: translateY(0); }
}
.nb-stagger > * {
  animation: nb-fade-in 220ms ease-out both;
  animation-delay: calc(var(--nb-i, 0) * 20ms);
}
...
.nb-orb {
  ...
  height: 6px;
  border-radius: 50%;
  background: var(--nb-ok);
  animation: nb-orb-pulse 2.5s ease-in-out infinite;
}
@keyframes nb-orb-pulse {
  0%, 100% { opacity: 1; }
  50% { opacity: 0.4; }
}

.nb-pulse-crit { animation: nb-pulse-red 1.8s ease-in-out infinite; }
@keyframes nb-pulse-red {
...
.nb-row:hover { background: rgba(30,42,54,0.5); }

.nb-input {
  width: 100%;
  background: var(--nb-surface-overlay);
  border: 1px solid var(--nb-border);
  border-radius: 8px;
  padding: 6px 12px;
  font-size: 13px;
...
.nb-input:focus { border-color: rgba(177,24,30,0.6); }
...
.nb-btn-ghost:hover { color: var(--nb-ink); border-color: rgba(177,24,30,0.4); }
.nb-btn-ghost:disabled { opacity: 0.5; cursor: default; }
...
.nb-btn-accent {
  ...
  font-size: 12px;
  font-weight: 600;
  border: 1px solid rgba(177,24,30,0.3);
  background: rgba(177,24,30,0.1);
  color: var(--nb-brand);
  cursor: pointer;
  transition: background 150ms;
}
.nb-btn-accent:hover { background: rgba(177,24,30,0.2); }
.nb-btn-accent:disabled { opacity: 0.5; cursor: default; }
...
.nb-scroll::-webkit-scrollbar { width: 8px; height: 8px; }
.nb-scroll::-webkit-scrollbar-track { background: transparent; }
.nb-scroll::-webkit-scrollbar-thumb { background: #2A3845; border-radius: 4px; border: 2px solid var(--nb-surface-base); }
.nb-scroll::-webkit-scrollbar-thumb:hover { background: #3B4D5E; }
...
@media (prefers-reduced-motion: reduce) {
  ...
  .nb-skeleton { animation: none !important; }
  .nb-fade-in { animation: none !important; opacity: 1 !important; transform: none !important; }
  .nb-stagger > * { animation: none !important; }
  .nb-orb { animation: none !important; }
  .nb-pulse-crit { animation: none !important; }
}
`;
...
export const ShieldAlertIcon = (p) => <Icon {...p}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10Z" /><path d="M12 8v4M12 16h.01" /></Icon>;
export const ShieldOffIcon = (p) => <Icon {...p}><path d="M19.7 14a6.9 6.9 0 0 0 .3-2V5l-8-3-3.2 1.2M4.7 4.7 4 5v7c0 6 8 10 8 10a20.3 20.3 0 0 0 5.6-4.2M1 1l22 22" /></Icon>;
...
export const GaugeIcon = (p) => <Icon {...p}><path d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z" /><path d="M12 3a9 9 0 0 0-9 9c0 2.4.9 4.6 2.4 6.2M12 3a9 9 0 0 1 9 9c0 2.4-.9 4.6-2.4 6.2M12 12l4-4" /></Icon>;
...
export const FileTextIcon = (p) => <Icon {...p}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6Z" /><path d="M14 2v6h6M9 13h6M9 17h6" /></Icon>;
...
export const ClipboardCheckIcon = (p) => <Icon {...p}><rect x="8" y="2" width="8" height="4" rx="1" /><path d="M8 4H6a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V6a2 2 0 0 0-2-2h-2" /><path d="m9 14 2 2 4-4" /></Icon>;
...
export const ArrowLeftRightIcon = (p) => <Icon {...p}><path d="m8 3-4 4 4 4" /><path d="M4 7h16" /><path d="m16 21 4-4-4-4" /><path d="M20 17H4" /></Icon>;
...
export const CableIcon = (p) => <Icon {...p}><path d="M4 9V5a2 2 0 0 1 2-2h1a2 2 0 0 1 2 2v10a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2v-1" /><path d="M6 9h.01M11 9h.01" /><circle cx="19" cy="17" r="2" /></Icon>;
...
export const SparklesIcon = (p) => <Icon {...p}><path d="m12 3 1.9 4.9L19 9.8l-5.1 1.9L12 16.6l-1.9-4.9L5 9.8l5.1-1.9Z" /><path d="M5 3v3M3 4.5h4M19 17v3M17 18.5h4" /></Icon>;
...
export const KeyRoundIcon = (p) => <Icon {...p}><path d="M2 18v3a1 1 0 0 0 1 1h3v-2h2v-2h2l1.4-1.4a6 6 0 1 0-4-4Z" /><circle cx="16.5" cy="7.5" r="1.5" /></Icon>;
export const WorkflowIcon = (p) => <Icon {...p}><rect x="3" y="3" width="8" height="6" rx="1" /><rect x="13" y="15" width="8" height="6" rx="1" /><path d="M7 9v3a2 2 0 0 0 2 2h4a2 2 0 0 1 2 2" /></Icon>;
export const GitCompareArrowsIcon = (p) => <Icon {...p}><circle cx="6" cy="6" r="2" /><circle cx="18" cy="18" r="2" /><path d="M6 8v4a4 4 0 0 0 4 4h6" /><path d="m14 12 4 4-4 4" /><path d="M18 16v-4a4 4 0 0 0-4-4H8" /><path d="m10 12-4-4 4-4" /></Icon>;
export const InfoIcon = (p) => <Icon {...p}><circle cx="12" cy="12" r="10" /><path d="M12 16v-4M12 8h.01" /></Icon>;
export const HeartPulseIcon = (p) => <Icon {...p}><path d="M19 14c1.5-1.5 3-3.4 3-5.5A4.5 4.5 0 0 0 17.5 4c-1.7 0-3 .8-4.5 2.5C11.5 4.8 10.2 4 8.5 4A4.5 4.5 0 0 0 4 8.5c0 2.1 1.5 4 3 5.5l6.5 6.5Z" /><path d="M4 12h3l2-4 3 8 2-4h3" /></Icon>;
export const CloudIcon = (p) => <Icon {...p}><path d="M17.5 19H8a5 5 0 1 1 1.3-9.8A6 6 0 0 1 21 11.5a4 4 0 0 1-.5 7.5Z" /></Icon>;
export const GlobeIcon = (p) => <Icon {...p}><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15 15 0 0 1 0 20M12 2a15 15 0 0 0 0 20" /></Icon>;
export const CheckIcon = (p) => <Icon {...p}><path d="M20 6 9 17l-5-5" /></Icon>;
export const PlusIcon = (p) => <Icon {...p}><path d="M12 5v14M5 12h14" /></Icon>;
export const SaveIcon = (p) => <Icon {...p}><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2Z" /><path d="M17 21v-8H7v8M7 3v5h8" /></Icon>;
export const PlugIcon = (p) => <Icon {...p}><path d="M12 22v-5" /><path d="M9 8V2M15 8V2" /><path d="M18 8a2 2 0 0 1 2 2v2a8 8 0 0 1-8 8 8 8 0 0 1-8-8v-2a2 2 0 0 1 2-2Z" /></Icon>;
export const FanIcon = (p) => <Icon {...p}><path d="M12 12a3 3 0 1 0 0-6c-1.5 0-2 1.5-2 3-2 0-4.5-1-4.5 1.5S7.5 14 9.5 13c-.5 2 0 4.5-1.5 5S6 15 7 13.5A3 3 0 0 0 12 12Zm0 0c1.5 0 2-1.5 2-3 2 0 4.5 1 4.5-1.5S16.5 5 14.5 6c.5-2 0-4.5 1.5-5" /></Icon>;
export const ThermometerIcon = (p) => <Icon {...p}><path d="M14 4v10.5a4 4 0 1 1-4 0V4a2 2 0 0 1 4 0Z" /></Icon>;
export const BatteryIcon = (p) => <Icon {...p}><rect x="2" y="7" width="16" height="10" rx="2" /><path d="M22 11v2" /><path d="M6 11v2" /></Icon>;
export const UsersIcon = (p) => <Icon {...p}><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M22 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" /></Icon>;
  ...
  const item = (s) => {
    ...
    return (
      <button key={s.key} onClick={() => onSelect(s.key)} aria-current={on ? 'page' : undefined}
        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 8, textAlign: 'left', fontSize: 12, fontWeight: on ? 600 : 400, border: 'none', cursor: 'pointer', background: on ? 'var(--nb-surface-overlay)' : 'transparent', color: on ? 'var(--nb-ink)' : 'var(--nb-ink-muted)' }}>
        {Icon && <Icon size={13} style={{ color: on ? 'var(--nb-brand)' : 'var(--nb-ink-faint)' }} />}
        {s.label}
      </button>
    );
  };
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-start' }}>
      <nav aria-label={label ? label + ' settings sections' : 'Settings sections'}
        style={{ width: 224, flexShrink: 0, padding: 8, borderRadius: 12, background: 'var(--nb-surface)', border: '1px solid var(--nb-border)', borderTop: '3px solid ' + brand }}>
        {groups.map((g) => (
          <div key={g || 'all'} style={{ marginBottom: grouped ? 8 : 0 }}>
            {g && <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--nb-ink-faint)', padding: '4px 8px 0', margin: '0 0 4px' }}>{g}</p>}
...
export function PageHeader({ icon: IconComp, title, description, children }) {
  return (
    <div className="nb-fade-in" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
        {IconComp && (
          <div style={{ marginTop: 2, display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(177,24,30,0.1)', border: '1px solid rgba(177,24,30,0.2)', flexShrink: 0 }}>
            <IconComp size={18} style={{ color: 'var(--nb-brand)' }} />
          </div>
        )}
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--nb-ink)', lineHeight: 1.2, margin: 0 }}>{title}</h1>
          {description && <p style={{ fontSize: 12, color: 'var(--nb-ink-muted)', margin: '2px 0 0' }}>{description}</p>}
        </div>
      </div>
...
const TONE = {
  default: { icon: 'var(--nb-ink-muted)', iconBg: 'var(--nb-surface-overlay)', iconBorder: 'var(--nb-border)' },
  brand: { icon: 'var(--nb-brand)', iconBg: 'rgba(177,24,30,0.1)', iconBorder: 'rgba(177,24,30,0.2)' },
  ok: { icon: 'var(--nb-ok)', iconBg: 'rgba(52,211,153,0.1)', iconBorder: 'rgba(52,211,153,0.2)' },
  warn: { icon: 'var(--nb-warn)', iconBg: 'rgba(251,191,36,0.1)', iconBorder: 'rgba(251,191,36,0.2)' },
  crit: { icon: 'var(--nb-crit)', iconBg: 'rgba(248,113,113,0.1)', iconBorder: 'rgba(248,113,113,0.2)' },
  info: { icon: 'var(--nb-info)', iconBg: 'rgba(96,165,250,0.1)', iconBorder: 'rgba(96,165,250,0.2)' },
};
...
export function StatCard({ icon: IconComp, label, value, sub, tone = 'default', onClick, loading }) {
  ...
  return (
    <Tag
      onClick={onClick}
      className={`nb-panel${onClick ? ' nb-panel-hover' : ''}`}
      style={{
        padding: '14px 16px',
        display: 'flex',
        alignItems: 'center',
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--nb-ink-faint)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</p>
        {loading ? (
          <div className="nb-skeleton" style={{ height: 24, width: 80, marginTop: 4 }} />
        ) : (
          <p className="nb-tnum" style={{ fontSize: 20, fontWeight: 700, color: 'var(--nb-ink)', lineHeight: 1.2, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</p>
        )}
        {sub && !loading && (
          <p style={{ fontSize: 11, color: 'var(--nb-ink-muted)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sub}</p>
        )}
      </div>
    </Tag>
  );
...
const BADGE_TONES = {
  ok: { bg: 'rgba(52,211,153,0.1)', color: 'var(--nb-ok)', border: 'rgba(52,211,153,0.25)' },
  warn: { bg: 'rgba(251,191,36,0.1)', color: 'var(--nb-warn)', border: 'rgba(251,191,36,0.25)' },
  crit: { bg: 'rgba(248,113,113,0.1)', color: 'var(--nb-crit)', border: 'rgba(248,113,113,0.25)' },
  info: { bg: 'rgba(96,165,250,0.1)', color: 'var(--nb-info)', border: 'rgba(96,165,250,0.25)' },
  brand: { bg: 'rgba(177,24,30,0.1)', color: 'var(--nb-brand)', border: 'rgba(177,24,30,0.25)' },
  neutral: { bg: 'var(--nb-surface-overlay)', color: 'var(--nb-ink-muted)', border: 'var(--nb-border)' },
};

export function Badge({ tone = 'neutral', children, style }) {
  const t = BADGE_TONES[tone] || BADGE_TONES.neutral;
  return (
    <span className="nb-chip" style={{ background: t.bg, color: t.color, borderColor: t.border, ...style }}>
      {children}
    </span>
  );
}
...
export function Spinner({ size = 16, style }) {
  return (
    <LoaderIcon
      size={size}
      style={{ color: 'var(--nb-brand)', animation: 'nb-spin 0.8s linear infinite', ...style }}
    />
  );
}
...
if (typeof document !== 'undefined' && !document.getElementById('nb-spin-kf')) {
...
export function LoadingPanel({ label = 'Loading data…', height = 200 }) {
  return (
    <div role="status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <Spinner size={22} />
      <p style={{ fontSize: 12, color: 'var(--nb-ink-muted)', margin: 0 }}>{label}</p>
    </div>
  );
}
  ...
  return (
    <div>
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: i < rows - 1 ? '1px solid var(--nb-border)' : 'none' }}>
          {colWidths.map((w, j) => (
            <div key={j} className="nb-skeleton" style={{ height: 14, width: w }} />
          ))}
        </div>
      ))}
    </div>
...
export function EmptyState({ icon: IconComp, title = 'No data', description, children }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '40px 16px', textAlign: 'center' }}>
      {IconComp && <IconComp size={28} style={{ color: 'var(--nb-ink-faint)' }} />}
      <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--nb-ink-muted)', margin: 0 }}>{title}</p>
      {description && <p style={{ fontSize: 12, color: 'var(--nb-ink-faint)', margin: 0, maxWidth: 360 }}>{description}</p>}
      {children}
    </div>
  );
}
...
export function Panel({ title, icon: IconComp, actions, children, style, bodyStyle }) {
  return (
    <div className="nb-panel" style={{ padding: 16, ...style }}>
      {(title || actions) && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {IconComp && <IconComp size={14} style={{ color: 'var(--nb-brand)', flexShrink: 0 }} />}
            <p className="nb-panel-title" style={{ margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</p>
          </div>
...
export function LastUpdated({ date, prefix = 'Updated' }) {
  ...
  return (
    <span className="nb-tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--nb-ink-faint)' }}>
...
export function RefreshButton({ onClick, refreshing, label = 'Refresh' }) {
  return (
    <button onClick={onClick} disabled={refreshing} className="nb-btn-ghost">
      <RefreshIcon size={14} style={refreshing ? { animation: 'nb-spin 0.8s linear infinite' } : undefined} />
  ...
  return (
    <th style={{ padding: '8px 12px 8px 0', textAlign: align, ...style }}>
      <button
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, textTransform: 'uppercase', letterSpacing: '0.03em', fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'transparent', color: active ? 'var(--nb-ink)' : 'var(--nb-ink-muted)', padding: 0 }}
      >
        {label}
...
export function TableSearch({ ctl, placeholder = 'Search…', style }) {
  return (
    <div style={{ position: 'relative', width: '100%', maxWidth: 320, ...style }}>
      <SearchIcon size={14} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--nb-ink-faint)', pointerEvents: 'none' }} />
      <input
        value={ctl.q}
...
export function TableControls({ ctl, rows, searchPlaceholder, filters = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      <span className="nb-tnum" style={{ fontSize: 11, color: 'var(--nb-ink-faint)', marginLeft: 'auto' }}>
...
export function TablePager({ ctl, sizes = PAGE_SIZES }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, marginTop: 4, borderTop: '1px solid var(--nb-border)' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--nb-ink-faint)' }}>
        Rows per page
        <select
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="nb-tnum" style={{ fontSize: 11, color: 'var(--nb-ink-faint)' }}>{start}–{end} of {total}</span>
        {!all && ctl.pageCount > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span className="nb-tnum" style={{ fontSize: 11, color: 'var(--nb-ink-faint)', padding: '0 4px' }}>{ctl.page + 1} / {ctl.pageCount}</span>
...
export function CsvExportButton({ filename, columns, rows }) {
  ...
  return (
    <button onClick={onClick} disabled={!rows?.length} className="nb-btn-ghost" style={{ fontSize: 11, padding: '4px 10px' }}>
