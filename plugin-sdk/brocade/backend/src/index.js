// Brocade SAN plugin manifest.
//
// Migrations copied VERBATIM (same scope id 'brocade') so an existing local
// DB's brocade_* data (and its schema_migrations row) is adopted intact on
// install.
//
// HOOKS moved from core into this manifest (ported VERBATIM logic, db now
// via coreApi): opsSummary, collectAlerts, searchCategories, server360,
// server360Suggest — from backend/platforms/brocade/index.js.
const migrations = require('./migrations');
const { createRouter } = require('./router');
const { createBrocadePoller } = require('./poller');
...
// `params` tells the host's search route how many times to repeat the LIKE
// pattern bind (default 1) — required whenever the SQL has more than one `?`
 ...
 * registry provider contract; never throws.
 */
...
module.exports = {
  ...
  createPoller(coreApi) {
    return createBrocadePoller(coreApi);
  },
