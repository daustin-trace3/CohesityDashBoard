// Computed AWS issues (shared by router.js and the poller) plus their
// lifecycle history: each poll reconciles the freshly computed issue set
// against aws_issue_history so every issue gets first-seen / resolved
// timestamps instead of existing only as a live snapshot. Ported from
// backend/services/awsIssues.js — db/getSetting now come from coreApi rather
// than direct host requires.
function costSpikePct(coreApi) {
  const n = Number(coreApi.settings.getSetting('aws_cost_spike_pct'));
  return Number.isFinite(n) && n >= 5 && n <= 500 ? Math.round(n) : 30;
}

function rdsStorageWarnPct(coreApi) {
  const n = Number(coreApi.settings.getSetting('aws_rds_storage_warn_pct'));
...
function computeIssues(coreApi) {
  ...
  const pct = costSpikePct(coreApi);
