...
router.post('/instances/test', [
  ...
  const saved = id ? db.prepare('SELECT * FROM aria_instances WHERE id = ?').get(id) : null;
  ...
  const target = testTarget({
    stored: saved,
    incoming: req.body,
    fields: { host: 'host', username: 'username', domain: 'domain', sslVerify: 'ssl_verify' },
    secretSupplied,
  });
  const candidate = {
    host: target.host,
...
router.post('/instances/:id/refresh', [param('id').isInt().toInt()], validate, async (req, res, next) => {
  try {
    ...
    // A raw axios error carries the request config (login body, bearer
