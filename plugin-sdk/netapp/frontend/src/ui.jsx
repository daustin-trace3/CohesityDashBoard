// NetApp plugin style kit. No Tailwind build step is available inside a
// plugin bundle, so this installs a hand-rolled utility stylesheet that
// mirrors the class *names* the built-in frontend/src/pages/netapp/* pages
...
// plugin-sdk/unifi/frontend/src/ui.jsx exactly, renamed uf- -> na- and
// rebranded to NetApp blue (#0067C5 = rgb(0,103,197)).
//
...
const STYLE_ID = 'na-plugin-styles';
...
const CSS = `
:root {
  --na-surface-base: #0B1015;
  --na-surface: #131B23;
  --na-surface-raised: #18222C;
  --na-surface-overlay: #1E2A36;
  --na-border: #1F2B37;
  --na-gray: #131B23;
  --na-ink: #E8EDF2;
  --na-ink-muted: #94A3B3;
...
.na-root { font-family: inherit; color: var(--na-ink); }

/* component classes used by ported page bodies */
.panel { background: var(--na-surface); border: 1px solid var(--na-border); border-radius: 12px; box-shadow: 0 1px 2px rgba(0,0,0,.4), inset 0 0 0 1px rgba(255,255,255,.02); }
.tnum { font-variant-numeric: tabular-nums; }
.animate-fade-in { animation: na-fade-in 220ms ease-out both; }
.animate-spin { animation: na-spin 0.8s linear infinite; }
...
/* colors */
.text-ink { color: var(--na-ink); }
.text-ink-muted { color: var(--na-ink-muted); }
.text-ink-faint { color: var(--na-ink-faint); }
.text-brand { color: var(--na-brand); }
.text-status-ok { color: var(--na-ok); }
.text-status-warn { color: var(--na-warn); }
.text-status-crit { color: var(--na-crit); }
.text-status-bad { color: var(--na-crit); }
.text-status-info { color: var(--na-info); }
.text-white { color: #fff; }
.text-white\\/80 { color: rgba(255,255,255,.8); }
.bg-black\\/40 { background: rgba(0,0,0,.4); }
.bg-black\\/50 { background: rgba(0,0,0,.5); }
.bg-black\\/60 { background: rgba(0,0,0,.6); }
.bg-brand\\/10 { background: rgba(0,103,197,.1); }
.bg-brand\\/20 { background: rgba(0,103,197,.2); }
...
.bg-cohesity-gray { background: var(--na-gray); }
.bg-surface { background: var(--na-surface); }
.bg-surface-overlay { background: var(--na-surface-overlay); }
.bg-white { background: #fff; }

/* borders */
.border { border: 1px solid var(--na-border); }
.border-b { border-bottom: 1px solid var(--na-border); }
.border-t { border-top: 1px solid var(--na-border); }
.border-l { border-left: 1px solid var(--na-border); }
.border-transparent { border-color: transparent; }
.border-cohesity-border { border-color: var(--na-border); }
.border-cohesity-border\\/40 { border-color: rgba(31,43,55,.4); }
.border-cohesity-border\\/50 { border-color: rgba(31,43,55,.5); }
.border-brand\\/20 { border-color: rgba(0,103,197,.2); }
.border-brand\\/30 { border-color: rgba(0,103,197,.3); }
.border-brand\\/40 { border-color: rgba(0,103,197,.4); }
.border-brand\\/50 { border-color: rgba(0,103,197,.5); }
...
.accent-brand { accent-color: var(--na-brand); }
.accent-\\[\\#0067C5\\] { accent-color: #0067C5; }
.opacity-50 { opacity: .5; }
.ring-1 { box-shadow: 0 0 0 1px var(--na-ring, transparent); }
.ring-2 { box-shadow: 0 0 0 2px var(--na-ring, transparent); }
.ring-brand\\/30 { --na-ring: rgba(0,103,197,.3); }
.ring-brand\\/70 { --na-ring: rgba(0,103,197,.7); }
...
.hover\\:border-brand\\/50:hover { border-color: rgba(0,103,197,.5); }
.hover\\:border-status-crit\\/50:hover { border-color: rgba(248,113,113,.5); }
.hover\\:text-ink:hover { color: var(--na-ink); }
.hover\\:text-white:hover { color: #fff; }
.hover\\:text-status-crit:hover { color: var(--na-crit); }
.hover\\:underline:hover { text-decoration: underline; }
.hover\\:ring-1:hover { box-shadow: 0 0 0 1px var(--na-ring, transparent); }
.hover\\:ring-brand\\/30:hover { --na-ring: rgba(0,103,197,.3); }
.hover\\:-translate-y-0\\.5:hover { transform: translateY(-2px); }
.focus\\:border-brand\\/60:focus { border-color: rgba(0,103,197,.6); }
...
.na-scroll::-webkit-scrollbar-thumb { background: #2A3845; border-radius: 4px; border: 2px solid var(--na-surface-base); }
.na-scroll::-webkit-scrollbar-thumb:hover { background: #3B4D5E; }

.na-input {
  width: 100%;
  background: var(--na-surface-overlay);
...
.na-input:focus { border-color: rgba(0,103,197,0.6); }
...
.na-btn-ghost:hover { color: var(--na-ink); border-color: rgba(0,103,197,0.4); }
.na-btn-ghost:disabled { opacity: 0.5; cursor: default; }
...
@media (prefers-reduced-motion: reduce) {
  .na-root *, .na-root *::before, .na-root *::after {
...
/* Scope every rule under .na-root so the utility vocabulary (w-full, flex,
 ...
 * (the unifi 1.0.2 bug). :root var declarations stay global (na- prefixed,
  ...
  return parts.map((part, i) => {
    ...
    return part.replace(/(^|\{|\})(\s*)([^@{}]+?)(\s*\{)/g, (m, brace, ws, sel, open) => {
      const scoped = sel.split(',').map((s) => {
        const t = s.trim();
        if (!t || t === ':root' || t.startsWith('.na-root')) return t;
        return `.na-root ${t}`;
      }).join(', ');
...
/* ────────────────────────────────────────────────────────────────────────
 * Formatting helpers — ported from frontend/src/pages/netapp/helpers.js,
...
// Binary (1024-based) formatting, matching frontend/src/pages/netapp/helpers.js
// exactly — NetApp's own pages format capacity this way (not decimal TB/GB).
...
export function fmtLatency(usec) {
  if (usec == null || Number.isNaN(usec)) return '—';
...
export function severityTone(sev) {
  ...
  if (s === 'emergency' || s === 'alert' || s === 'critical' || s === 'error') return 'crit';
  ...
  const item = (s) => {
    ...
    return (
      <button key={s.key} onClick={() => onSelect(s.key)} aria-current={on ? 'page' : undefined}
        style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '6px 8px', borderRadius: 8, textAlign: 'left', fontSize: 12, fontWeight: on ? 600 : 400, border: 'none', cursor: 'pointer', background: on ? 'var(--na-surface-overlay)' : 'transparent', color: on ? 'var(--na-ink)' : 'var(--na-ink-muted)' }}>
        {Icon && <Icon size={13} style={{ color: on ? 'var(--na-brand)' : 'var(--na-ink-faint)' }} />}
        {s.label}
      </button>
    );
  };
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-start' }}>
      <nav aria-label={label ? label + ' settings sections' : 'Settings sections'}
        style={{ width: 224, flexShrink: 0, padding: 8, borderRadius: 12, background: 'var(--na-surface)', border: '1px solid var(--na-border)', borderTop: '3px solid ' + brand }}>
        {groups.map((g) => (
          <div key={g || 'all'} style={{ marginBottom: grouped ? 8 : 0 }}>
            {g && <p style={{ fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--na-ink-faint)', padding: '4px 8px 0', margin: '0 0 4px' }}>{g}</p>}
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 20 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, minWidth: 0 }}>
        {IconComp && (
          <div style={{ marginTop: 2, display: 'flex', height: 36, width: 36, alignItems: 'center', justifyContent: 'center', borderRadius: 8, background: 'rgba(0,103,197,0.1)', border: '1px solid rgba(0,103,197,0.2)', flexShrink: 0 }}>
            <IconComp size={18} style={{ color: 'var(--na-brand)' }} />
          </div>
        )}
        <div style={{ minWidth: 0 }}>
          <h1 style={{ fontSize: 18, fontWeight: 700, color: 'var(--na-ink)', lineHeight: 1.2, margin: 0 }}>{title}</h1>
          {description && <p style={{ fontSize: 12, color: 'var(--na-ink-muted)', margin: '2px 0 0' }}>{description}</p>}
        </div>
      </div>
...
const TONE = {
  default: { icon: 'var(--na-ink-muted)', iconBg: 'var(--na-surface-overlay)', iconBorder: 'var(--na-border)' },
  neutral: { icon: 'var(--na-ink-muted)', iconBg: 'var(--na-surface-overlay)', iconBorder: 'var(--na-border)' },
  brand: { icon: 'var(--na-brand)', iconBg: 'rgba(0,103,197,0.1)', iconBorder: 'rgba(0,103,197,0.2)' },
  ok: { icon: 'var(--na-ok)', iconBg: 'rgba(52,211,153,0.1)', iconBorder: 'rgba(52,211,153,0.2)' },
  warn: { icon: 'var(--na-warn)', iconBg: 'rgba(251,191,36,0.1)', iconBorder: 'rgba(251,191,36,0.2)' },
  crit: { icon: 'var(--na-crit)', iconBg: 'rgba(248,113,113,0.1)', iconBorder: 'rgba(248,113,113,0.2)' },
  info: { icon: 'var(--na-info)', iconBg: 'rgba(96,165,250,0.1)', iconBorder: 'rgba(96,165,250,0.2)' },
};
  ...
  return (
    ...
    >
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <p style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--na-ink-faint)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</p>
        {loading ? (
          <div style={{ height: 24, width: 80, marginTop: 4, borderRadius: 6, background: 'var(--na-surface-overlay)' }} />
        ) : (
          <p className="tnum" style={{ fontSize: 20, fontWeight: 700, color: 'var(--na-ink)', lineHeight: 1.2, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{value}</p>
        )}
        {sub && !loading && (
          <p style={{ fontSize: 11, color: 'var(--na-ink-muted)', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{sub}</p>
        )}
      </div>
    </Tag>
  );
...
const BADGE_TONES = {
  ok: { bg: 'rgba(52,211,153,0.1)', color: 'var(--na-ok)', border: 'rgba(52,211,153,0.25)' },
  warn: { bg: 'rgba(251,191,36,0.1)', color: 'var(--na-warn)', border: 'rgba(251,191,36,0.25)' },
  crit: { bg: 'rgba(248,113,113,0.1)', color: 'var(--na-crit)', border: 'rgba(248,113,113,0.25)' },
  info: { bg: 'rgba(96,165,250,0.1)', color: 'var(--na-info)', border: 'rgba(96,165,250,0.25)' },
  brand: { bg: 'rgba(0,103,197,0.1)', color: 'var(--na-brand)', border: 'rgba(0,103,197,0.25)' },
  neutral: { bg: 'var(--na-surface-overlay)', color: 'var(--na-ink-muted)', border: 'var(--na-border)' },
};

export function Badge({ tone = 'neutral', children, style, className }) {
  const t = BADGE_TONES[tone] || BADGE_TONES.neutral;
  return (
    <span className={cls('na-chip', className)} style={{ background: t.bg, color: t.color, borderColor: t.border, ...style }}>
      {children}
    </span>
  );
}
...
export function Spinner({ size = 16, style }) {
  return <LoaderGlyph size={size} style={{ color: 'var(--na-brand)', animation: 'na-spin 0.8s linear infinite', ...style }} />;
}
...
export function LoadingPanel({ label = 'Loading data…', height = 200 }) {
  return (
    <div role="status" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, height }}>
      <Spinner size={22} />
      <p style={{ fontSize: 12, color: 'var(--na-ink-muted)', margin: 0 }}>{label}</p>
    </div>
  );
}
...
export function RefreshButton({ onClick, refreshing, label = 'Refresh' }) {
  return (
    <button onClick={onClick} disabled={refreshing} className="na-btn-ghost">
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={refreshing ? { animation: 'na-spin 0.8s linear infinite' } : undefined}>
...
export function LastUpdated({ date, prefix = 'Updated' }) {
  ...
  return (
    <span className="tnum" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--na-ink-faint)' }}>
...
export function Fact({ label, value }) {
  return (
    <div>
      <p style={{ margin: 0, fontSize: 10, textTransform: 'uppercase', letterSpacing: '.05em', color: 'var(--na-ink-faint)' }}>{label}</p>
      <p className="tnum" style={{ margin: 0, fontSize: 13, color: 'var(--na-ink)' }}>{value ?? '—'}</p>
    </div>
  );
}
...
export function portalOrInline(node) {
  // Portaled content lands outside the .na-root wrapper, so re-wrap it —
  // otherwise the scoped stylesheet (see scopeCss) no longer applies.
  const wrapped = <div className="na-root">{node}</div>;
  ...
  return portalOrInline(
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} role="dialog" aria-modal="true">
      ...
      <div className="panel" style={{ position: 'relative', width: maxWidth, maxHeight: '85vh', display: 'flex', flexDirection: 'column', borderTop: `3px solid ${BRAND}` }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, padding: '16px 16px 12px', borderBottom: '1px solid var(--na-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
            {IconComp && <IconComp size={17} style={{ color: 'var(--na-brand)', flexShrink: 0 }} />}
            <div style={{ minWidth: 0 }}>
              <p style={{ margin: 0, fontSize: 14, fontWeight: 700, color: 'var(--na-ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</p>
              {subtitle && <p style={{ margin: 0, fontSize: 11, color: 'var(--na-ink-faint)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{subtitle}</p>}
            </div>
          </div>
          <button onClick={onClose} aria-label="Close" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: 28, width: 28, borderRadius: 8, border: 'none', background: 'transparent', color: 'var(--na-ink-faint)', cursor: 'pointer', flexShrink: 0 }}>
        ...
        <div className="na-scroll" style={{ padding: 16, overflowY: 'auto' }}>{children}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <th className={cls('py-2 pr-3', align === 'right' ? 'text-right' : align === 'center' ? 'text-center' : 'text-left')} style={style}>
      <button
        ...
        style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 600, cursor: 'pointer', border: 'none', background: 'transparent', color: active ? 'var(--na-ink)' : 'var(--na-ink-muted)', padding: 0 }}
      >
        {label}
        {active && (
...
export function TableSearch({ ctl, placeholder = 'Search…', className }) {
  return (
    <div className={className || 'w-full max-w-md'} style={{ position: 'relative' }}>
      <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--na-ink-faint)', pointerEvents: 'none' }}>
        <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
      </svg>
      <input value={ctl.q} onChange={(e) => ctl.setQ(e.target.value)} placeholder={placeholder} className="na-input" style={{ paddingLeft: 32 }} />
    </div>
  );
}
...
export function FilterSelect({ ctl, k, rows, label }) {
  ...
  return (
    <select value={ctl.filters[k] || ''} onChange={(e) => ctl.setFilter(k, e.target.value)} className="na-input" style={{ width: 'auto', cursor: 'pointer' }}>
...
export function TableControls({ ctl, rows, searchPlaceholder, filters = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, marginBottom: 12 }}>
      ...
      <span className="tnum" style={{ fontSize: 11, color: 'var(--na-ink-faint)', marginLeft: 'auto' }}>
...
const pagerBtnStyle = { fontSize: 11, padding: '4px 8px', borderRadius: 6, border: '1px solid var(--na-border)', background: 'transparent', color: 'var(--na-ink-muted)', cursor: 'pointer' };
...
export function TablePager({ ctl, sizes = PAGE_SIZES }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 12, paddingTop: 12, marginTop: 4, borderTop: '1px solid var(--na-border)' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'var(--na-ink-faint)' }}>
        Rows per page
        <select value={String(ctl.pageSize)} onChange={(e) => ctl.setPageSize(e.target.value === 'all' ? 'all' : Number(e.target.value))} className="na-input" style={{ width: 'auto', padding: '4px 8px', cursor: 'pointer' }}>
          {sizes.map((s) => <option key={s} value={String(s)}>{s === 'all' ? 'All' : s}</option>)}
        </select>
      </label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <span className="tnum" style={{ fontSize: 11, color: 'var(--na-ink-faint)' }}>{start}–{end} of {total}</span>
        {!all && ctl.pageCount > 1 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span className="tnum" style={{ fontSize: 11, color: 'var(--na-ink-faint)', padding: '0 4px' }}>{ctl.page + 1} / {ctl.pageCount}</span>
...
/* ────────────────────────────────────────────────────────────────────────
 * Column show/hide preference (persisted per table) + a portaled picker
 * dropdown — ported from frontend/src/components/ui/tableTools.jsx
 * useVisibleColumns/ColumnPicker. The host version uses react-dom createPortal
 * directly; here we route through portalOrInline() since window.ReactDOM
 * (react-dom/client) has no createPortal.
 * ────────────────────────────────────────────────────────────────────── */
...
export function ColumnPicker({ columns, prefs }) {
  ...
  return (
    <>
      <button ref={btnRef} onClick={toggleOpen} className="na-btn-ghost">
        <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
          <path d="M4 6h6M14 6h6M4 12h11M19 12h1M4 18h4M12 18h8" /><circle cx="12" cy="6" r="2" /><circle cx="17" cy="12" r="2" /><circle cx="10" cy="18" r="2" />
        </svg>
        Columns
      </button>
      {open && pos && portalOrInline(
        <div ref={menuRef} style={{ position: 'fixed', top: pos.top, right: pos.right, zIndex: 50, background: 'var(--na-gray)', border: '1px solid var(--na-border)', borderRadius: 8, boxShadow: '0 20px 25px -5px rgba(0,0,0,.4)', padding: 8, width: 240, maxHeight: 320, overflowY: 'auto' }}>
          {columns.map((c) => (
            <label key={c.k} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 8px', fontSize: 12, borderRadius: 6, cursor: c.always ? 'default' : 'pointer', color: c.always ? 'var(--na-ink-faint)' : 'var(--na-ink)' }}>
              <input type="checkbox" checked={c.always || prefs.show(c.k)} disabled={c.always} onChange={() => prefs.toggle(c.k)} className="accent-brand" />
              {c.label}
            </label>
          ))}
        </div>
...
/* ────────────────────────────────────────────────────────────────────────
 * Per-table CSV download of the FULL dataset — mirrors host tableTools.jsx
 * CsvExportButton. `columns` = [{ label, get }] where `get` is a field name
 * or (row) => value.
 * ────────────────────────────────────────────────────────────────────── */
export function CsvExportButton({ filename, columns, rows }) {
  ...
  return (
    <button onClick={onClick} disabled={!rows?.length} className="na-btn-ghost" style={{ opacity: rows?.length ? 1 : 0.5, cursor: rows?.length ? 'pointer' : 'not-allowed' }}>
      <svg width={12} height={12} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3v12" /><path d="m7 10 5 5 5-5" /><path d="M4 19h16" /></svg>
      Export
    </button>
  );
}
...
/* ────────────────────────────────────────────────────────────────────────
 * Reverse-DNS resolution + IP display — ported from
 * frontend/src/api/useDnsResolve.js + frontend/src/components/IpWithHost.jsx.
 * Uses the generic /dns/resolve endpoint (not NetApp-specific), which is
 * safe to call from a plugin bundle like any other apiFetch call.
 * ────────────────────────────────────────────────────────────────────── */
export function useDnsResolve(ips) {
  ...
  React.useEffect(() => {
    ...
    let cancelled = false;
    apiFetch('/dns/resolve', { method: 'POST', body: { ips: list } })
      .then((json) => { if (!cancelled) setMap(json?.map || {}); })
      .catch(() => {});
...
// Renders an IP with its reverse-resolved hostname beneath it, or — when a
// VM name is already known from inventory (vCenter/Aria) — deep-links into
// Server 360 for the full cross-platform story. window.ReactRouterDOM is
// injected the same way React/Chart are (see plugin-sdk/build.mjs).
export function IpWithHost({ ip, dns, muted = false, vm = null }) {
  if (!ip) return <span style={{ color: 'var(--na-ink-faint)' }}>—</span>;
  const Link = typeof window !== 'undefined' && window.ReactRouterDOM ? window.ReactRouterDOM.Link : 'a';
  if (vm) {
    return (
      <span className="tnum">
        <Link to={`/ops/server360?name=${encodeURIComponent(vm)}`} title="Open Server 360"
          style={{ color: 'var(--na-ink)', fontWeight: 500, display: 'block', lineHeight: 1.2, textDecoration: 'none' }}>{vm}</Link>
        <span style={{ color: 'var(--na-ink-faint)', fontSize: 11 }}>{ip}</span>
      </span>
    );
  }
  ...
  return (
    <span className="tnum">
      <span style={{ color: muted ? 'var(--na-ink-faint)' : 'var(--na-ink)' }}>{ip}</span>
      {name
        ? <span style={{ color: 'var(--na-ink-faint)', fontSize: 11, display: 'block' }}>{name}</span>
        : resolved
          ? <span style={{ color: 'var(--na-ink-faint)', opacity: 0.7, fontSize: 10, fontStyle: 'italic', display: 'block' }}>no DNS record</span>
          : null}
    </span>
  );
}
