...
async function trySection(label, fn, { required = false } = {}) {
  try {
    ...
    logger.warn(`[NutanixPoller] ${label} failed: ${safeMsg(err)}`);
...
async function collectPE(source) {
  const cluster = await trySection('PE cluster', () => nutanixApi.fetchPECluster(source), { required: true });
  const unprotected = await trySection('PE unprotected VM count', () => nutanixApi.fetchPEUnprotectedVmCount(source));
  if (cluster && unprotected != null) cluster.unprotectedVmCount = unprotected;

  const hosts = await trySection('PE hosts', () => nutanixApi.fetchPEHosts(source));
  const vms = await trySection('PE vms', () => nutanixApi.fetchPEVms(source));
  if (vms) {
    const vmStats = await trySection('PE vm stats', () => nutanixApi.fetchPEVmStats(source)) || new Map();
  ...
  const containers = await trySection('PE containers', () => nutanixApi.fetchPEContainers(source));
  if (containers) for (const c of containers) { c.clusterUuid = cluster?.uuid || null; c.clusterName = cluster?.name || null; }

  const disks = await trySection('PE disks', () => nutanixApi.fetchPEDisks(source));
  if (disks) for (const d of disks) d.clusterUuid = cluster?.uuid || null;

  const alerts = await trySection('PE alerts', () => nutanixApi.fetchPEAlerts(source));
  if (alerts) for (const a of alerts) { a.clusterUuid = cluster?.uuid || null; a.clusterName = cluster?.name || null; }

  const pds = await trySection('PE protection domains', () => nutanixApi.fetchPEPds(source));
  const replications = await trySection('PE replications', () => nutanixApi.fetchPEReplications(source));
  const remoteSites = await trySection('PE remote sites', () => nutanixApi.fetchPERemoteSites(source));
  const snapshots = await trySection('PE snapshots', () => nutanixApi.fetchPESnapshots(source));
...
async function collectPC(source) {
  const clusters = await trySection('PC clusters', () => nutanixApi.fetchPCClusters(source), { required: true });

  const hosts = await trySection('PC hosts', () => nutanixApi.fetchPCHosts(source));
  const vms = await trySection('PC vms', () => nutanixApi.fetchPCVms(source));
  if (vms) {
    const vmStats = await trySection('PC vm groups stats', () => nutanixApi.fetchGroupsVmStats(source)) || new Map();
    for (const v of vms) {
      const s = vmStats.get(v.uuid);
      if (s) Object.assign(v, s);
    }
  ...
  const alerts = await trySection('PC alerts', () => nutanixApi.fetchPCAlerts(source));
  const policies = await trySection('PC protection policies', () => nutanixApi.fetchPCPolicies(source));
  const recoveryPoints = await trySection('PC recovery points', () => nutanixApi.fetchPCRecoveryPoints(source));

  return {
    clusters, hosts, vms,
    containers: undefined, // no first-class v3 container endpoint (prism-central.md §3) — CE/PE-only for now
    disks: undefined,
...
const store = db.transaction((sourceId, data) => {
  if (data.clusters) {
  ...
  if (data.alerts) {
    ...
    for (const a of data.alerts) {
      stmt.run(sourceId, a.clusterUuid ?? null, a.clusterName ?? null, a.alertUuid, a.severity, a.title, a.message,
        a.entityType, a.entityName, a.acknowledged, a.resolved, a.createdUsecs, nutanixApi.usecsToIso(a.createdUsecs));
    }
  }
...
const storeEvents = db.transaction((sourceId, events) => {
  const stmt = db.prepare(`
  ...
  for (const e of events) {
    inserted += stmt.run(sourceId, e.clusterUuid ?? null, e.message, e.entityType, e.entityName,
      e.createdUsecs, nutanixApi.usecsToIso(e.createdUsecs)).changes;
  }
...
async function collectEvents(source) {
  try {
    const events = source.source_type === 'prism_central'
      ? await nutanixApi.fetchPCEvents(source)
      : await nutanixApi.fetchPEEvents(source, null);
    const inserted = storeEvents(source.id, events || []);
    if (inserted) logger.debug(`[NutanixPoller] ${source.name}: ${inserted} new event(s)`);
  } catch (err) {
    logger.debug(`[NutanixPoller] event fetch failed for ${source.name}: ${safeMsg(err)}`);
  }
}
...
async function pollSource(source) {
  try {
    ...
    try { reconcileIssueHistory(); } catch (err) {
      logger.warn(`[NutanixPoller] issue-history reconcile failed: ${err.message}`);
    }
  }
}

async function pollMoveConn(conn) {
  try {
    const [info, plans] = await Promise.all([moveApi.fetchAppInfo(conn), moveApi.fetchPlans(conn)]);
    const workloadLists = await Promise.all(plans.map((p) => moveApi.fetchWorkloads(conn, p)));
    const events = await moveApi.fetchEvents(conn).catch((err) => {
      logger.debug(`[NutanixPoller] Move events fetch failed for ${conn.name}: ${safeMsg(err)}`);
      return [];
    });
    ...
    db.transaction(() => {
      ...
      for (const e of events) {
        evStmt.run(conn.id, e.eventId, e.eventName, e.vmName, e.planName, e.status, e.failureNotes, e.createdUsecs, nutanixApi.usecsToIso(e.createdUsecs));
      }
    })();
    ...
    try { reconcileIssueHistory(); } catch (err) {
      logger.warn(`[NutanixPoller] issue-history reconcile (move) failed: ${err.message}`);
    }
  }
}

const nutanixPoller = createPoller({
  id: 'nutanix',
  loadSources: () => db.prepare('SELECT * FROM nutanix_sources').all(),
  intervalMinutes: (s) => s.polling_interval_minutes,
  poll: pollSource,
});

const nutanixMovePoller = createPoller({
  id: 'nutanix-move',
  loadSources: () => db.prepare('SELECT * FROM nutanix_move_conns').all(),
  poll: pollMoveConn,
});

function initNutanixPoller() {
  const sources = nutanixPoller.init();
  const moveConns = nutanixMovePoller.init();
  logger.info(`[NutanixPoller] Initialized ${sources.length} source(s), ${moveConns.length} Move connection(s)`);
  return { sources, moveConns };
}

function createNutanixPollerHandle() {
  return {
    init: () => initNutanixPoller(),
    stopAll: () => { nutanixPoller.stopAll(); nutanixMovePoller.stopAll(); },
    trigger: (sourceOrId) => {
      const source = typeof sourceOrId === 'object' ? sourceOrId : db.prepare('SELECT * FROM nutanix_sources WHERE id = ?').get(sourceOrId);
      return source ? nutanixPoller.trigger(source) : Promise.resolve();
    },
    triggerMove: (connOrId) => {
      const conn = typeof connOrId === 'object' ? connOrId : db.prepare('SELECT * FROM nutanix_move_conns WHERE id = ?').get(connOrId);
...
module.exports = {
  nutanixPoller, nutanixMovePoller, initNutanixPoller, createNutanixPollerHandle,
  pollSource, pollMoveConn,
};
