...
const { computeIssues, staleBackupHours } = require('../netbackupIssues');
...
function gatherBackupHealth() {
  ...
  return {
    ...
    failingPolicies,
    staleClients,
    staleBackupThresholdHours: staleBackupHours(),
...
function gatherCapacityPlanning() {
...
function gatherResilienceReview() {
  const sources = db.prepare('SELECT id, name FROM netbackup_sources').all();
  const names = new Map(sources.map((s) => [s.id, s.name]));
