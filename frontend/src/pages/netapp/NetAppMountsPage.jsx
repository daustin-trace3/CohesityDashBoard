...
export default function NetAppMountsPage() {
  const { toast } = useToast();
  const [rows, setRows] = useState(null);
  const [lastRefreshed, setLastRefreshed] = useState(null);

  const load = useCallback(() => client.get('/netapp/mounts')
    .then(({ data }) => { setRows(data); setLastRefreshed(new Date()); })
    .catch(() => { setRows([]); toast({ type: 'error', title: 'Failed to load mounts' }); }), [toast]);

  useEffect(() => { load(); }, [load]);

  const list = rows || [];
  const ipList = useMemo(() => [...new Set(list.map((m) => m.client_ip).filter(Boolean))], [list]);
  ...
  const COLUMNS = [
    { k: 'client_name', label: 'VM / Client', always: true,
      render: (m, i) => <td key="client_name" className="py-2 pr-3"><IpWithHost ip={m.client_ip} dns={dns} vm={m.client_name} /></td>,
      csv: (m) => m.client_name },
    { k: 'client_ip', label: 'Client IP', pickerHidden: true,
      render: () => null, csv: (m) => m.client_ip },
    ...
    { k: 'protocols', label: 'Protocol',
      render: (m) => <td key="protocols" className="py-2 pr-3 text-ink-muted text-[11px]">{m.protocols || '—'}</td>,
      csv: (m) => m.protocols },
    { k: 'smb_users', label: 'SMB User',
      render: (m) => <td key="smb_users" className="py-2 pr-3 text-ink-muted text-[11px] max-w-[160px] truncate" title={m.smb_users || ''}>{m.smb_users || '—'}</td>,
      csv: (m) => m.smb_users },
    { k: 'volume_name', label: 'Volume',
      render: (m) => <td key="volume_name" className="py-2 pr-3 text-ink font-medium max-w-[200px] truncate" title={m.volume_name || ''}>{m.volume_name || '—'}</td>,
      csv: (m) => m.volume_name },
    { k: 'svm_name', label: 'SVM',
      render: (m) => <td key="svm_name" className="py-2 pr-3 text-ink-muted">{m.svm_name || '—'}</td>,
      csv: (m) => m.svm_name },
    { k: 'array_name', label: 'Cluster',
      render: (m) => <td key="array_name" className="py-2 pr-3 text-ink-muted whitespace-nowrap">{m.array_name}</td>,
      csv: (m) => m.array_name },
    { k: 'aggregate_name', label: 'Aggregate',
      render: (m) => <td key="aggregate_name" className="py-2 pr-3 text-ink-muted text-[11px]">{m.aggregate_name || '—'}</td>,
      csv: (m) => m.aggregate_name },
    { k: 'type', label: 'Type',
      ...
      csv: (m) => m.type },
    { k: 'style', label: 'Style',
      render: (m) => <td key="style" className="py-2 pr-3 text-ink-muted text-[11px]">{m.style || '—'}</td>,
      csv: (m) => m.style },
    { k: 'junction_path', label: 'Junction Path',
      render: (m) => <td key="junction_path" className="py-2 pr-3 text-ink-muted text-[11px] tnum max-w-[220px] truncate" title={m.junction_path || ''}>{m.junction_path || '—'}</td>,
      csv: (m) => m.junction_path },
    { k: 'used_bytes', label: 'Used / Size',
      render: (m) => <td key="used_bytes" className="py-2 pr-3 text-ink-muted text-[11px] tnum whitespace-nowrap">{usedSize(m)}</td>,
      csv: (m) => usedSize(m) },
    { k: 'state', label: 'State',
  ...
  const uniqueVms = useMemo(() => new Set(list.map((m) => m.client_name || m.client_ip)).size, [list]);
  const uniqueVolumes = useMemo(() => new Set(list.map((m) => `${m.array_name}|${m.svm_name}|${m.volume_name}`)).size, [list]);
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader icon={Cable} title="Mounts" description="Live client↔volume mounts across NFS and SMB, resolved to VMs where inventory knows the IP">
        ...
        <CsvExportButton filename="netapp-mounts" rows={ctl.rows}
      ...
      <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
        ...
        {rows == null ? (
          ...
          <div className="text-sm text-ink-muted py-6 text-center">No live NFS clients or SMB sessions found.</div>
        ) : ctl.rows.length === 0 ? (
          <div className="text-sm text-ink-muted py-6 text-center">No mounts match your filters.</div>
        ) : (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {ctl.pageRows.map((m, i) => (
                  <tr key={`${m.client_ip}|${m.mount_type}|${m.svm_name}|${m.volume_name}|${i}`} className="border-b border-cohesity-border/50">
