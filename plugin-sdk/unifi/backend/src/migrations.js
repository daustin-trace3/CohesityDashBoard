...
// failed section keeps prior rows (see poller.js trySection). History/
// events/issue tables append and self-retain. Single version — ships whole.
//
// Ported VERBATIM from backend/db/migrations/unifi.js (same scope id
// 'unifi') so an existing local DB's unifi_* data (and its schema_migrations
// row) is adopted intact on install. No changes.
module.exports = [
  {
    version: 1,
    up(db) {
