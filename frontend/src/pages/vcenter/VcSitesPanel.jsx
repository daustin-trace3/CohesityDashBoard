...
import { Building2, Layers, Trash2, ArrowLeftRight } from 'lucide-react';
...
export function FailoverPairsSection({ flash }) {
  ...
  const load = useCallback(() => Promise.all([
    client.get('/vcenter/capacity/sites').then(({ data }) => setSites(data.sites || [])),
    client.get('/vcenter/capacity/pairs').then(({ data }) => setPairs(Array.isArray(data) ? data : [])),
  ...
  const addPair = async () => {
    ...
    setSaving(true);
    try {
      await client.post('/vcenter/capacity/pairs', { siteAId: Number(form.a), siteBId: Number(form.b) });
      setForm({ a: '', b: '' });
      await load();
      flash?.('success', 'Failover pair added');
    } catch (err) {
      flash?.('error', 'Failed to add pair', err?.response?.data?.error);
    } finally { setSaving(false); }
  };
  const removePair = async (p) => {
    try {
      await client.delete(`/vcenter/capacity/pairs/${p.id}`);
      await load();
    } catch (err) { flash?.('error', 'Failed to remove pair', err?.response?.data?.error); }
  };
  const byId = new Map((sites || []).map((s) => [s.id, s]));
  const sel = 'bg-surface-overlay border border-cohesity-border rounded-lg px-3 py-2 text-sm text-ink focus:border-brand/60 outline-none';
