...
import { Gauge, Boxes, Server, ShieldAlert, AlertTriangle } from 'lucide-react';
...
import { BRAND, fmtNum, fmtWhen, asDate } from './helpers';
...
export default function AriaOpsOverviewPage() {
  ...
  const load = useCallback(() => Promise.all([
    client.get('/ariaops/overview').then(({ data }) => setData(data)),
    client.get('/ariaops/metrics-history', { params: { hours: 168 } }).then(({ data }) => setHistory(data)).catch(() => setHistory([])),
  ]).then(() => setLastRefreshed(new Date()))
    .catch(() => { setData({ instances: [], totals: {} }); toast({ type: 'error', title: 'Failed to load Aria Operations overview' }); }), [toast]);
    ...
    <div className="animate-fade-in">
      ...
      {data != null && instances.length === 0 && (
        <div className="panel p-4 mb-4 text-sm text-ink-muted">
          ...
          <Link to="/ariaops/settings" className="text-brand hover:underline">Settings</Link>.
        </div>
      )}
