...
//
// DEVIATION FROM THE BUILT-IN: axios -> node https (same rationale as
// netbackupApi.js); decrypt/logger now come from a `coreApi` argument threaded
// through every exported fetcher instead of direct host requires.
...
function creds(conn, coreApi) {
  ...
  if (conn.encrypted_credentials) return JSON.parse(coreApi.encryption.decrypt(conn.encrypted_credentials));
  return {};
}
...
/** Minimal JSON https client, mirroring netbackupApi.js's rawRequest. */
function rawRequest(conn, method, path, { data, headers = {}, timeout = 30000 } = {}) {
    ...
    const req = https.request(
      {
        ...
        timeout,
        rejectUnauthorized: !!conn.sslVerify,
        headers: reqHeaders,
      },
      (res) => {
        let raw = '';
        ...
        res.on('end', () => {
          ...
          const e = new Error((json && json.message) || `HTTP ${status}`);
...
async function login(conn, coreApi, force = false) {
  const key = connKey(conn);
  const cached = sessions.get(key);
  if (!force && cached && Date.now() - cached.fetchedAt < SESSION_TTL_MS) return cached.token;
  const { password } = creds(conn, coreApi);
  const { data } = await rawRequest(conn, 'post', '/api/appliance/v1.0/auth/login', {
    data: { userName: conn.username, password },
  });
 ...
 */
async function apiRequest(rawConn, coreApi, method, path, opts = {}) {
  const conn = normConn(rawConn);
  const key = connKey(conn);
  const { password } = creds(conn, coreApi);

  if (basicFallback.has(key)) {
    return rawRequest(conn, method, path, {
      data: opts.data,
      headers: { Authorization: `Basic ${Buffer.from(`${conn.username}:${password}`).toString('base64')}` },
    });
  }

  let token;
  try {
    token = await login(conn, coreApi);
  } catch (err) {
  ...
  try {
    return await rawRequest(conn, method, path, { data: opts.data, headers: { Authorization: `Bearer ${token}` } });
  } catch (err) {
    if (err.response?.status === 401) {
      token = await login(conn, coreApi, true);
      return rawRequest(conn, method, path, { data: opts.data, headers: { Authorization: `Bearer ${token}` } });
    }
    throw err;
  }
}
...
async function fetchHardware(rawConn, coreApi) {
  const conn = normConn(rawConn);
  for (const path of CANDIDATE_PATHS) {
    try {
      const resp = await apiRequest(conn, coreApi, 'get', path);
  ...
  coreApi.logger.warn(`[netbackupApplianceApi] hardware fetch failed for all candidate paths on ${conn.name || conn.host}`);
...
async function fetchProbe(rawConn, coreApi) {
  const conn = normConn(rawConn);
  const out = [];
  for (const path of CANDIDATE_PATHS) {
    try {
