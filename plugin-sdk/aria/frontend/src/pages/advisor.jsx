// Aria AI Advisor — ported from the built-in AriaAdvisorPage.jsx, which
...
// sandbox). Reimplemented self-contained here (mirrors
// plugin-sdk/dell/frontend/src/pages/advisor.jsx): same tabs, same
// endpoints (GET/POST /api/aria/advisor/:slug), same cached-report/stale/
// run-new behavior, just apiFetch + inline styles instead of axios +
// Tailwind component imports.
import { Sparkles, FileText, Clock, Activity, ClipboardCheck, MonitorSmartphone } from '../icons.jsx';
...
function renderInline(text, keyPrefix) {
  ...
  return parts.map((p, i) => {
    if (p.startsWith('**') && p.endsWith('**')) {
      return <strong key={`${keyPrefix}-${i}`} style={{ color: 'var(--ar-ink)', fontWeight: 600 }}>{p.slice(2, -2)}</strong>;
    }
    if (p.startsWith('`') && p.endsWith('`')) {
      return <code key={`${keyPrefix}-${i}`} style={{ color: 'var(--ar-brand)', background: 'rgba(30,42,54,0.6)', borderRadius: 4, padding: '1px 4px', fontSize: 11 }}>{p.slice(1, -1)}</code>;
    }
...
function Markdown({ text }) {
  ...
  return (
    <div style={{ fontSize: 12, color: 'var(--ar-ink-muted)', lineHeight: 1.6 }}>
      {blocks.map((b, idx) => {
        if (b.type === 'h') {
          return (
            <p key={idx} style={{ fontSize: 13, fontWeight: 700, color: 'var(--ar-ink)', marginTop: idx === 0 ? 0 : 16, marginBottom: 6, paddingBottom: 4, borderBottom: '1px solid rgba(31,43,55,0.5)' }}>
  ...
  return (
    <div className="panel" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16, minHeight: 200 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div style={{ display: 'flex', height: 40, width: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 12, background: 'rgba(0,162,199,0.1)', border: '1px solid rgba(0,162,199,0.2)', flexShrink: 0 }}>
          <Icon size={20} style={{ color: BRAND }} />
        </div>
        <div style={{ minWidth: 0 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: 'var(--ar-ink)', margin: 0 }}>{tab.label}</p>
          <p style={{ fontSize: 12, color: 'var(--ar-ink-muted)', lineHeight: 1.5, margin: '2px 0 0' }}>{tab.blurb}</p>
        </div>
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, flexWrap: 'wrap', marginTop: 'auto' }}>
        {!loaded ? (
          <span style={{ color: 'var(--ar-ink-faint)' }}>Loading…</span>
        ) : !enabled ? (
          <span style={{ color: 'var(--ar-warn)' }}>AI not configured</span>
        ) : report?.generatedAt ? (
          <>
            <Clock size={12} style={{ color: 'var(--ar-ink-faint)' }} />
            <span style={{ color: 'var(--ar-ink-muted)' }}>Last run {timeAgoShort(report.generatedAt)}</span>
            {report.stale && (
              <span style={{ color: 'var(--ar-warn)', border: '1px solid rgba(251,191,36,0.4)', background: 'rgba(251,191,36,0.1)', borderRadius: 4, padding: '1px 4px', fontSize: 10, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '.03em' }}>Stale</span>
            )}
            {report.model && <span style={{ color: 'var(--ar-ink-faint)' }}>· {report.model}</span>}
          </>
        ) : (
          <span style={{ color: 'var(--ar-ink-faint)' }}>Not run yet</span>
        )}
      </div>

      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <button onClick={onOpen} disabled={!report?.generatedAt} className="ar-btn-ghost" style={{ opacity: report?.generatedAt ? 1 : 0.4, cursor: report?.generatedAt ? 'pointer' : 'not-allowed' }}>
          <FileText size={13} /> Open last run
        </button>
        <button onClick={onRun} disabled={!enabled}
          style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, padding: '8px 12px', background: 'rgba(0,162,199,0.1)', border: '1px solid rgba(0,162,199,0.3)', color: BRAND, borderRadius: 8, cursor: enabled ? 'pointer' : 'default', opacity: enabled ? 1 : 0.5 }}>
...
function AdvisorReportModal({ tab, initialReport, enabled, autoRun = false, onClose, onUpdated }) {
  ...
  const run = React.useCallback(async () => {
    ...
    setError(null);
    try {
      const json = await apiFetch(`/aria/advisor/${tab.slug}`, { method: 'POST', body: {} });
      setReport(json);
      onUpdated?.(json);
    } catch (e) {
  ...
  return (
    <Modal title={tab.label} icon={Icon} onClose={onClose} maxWidth="min(768px,92vw)">
      {report?.generatedAt && (
        <p style={{ fontSize: 11, color: 'var(--ar-ink-faint)', display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', marginTop: -8, marginBottom: 12 }}>
          ...
          {report.stale && (
            <span title={`Older than ${report.ttlHours || 24}h — re-run for current data`}
              style={{ color: 'var(--ar-warn)', border: '1px solid rgba(251,191,36,0.4)', background: 'rgba(251,191,36,0.1)', borderRadius: 4, padding: '1px 4px', fontSize: 10, fontWeight: 600, textTransform: 'uppercase' }}>
              Stale
            </span>
          )}
        </p>
      ...
      {!enabled ? (
        <p style={{ fontSize: 12, color: 'var(--ar-ink-muted)', lineHeight: 1.6 }}>
        ...
        <>
          {error && <p style={{ color: 'var(--ar-crit)', fontSize: 12, marginBottom: 8 }}>{error}</p>}
          {report.stale && (
            <p style={{ marginBottom: 12, fontSize: 11, color: 'var(--ar-warn)', background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: 6, padding: '6px 10px' }}>
        ...
        <p style={{ color: 'var(--ar-ink-muted)', fontSize: 12, padding: '40px 0', textAlign: 'center' }}>
          {error ? <span style={{ color: 'var(--ar-crit)' }}>{error}</span> : 'No report yet. Run one to analyze the estate.'}
        </p>
      )}

      {enabled && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16, paddingTop: 12, borderTop: '1px solid var(--ar-border)' }}>
          <button onClick={run} disabled={running}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, padding: '7px 12px', background: 'rgba(0,162,199,0.1)', border: '1px solid rgba(0,162,199,0.3)', color: BRAND, borderRadius: 8, cursor: running ? 'default' : 'pointer', opacity: running ? 0.5 : 1 }}>
            <Sparkles size={13} />
...
export default function AriaAdvisorPage() {
  ...
  const loadOne = React.useCallback(async (slug) => {
    try {
      const json = await apiFetch(`/aria/advisor/${slug}`);
