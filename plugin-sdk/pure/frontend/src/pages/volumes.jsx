// Ported from frontend/src/pages/pure/PureVolumesPage.jsx.
import { Layers, Activity, Timer, Gauge } from '../icons.jsx';
...
import {
  ...
  BRAND, fmtBytes, fmtNum, fmtDateMs,
} from '../ui.jsx';
...
export default function PureVolumesPage() {
  ...
  const [vols, setVols] = React.useState(null);
  ...
  const load = React.useCallback(() => {
    ...
    return Promise.allSettled([
      apiFetch(`/pure/pure1/volumes?arrayId=${arrayId}`),
      apiFetch(`/pure/pure1/performance/history?arrayId=${arrayId}&days=${days}`),
    ]).then(([v, p]) => {
      setVols(v.status === 'fulfilled' ? v.value : []);
      setPerf(p.status === 'fulfilled' ? p.value : { series: {} });
      if (v.status === 'fulfilled') setLastRefreshed(new Date());
    }).finally(() => setLoading(false));
  }, [arrayId, days]);

  React.useEffect(() => { load(); }, [load]);

  const totalProvisioned = React.useMemo(() => (vols || []).reduce((s, v) => s + (v.provisioned || 0), 0), [vols]);
  const list = vols || [];
  ...
  const labels = React.useMemo(() => (s.array_read_iops || []).map(([ts]) =>
    ...
    <div className="animate-fade-in">
      <PageHeader icon={Layers} title="Pure Volumes" description="Volumes and live array performance from Pure Storage">
        <div className="flex items-center gap-2">
          ...
          <select value={days} onChange={(e) => setDays(Number(e.target.value))} className="pu-input" style={{ width: 'auto' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <StatCard icon={Timer} label="Latency (r/w)" value={`${(perfLast('array_read_latency_us') != null ? (perfLast('array_read_latency_us') / 1000).toFixed(2) + ' ms' : '—')} / ${(perfLast('array_write_latency_us') != null ? (perfLast('array_write_latency_us') / 1000).toFixed(2) + ' ms' : '—')}`} />
      </div>
