...
// This branch's registry has no markBuiltin/isBuiltinPresent (icc-phase1);
// platformGateOk's typeof guard already falls back to always-on for
// cohesity when isBuiltinPresent doesn't exist, so no setup call is needed.
...
function insertBrocadeIssue({ sourceId = 1, source = 'switch1', type, target = 'port1', severity = 'critical', message = 'brocade issue' }) {
  db.prepare(`
    INSERT INTO brocade_issue_history (source_id, source, type, target, severity, message, first_seen, last_seen)
    VALUES (?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
  `).run(sourceId, source, type, target, severity, message);
}
...
beforeEach(() => {
  ...
  db.exec('DELETE FROM brocade_issue_history');
});
...
describe('brocade issue types (a non-Cohesity platform with a real type column)', () => {
  it('a brocade issue type is cataloged from its type column and can be muted', async () => {
    insertBrocadeIssue({ type: 'PortDown', target: 'port1', severity: 'critical', message: 'port down' });
    configureSmtp();

    await alertNotifier.run();
    expect(sent).toHaveLength(1);
    expect(typeRow('brocade', 'PortDown')).toMatchObject({ enabled: 1, label: 'PortDown' });

    db.exec('DELETE FROM alert_notifications');
    sent.length = 0;
    muteType('brocade', 'PortDown');
    await alertNotifier.run();
    expect(sent).toHaveLength(0);
    expect(typeRow('brocade', 'PortDown').enabled).toBe(0);
  });
});
