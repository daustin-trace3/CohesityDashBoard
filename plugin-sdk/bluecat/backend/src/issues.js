// Computed BlueCat issues (shared by router.js and poller.js) plus their
// lifecycle history - ported VERBATIM from backend/services/bluecatIssues.js.
// Issue identity is `type|source|target`. db/getSetting now come from
// coreApi (thread it through) instead of direct host requires.
...
const lowFreeWarn = (coreApi) => clampedInt(coreApi, 'bluecat_low_free_warn', 20, 1, 10000);
const lowFreePct = (coreApi) => clampedInt(coreApi, 'bluecat_low_free_pct', 10, 1, 90);
...
function computeIssues(coreApi) {
  ...
  const warnCount = lowFreeWarn(coreApi);
  const warnPct = lowFreePct(coreApi);
...
function reconcileIssueHistory(coreApi) {
  ...
  // BEGIN IMMEDIATE, same rationale as the built-in's unifiIssues.js model.
