// IP Spaces - ported from the built-in BluecatIpSpacesPage.jsx (blocks tree
// + networks table + NetworkDetailModal deep link).
import { Network, Box, AlertTriangle } from '../icons.jsx';
import client from '../api.js';
import {
  useToast, PageHeader, Badge, LoadingPanel, ErrorPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TableSearch, TablePager,
  BRAND, fmtNum, fmtPct, fmtWhen, gatewaySourceLabel, gatewaySourceTone, freeStaticTone,
} from '../ui.jsx';
import NetworkDetailModal from './networkDetailModal.jsx';
...
export default function BluecatIpSpacesPage() {
  ...
  const [networksFailed, setNetworksFailed] = React.useState(false);
  const [selectedBlock, setSelectedBlock] = React.useState(null);
  const [ipVersion, setIpVersion] = React.useState('');
  const [lowSpaceOnly, setLowSpaceOnly] = React.useState(false);
  ...
  const loadBlocks = React.useCallback(() => client.get('/bluecat/blocks')
  ...
  const loadNetworks = React.useCallback(() => {
    ...
    return client.get('/bluecat/networks', { params })
      ...
      .catch(() => { setNetworks(null); setNetworksFailed(true); toast({ type: 'error', title: 'Failed to load networks' }); });
  }, [selectedBlock, ipVersion, lowSpaceOnly, toast]);

  React.useEffect(() => { loadBlocks(); }, [loadBlocks]);
  React.useEffect(() => { loadNetworks(); }, [loadNetworks]);

  const byParent = React.useMemo(() => buildTree(blocks || []), [blocks]);
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0 panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {networksFailed ? (
            <ErrorPanel label="Failed to load networks." onRetry={loadNetworks} height={200} />
          ) : networks == null ? (
