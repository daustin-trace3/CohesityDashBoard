import { ScrollText, Search } from '../icons.jsx';
import {
...
export default function DellHardwareLogsPage() {
  ...
  const load = React.useCallback((params) => {
    ...
    return apiFetch(`/dell/hardware-logs${qs ? `?${qs}` : ''}`)
      .then((json) => {
        setData({ rows: Array.isArray(json?.rows) ? json.rows : [], total: json?.total || 0 });
    ...
    <div className="animate-fade-in">
      ...
      <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <div className="relative flex-1 min-w-[220px] max-w-md" style={{ position: 'relative' }}>
            <Search size={13} className="text-ink-faint" style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)' }} />
            <input
              value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Search device name, service tag, message, event code…"
              className="dl-input" style={{ paddingLeft: 32 }} />
          </div>
          <select value={severity} onChange={(e) => setSeverity(e.target.value)} className={selectCls} style={{ width: 'auto', cursor: 'pointer' }} aria-label="Severity">
            <option value="">All severities</option>
            {SEVERITIES.map((s) => <option key={s} value={s}>{s[0].toUpperCase() + s.slice(1)}</option>)}
          </select>
          <select value={days} onChange={(e) => setDays(e.target.value ? Number(e.target.value) : '')} className={selectCls} style={{ width: 'auto', cursor: 'pointer' }} aria-label="Timeframe">
            {TIMEFRAMES.map((t) => <option key={String(t.value)} value={t.value}>{t.label}</option>)}
          </select>
          {data && (
            <p className="text-[11px] text-ink-faint tnum" style={{ marginLeft: 'auto' }}>
