// Veritas NetBackup plugin chart kit — renders via the host's Chart.js
// instance (window.Chart, registered + exposed by frontend/src/main.jsx; see
// plugin-sdk/README.md "Charts"). Cloned from the Proxmox plugin's
// charts.jsx (same component names/props), brand color swapped to Veritas
// red (#B1181E).
...
// `xLabels` (optional) supplies category labels (dates) for the x axis —
// used by pages that plot day-keyed trends rather than index-keyed ones.
export function LineChart({ series = [], refLines = [], xLabels, width = 640, height = 220, yFmt, tooltip = true }) {
  const maxN = Math.max(1, ...series.map((s) => (s.points || []).length), (xLabels || []).length);
  const labels = React.useMemo(() => xLabels && xLabels.length ? xLabels : Array.from({ length: maxN }, (_, i) => i), [maxN, xLabels]);
  ...
  const options = React.useMemo(() => ({
    ...
    scales: {
      x: { display: !!xLabels, ticks: { color: TICK, font: { size: 9 }, maxTicksLimit: 10 }, grid: { display: false } },
      y: {
  ...
  }), [series.length, tooltip, yFmt, xLabels]);
...
/* ── BarChart ────────────────────────────────────────────────────────── */
// series: single dataset bar chart. rows: [{label,value,color}]. Optional
// `horizontal` for indexAxis:'y' layout (used interchangeably with HBar).
export function BarChart({ labels = [], values = [], colors, height = 220, yFmt, horizontal = false }) {
  const data = React.useMemo(() => ({
    labels,
    datasets: [{
      data: values,
  ...
  const options = React.useMemo(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        backgroundColor: TOOLTIP_BG, borderColor: TOOLTIP_BORDER, borderWidth: 1,
        titleColor: INK, bodyColor: INK_MUTED, padding: 10,
        callbacks: { label: (item) => (yFmt ? yFmt(horizontal ? item.parsed.x : item.parsed.y) : `${horizontal ? item.parsed.x : item.parsed.y}`) },
      },
    },
    scales: {
      x: { ticks: { color: TICK, font: { size: 10 }, callback: (v) => (horizontal && yFmt ? yFmt(v) : v) }, grid: { color: GRID } },
      y: { ticks: { color: TICK, font: { size: 10 }, callback: (v) => (!horizontal && yFmt ? yFmt(v) : v) }, grid: { display: horizontal ? false : true, color: GRID } },
    },
  }), [horizontal, yFmt]);
...
/* ── Donut ───────────────────────────────────────────────────────────── */
export function Donut({ pct, size = 74, stroke = 9, thresholds, centerLabel, centerSub, colorOverride }) {
  const clamped = Math.max(0, Math.min(100, pct || 0));
  let color = colorOverride || BRAND;
  if (!colorOverride && thresholds) {
...
/* ── Doughnut (multi-slice, legend) — used for state/storage breakdowns */
export function DoughnutChart({ labels = [], values = [], colors, height = 208, legend = true }) {
  const data = React.useMemo(() => ({
    labels,
    datasets: [{ data: values, backgroundColor: colors || [BRAND], borderWidth: 0 }],
  }), [labels, values, colors]);
  ...
  const options = React.useMemo(() => ({
    ...
    animation: false,
    plugins: {
      legend: legend ? { position: 'right', labels: { color: TICK, boxWidth: 10, boxHeight: 10, font: { size: 11 } } } : { display: false },
      tooltip: {
        backgroundColor: TOOLTIP_BG, borderColor: TOOLTIP_BORDER, borderWidth: 1,
        titleColor: INK, bodyColor: INK_MUTED, padding: 10,
      },
  ...
  const canvasRef = useChartJs('doughnut', data, options);
  return (
