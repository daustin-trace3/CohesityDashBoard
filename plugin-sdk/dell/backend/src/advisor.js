// Dell AI Advisor: hardware health/lifecycle-compliance/alert-triage/power-
// thermal/config-drift/job-health/hardware-log-forensics/capacity-
// consolidation/support-case-prep fleet reports, plus the scoped device_360
// per-server report. Ported from backend/services/advisors/dellAdvisor.js.
...
// createDellAdvisor(coreApi) — built lazily by router.js once coreApi is
// known (nutanix advisor.js pattern). Per the plugin contract, coreApi.advisor
// is the host's services/platformAdvisor module (createPlatformAdvisor/
// linReg/parseUtcMs/fmtBytes), never required directly. vcenterHostUtilization/
// shortHost come from the pack's own ./vcenterUtil (identical to the built-in's
// services/dellVcenterUtil) rather than a host require.
const { vcenterHostUtilization, shortHost } = require('./vcenterUtil');
...
function createDellAdvisor(coreApi) {
  ...
  function gatherDevice360({ scope } = {}) {
    ...
    // vcenter_hosts row using the same exact/short-name rule as vcenterUtil.js.
