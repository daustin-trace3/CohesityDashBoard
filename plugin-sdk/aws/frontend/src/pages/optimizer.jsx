// Ported from frontend/src/pages/aws/AwsOptimizerPage.jsx.
import { Wrench, DollarSign, ListChecks, BadgeCheck } from '../icons.jsx';
import {
  apiFetch, PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TableControls, TablePager, BRAND, fmtUsd,
} from '../ui.jsx';
...
export default function AwsOptimizerPage() {
  ...
  const load = React.useCallback(() => apiFetch('/aws/optimizer')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ totals: { count: 0, estMonthlySavingsUsd: 0, coEnrollment: null, lastCapture: null }, recommendations: [] })), []);

  React.useEffect(() => { load(); }, [load]);

  const flash = (type, msg) => { setStatusMsg({ type, msg }); setTimeout(() => setStatusMsg((s) => (s?.msg === msg ? null : s)), 5000); };

  const onRefresh = async () => {
    setRefreshing(true);
    try {
      await apiFetch('/aws/optimizer/refresh', { method: 'POST', body: {} });
      flash('success', 'Re-capture queued — refresh in a few minutes');
    } catch {
      flash('error', 'Failed to queue re-capture');
    } finally {
      setRefreshing(false);
    }
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      {statusMsg && (
        <div className={`panel p-3 mb-4 ${statusMsg.type === 'error' ? 'border border-status-crit/50' : 'border border-status-ok'}`}>
          <p className={`text-xs ${statusMsg.type === 'error' ? 'text-status-crit' : 'text-status-ok'}`}>{statusMsg.msg}</p>
        </div>
      )}
