...
// email dedupe keys.
//
// Copied VERBATIM from backend/db/migrations/aria.js, same scope id 'aria' —
// schema_migrations already has scope='aria' rows on an existing install, so
// these are skipped there and existing production data is adopted intact.
module.exports = [
  {
    version: 1,
    up(db) {
  ...
  // Migration: cached AI Advisor report content, one row per report key.
  {
    ...
    up(db) {
      db.exec(`
        CREATE TABLE IF NOT EXISTS aria_ai_reports (
