// Guest storage reads for the vCenter Guest Storage page, Server 360 and the
// CSV report: guest filesystems (VMware Tools guest.disk) and virtual disks,
// with the owner tag resolved per vCenter and the threshold state applied.
// Filtering, sorting and paging happen here so the browser gets one page.
const db = require('../db/database');
const { guestDiskThresholds, ownerFromTags } = require('./vcenterIssues');
...
// Earliest history row per filesystem inside the last 30 days: the growth
// baseline. Keyed "vcenterId|vmId|mount".
function growthBaselines(vcenterId) {
...
function baseFilesystemRows(vcenterId) {
  return db.prepare(`
...
/** Whole-estate (or one vCenter) summary for the tiles, independent of the
 *  page's state/search filters. */
function filesystemSummary(vcenterId, shaped) {
  const t = guestDiskThresholds();
...
/** Guest filesystems, filtered, sorted and paged, with a whole-set summary. */
function listFilesystems(opts = {}) {
  const t = guestDiskThresholds();
  const vcenterId = opts.vcenterId ? Number(opts.vcenterId) : null;
  const baselines = growthBaselines(vcenterId);
  const shaped = baseFilesystemRows(vcenterId).map(r => shapeFilesystem(r, t, baselines));
  ...
  return { summary: filesystemSummary(vcenterId, shaped), ...paged };
}

/** Virtual disks (provisioned versus consumed on the datastore), paged. */
...
/** Everything stored for one VM: its filesystems and virtual disks. */
function vmStorage(vcenterId, vmId) {
  const t = guestDiskThresholds();
  const baselines = growthBaselines(vcenterId);
  const filesystems = baseFilesystemRows(vcenterId).filter(r => r.vm_id === vmId).map(r => shapeFilesystem(r, t, baselines));
...
/** CSV text of the (unpaged) filesystem rows for the report download. */
function filesystemsCsv(opts = {}) {
  const { rows } = listFilesystems({ ...opts, pageSize: 'all', page: 0 });
  const esc = (v) => {
...
/** Distinct tag categories seen on VMs (per vCenter or everywhere), for the
 *  owner-category picker. */
function tagCategories(vcenterId) {
  const rows = db.prepare(`
