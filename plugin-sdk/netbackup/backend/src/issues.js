...
//
// Ported from backend/services/netbackupIssues.js — db/getSetting now come
// from coreApi rather than direct host requires (proxmoxIssues.js pattern).
function successWarnPct(coreApi) {
  const n = Number(coreApi.settings.getSetting('netbackup_success_warn_pct'));
  return Number.isFinite(n) && n >= 50 && n <= 100 ? Math.round(n) : 90;
}

function storageWarnPct(coreApi) {
  const n = Number(coreApi.settings.getSetting('netbackup_storage_warn_pct'));
  return Number.isFinite(n) && n >= 5 && n <= 50 ? Math.round(n) : 20;
}

function staleBackupHours(coreApi) {
  const n = Number(coreApi.settings.getSetting('netbackup_stale_backup_hours'));
...
function computeIssues(coreApi) {
  ...
  const successWarn = successWarnPct(coreApi);
  ...
  const storageWarn = storageWarnPct(coreApi);
...
function reconcileIssueHistory(coreApi) {
  ...
  const run = db.transaction(() => {
    const current = new Map(computeIssues(coreApi).map((i) => [i.issue_key, i]));
