// BlueCat plugin fetch wrapper - axios-shaped (client.get(path, { params })
// resolving to { data }, errors carrying err.response.data.error) so the
// host frontend/src/pages/bluecat/*.jsx pages (built against
// frontend/src/api/client.js, an axios instance) port with an import swap
// and no call-site rewrites. Ported from plugin-sdk/brocade/frontend/src/api.js.
// Fetch with credentials:'same-origin', no x-api-key; every mutating call
// spreads the CSRF header from window.__ICC_CSRF_TOKEN__. Paths are
// host-relative ('/bluecat/...') and get the '/api' prefix here, exactly
// like the host's axios instance baseURL.
