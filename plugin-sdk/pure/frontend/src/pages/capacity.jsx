// Ported from frontend/src/pages/pure/PureCapacityPage.jsx.
import { Database, HardDrive, Gauge } from '../icons.jsx';
import { TrendChart } from '../charts.jsx';
import { apiFetch, PageHeader, StatCard, LoadingPanel, RefreshButton, LastUpdated, BRAND, fmtBytes, fmtRatio } from '../ui.jsx';
...
export default function PureCapacityPage() {
  ...
  const load = React.useCallback(() => {
    ...
    setLoading(true);
    return apiFetch(`/pure/pure1/capacity/history?arrayId=${arrayId}&days=${days}`)
      .then((data) => { setHist(data); setLastRefreshed(new Date()); })
      .catch(() => setHist({ series: {} }))
  ...
  const labels = React.useMemo(() => rows.map((r) => new Date(r.ts).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })), [rows]);
