...
// minutes of the stated expiry.
//
// DEVIATION FROM THE BUILT-IN: a bundled plugin cannot require the host's
// axios (no npm deps in the sandbox - see the plugin conversion skill), so
// this is rewritten onto node's built-in https module (brocade/dell api.js
// rawRequest pattern). Every exported function keeps its built-in name and
// return shape; every function that talks to BAM now also takes `coreApi`
// as its second argument (needed for coreApi.encryption.decrypt) - the one
// unavoidable signature change, threaded through by poller.js/router.js/
// advisor.js. timeout/rejectUnauthorized/JSON parsing/status-carrying error
// objects match the axios behavior this replaces: statuses 200-499 resolve
// (callers decide what counts as an error), statuses >=500 reject with an
// Error carrying `.response = { status, data }` (axios's validateStatus:
// s => s >= 200 && s < 500 threw for 500+ the same way).
...
// -- Low-level https transport (axios replacement) ---------------------------
...
/** One HTTPS call against `/api/v2<path>` (path already includes its own
 *  leading slash, matching every call site below). Resolves {status, data}
 *  for any status < 500 (axios's default validateStatus range); rejects
 *  with an Error carrying `.response` for a >=500 or a transport failure. */
function rawRequest(source, method, path, { params, data, timeout = 30000, headers = {} } = {}) {
  return new Promise((resolve, reject) => {
    const port = source.port || 443;
    const qs = buildQuery(params);
    const fullPath = `/api/v2${path}${qs}`;
    const bodyStr = data !== undefined ? JSON.stringify(data) : null;
    const reqHeaders = Object.assign({ Accept: 'application/hal+json' }, headers);
    if (bodyStr != null) {
    ...
    try {
      req = https.request({
        ...
        headers: reqHeaders,
        rejectUnauthorized: !!source.ssl_verify,
        timeout,
      }, (res) => {
        const chunks = [];
    ...
    req.on('timeout', () => { req.destroy(new Error('BlueCat request timed out')); });
...
async function login(source, coreApi) {
  ...
  const res = await rawRequest(source, 'post', '/sessions', { data: { username, password }, timeout: 30000 });
  if (res.status >= 400) {
...
async function getSession(source, coreApi, force = false) {
  ...
  const session = await login(source, coreApi);
...
async function request(source, coreApi, method, path, { params, data, timeout = 30000, retriedAuth = false } = {}) {
  const session = await getSession(source, coreApi);
  const res = await rawRequest(source, method, path, {
    params, data, timeout,
    headers: { Authorization: `Basic ${session.basicAuthCredentials}`, Accept: 'application/hal+json' },
  });
  if (res.status === 401 && !retriedAuth) {
    ...
    if (code === 'InvalidAuthorizationToken' || code == null) {
      await getSession(source, coreApi, true);
      return request(source, coreApi, method, path, { params, data, timeout, retriedAuth: true });
    }
  }
  if (res.status >= 400) {
    const err = new Error(errMsg({ response: res }));
...
const apiGet = (source, coreApi, path, opts) => request(source, coreApi, 'get', path, opts);

/** GET with a `fields` param, retrying once WITHOUT fields on a 400 (a field
 *  name may not exist on this BAM version). */
async function getWithFieldsFallback(source, coreApi, path, params, timeout) {
  try {
    return await apiGet(source, coreApi, path, { params, timeout });
  } catch (err) {
    if (err.response?.status === 400 && params?.fields) {
      coreApi.logger.warn(`[BluecatApi] ${path} 400'd with fields=${params.fields}, retrying without fields`);
      const { fields, ...rest } = params;
      return apiGet(source, coreApi, path, { params: rest, timeout });
    }
    throw err;
  }
}
...
async function pagedGet(source, coreApi, path, { filter, fields, limit = 1000, timeout = 30000, cap = MAX_PAGE_ROWS } = {}) {
  const out = [];
  let offset = 0;
  for (;;) {
    ...
    const d = fields
      ? await getWithFieldsFallback(source, coreApi, path, params, timeout)
      : await apiGet(source, coreApi, path, { params, timeout });
    const rows = safeArr(d?.data);
    out.push(...rows);
    const count = numOrNull(d?.count) ?? rows.length;
    offset += rows.length;
    if (!rows.length || count < limit || out.length >= cap) {
      if (out.length >= cap) coreApi.logger.warn(`[BluecatApi] ${path} capped at ${cap} rows`);
      break;
    }
  }
  return out;
...
async function fetchVersion(source, coreApi, timeout) {
  try {
    const d = await apiGet(source, coreApi, '/settings', { params: { filter: "type:'SystemSettings'" }, timeout });
...
async function fetchConfigurations(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/configurations', { fields: 'id,name', timeout });
  return rows.map((c) => ({ id: numOrNull(c.id), name: strOrNull(c.name) }));
}

async function fetchViews(source, coreApi, configId, timeout) {
  const rows = await pagedGet(source, coreApi, `/configurations/${configId}/views`, { timeout });
  return rows.map((v) => ({
    id: numOrNull(v.id),
...
async function fetchZones(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/zones', { limit: 1000, timeout });
...
async function fetchResourceRecords(source, coreApi, timeout) {
  ...
  const rows = await pagedGet(source, coreApi, '/resourceRecords', { fields, limit: 1000, timeout });
  return rows.map(parseRecord);
}

/** searchRecords ladder (router.js /records/lookup): filter search ->
 *  root search -> address lookup -> failure. Never throws - returns
 *  {ok:false, error}. */
async function searchRecords(source, coreApi, q, timeout = 15000) {
  const escaped = String(q).replace(/'/g, "''");
  try {
    const d = await apiGet(source, coreApi, '/resourceRecords', {
  ...
  try {
    const d = await apiGet(source, coreApi, '/api/v2', { params: { filter: `name:contains('${escaped}')`, limit: 200 }, timeout });
    return { ok: true, method: 'root', results: safeArr(d?.data) };
  } catch { /* fall through */ }
  if (/^\d{1,3}(\.\d{1,3}){3}$/.test(String(q))) {
    try {
      const addrs = await apiGet(source, coreApi, '/addresses', { params: { filter: `address:eq('${escaped}')` }, timeout });
      const addr = safeArr(addrs?.data)[0];
      if (addr?.id) {
        const recs = await apiGet(source, coreApi, `/addresses/${addr.id}/resourceRecords`, { timeout });
...
async function fetchBlocks(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/blocks', { limit: 1000, timeout });
...
async function fetchNetworks(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/networks', { limit: 1000, timeout });
...
async function probeNetworkUsage(source, coreApi, timeout) {
  try {
    const d = await getWithFieldsFallback(source, coreApi, '/networks', { fields: 'id,range,usage', limit: 1 }, timeout);
...
async function fetchRanges(source, coreApi, networkId, timeout) {
  const rows = await pagedGet(source, coreApi, `/networks/${networkId}/ranges`, { limit: 1000, timeout });
...
async function fetchAllRanges(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/ranges', { limit: 1000, timeout });
...
async function fetchNetworkAddresses(source, coreApi, networkId, prefix, timeout) {
  const fields = 'id,address,state,name,macAddress,device,_links';
  try {
    const filter = `state:in('${ADDRESS_STATES.join("','")}')`;
    const rows = await pagedGet(source, coreApi, `/networks/${networkId}/addresses`, { filter, fields, limit: 1000, timeout });
    return rows.map(parseAddress);
  } catch (err) {
    coreApi.logger.warn(`[BluecatApi] addresses state:in filter failed for network ${networkId}: ${errMsg(err)}`);
  }
  try {
    ...
    coreApi.logger.warn(`[BluecatApi] addresses state:ne filter failed for network ${networkId}: ${errMsg(err)}`);
  }
  if (prefix != null && prefix >= 22) {
    const rows = await pagedGet(source, coreApi, `/networks/${networkId}/addresses`, { fields, limit: 1000, timeout });
    return rows.map(parseAddress);
  }
...
async function sampleNetworkAddresses(source, coreApi, networkId, timeout, limit = 200) {
  try {
    const d = await apiGet(source, coreApi, `/networks/${networkId}/addresses`, { params: { limit, fields: 'id,address,state,name,_links' }, timeout });
    return safeArr(d?.data).map(parseAddress);
  } catch (err) {
    coreApi.logger.warn(`[BluecatApi] address sample failed for network ${networkId}: ${errMsg(err)}`);
    return null;
  }
}
...
async function fetchNetworkAddressesUnfiltered(source, coreApi, networkId, timeout) {
  const rows = await pagedGet(source, coreApi, `/networks/${networkId}/addresses`, { fields: 'id,address,state,name,macAddress,device,_links', limit: 1000, timeout });
  return rows.map(parseAddress);
}
...
async function fetchDevices(source, coreApi, configId, timeout) {
  try {
    ...
    const rows = await pagedGet(source, coreApi, `/configurations/${configId}/devices`, { fields, limit: 1000, timeout });
    return rows.map(parseDevice);
  } catch (err) {
    coreApi.logger.warn(`[BluecatApi] devices embed(addresses) failed for config ${configId}: ${errMsg(err)}`);
    const rows = await pagedGet(source, coreApi, `/configurations/${configId}/devices`, { limit: 1000, timeout });
    ...
    for (const dv of parsed.slice(0, 500)) {
      try {
        const addrs = await apiGet(source, coreApi, `/devices/${dv.id}/addresses`, { timeout });
...
async function fetchServers(source, coreApi, timeout) {
  try {
    ...
    const rows = await pagedGet(source, coreApi, '/servers', { fields, limit: 1000, timeout });
    return rows.map(parseServer);
  } catch (err) {
    coreApi.logger.warn(`[BluecatApi] servers embed(interfaces) failed: ${errMsg(err)}`);
    const rows = await pagedGet(source, coreApi, '/servers', { limit: 1000, timeout });
    return rows.map(parseServer);
  }
}
...
async function fetchDeploymentRoles(source, coreApi, timeout) {
  const rows = await pagedGet(source, coreApi, '/deploymentRoles', { limit: 1000, timeout });
  return rows.map(parseDeploymentRole);
}

async function fetchLatestDeployment(source, coreApi, serverId, timeout) {
  try {
    const d = await apiGet(source, coreApi, `/servers/${serverId}/deployments`, { params: { orderBy: 'desc(id)', limit: 1 }, timeout });
...
async function testConnection(candidate, coreApi) {
  try {
    ...
    const version = await fetchVersion(candidate, coreApi, 15000);
