// DNS - ported from the built-in BluecatDnsPage.jsx (views/zones tree +
// records table + live lookup).
import { Globe, Layers, Search } from '../icons.jsx';
import client from '../api.js';
import {
  useToast, PageHeader, Badge, LoadingPanel, ErrorPanel, RefreshButton, LastUpdated, Spinner,
...
export default function BluecatDnsPage() {
  ...
  const [rrType, setRrType] = React.useState('');
  const [records, setRecords] = React.useState(null);
  const [recordsFailed, setRecordsFailed] = React.useState(false);
  ...
  const loadViews = React.useCallback(() => client.get('/bluecat/views')
  ...
  const loadRecords = React.useCallback(() => {
    ...
    return client.get('/bluecat/records', { params })
      ...
      .catch(() => { setRecords(null); setRecordsFailed(true); setTotal(0); setLimited(false); toast({ type: 'error', title: 'Failed to load records' }); });
  ...
  const scopedZones = React.useMemo(() => zones || [], [zones]);
    ...
    <div className="animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0 panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
          ...
          {recordsFailed ? (
            <ErrorPanel label="Failed to load records." onRetry={loadRecords} height={200} />
          ) : records == null ? (
                    ...
                    {ctl.rows.map((r) => (
                      <tr key={r.id} className="border-b border-cohesity-border/50">
                        ...
                        <td className="py-2 pr-3 text-[12px]">
                          {r.network ? (
                            <ReactRouterDOM.Link to={`/bluecat/ipspaces?network=${r.network.id}`} className="text-brand hover:underline tnum" title={r.network.blockRange ? `Block ${r.network.blockRange}${r.network.blockName ? ` (${r.network.blockName})` : ''}` : undefined}>
