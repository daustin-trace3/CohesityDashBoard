// Ported from frontend/src/pages/aws/AwsEcsPage.jsx.
import { Container } from '../icons.jsx';
import {
...
export default function AwsEcsPage() {
  ...
  const load = React.useCallback(() => apiFetch('/aws/ecs')
    .then((json) => { setClusters(json?.clusters || []); setServices(json?.services || []); setLastRefreshed(new Date()); setError(null); })
    .catch(() => { setClusters([]); setServices([]); setError('Failed to load ECS data'); }), []);
