...
import {
  ShieldCheck, HardDrive, Cpu, GitCommitVertical, Layers, X,
} from 'lucide-react';
import client from '../../api/client';
import { useToast } from '../../components/ui/Toaster';
import { PageHeader, StatCard, Badge, LoadingPanel, RefreshButton, LastUpdated } from '../../components/ui/primitives';
import { useTableControls, SortTh, TablePager, CsvExportButton } from '../../components/ui/tableTools';
import { BRAND, fmtBytes, fmtNum, statusTone, timeAgo } from './helpers';
...
function FilterChip({ label, onClear }) {
  return (
    <span className="chip bg-brand/10 text-brand border-brand/25">
      {label}
      <button onClick={onClear} aria-label={`Clear ${label}`} className="ml-1 text-brand/70 hover:text-brand cursor-pointer">
        <X size={10} />
      </button>
    </span>
  );
...
function ClusterDetailModal({ cluster, nodes, onClose }) {
  return (
    <ModalShell title={cluster.name} subtitle={`${cluster.mgmt_host} | ${cluster.source}`} onClose={onClose}>
      ...
      <Link to="/netapp/hardware" className="text-xs text-brand hover:underline">View in NetApp Hardware &rsaquo;</Link>
    </ModalShell>
  );
}

export default function NetAppGovernancePage() {
  ...
  }, [section, filters]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // (routes/netapp.js), computed from the canonical parsed version - never
  // recomputed here from the raw stored strings, which can differ in format
  // (short "9.13.1P8" for AIQUM-sourced arrays vs the direct/node full
  // "NetApp Release 9.13.1P8: <date>" form) for the exact same release.
  const clusters = useMemo(() => rawClusters.map((c) => ({
    ...c,
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex gap-4 items-start">
        <div className="panel p-2 w-52 shrink-0" style={{ borderTop: `3px solid ${BRAND}` }}>
          {nav.map((n) => (
            <button
              key={n.id}
              type="button"
              ...
              {clusters.some((c) => c.mixed_versions) && (
                <div className="pt-3 border-t border-cohesity-border">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {clusters.filter((c) => c.mixed_versions).map((c) => (
                      <span key={c.id} className="chip bg-status-warn/10 text-status-warn border-status-warn/25">{c.name}</span>
                    ))}
                  </div>
                </div>
              )}
