...
/**
 * GET /api/insights
 * Computes prioritized, actionable insights across capacity, availability,
 * alerts, data protection, and replication. Pure SQLite reads — fast.
 */
router.get('/', (req, res, next) => {
  try {
    ...
    res.json({ generatedAt: new Date().toISOString(), summary, insights: insights.slice(0, 30) });
  } catch (err) {
    next(err);
  }
});
