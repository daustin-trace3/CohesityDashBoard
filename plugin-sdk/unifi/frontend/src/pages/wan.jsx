// UniFi WAN / ISP — port of frontend/src/pages/unifi/UnifiWanPage.jsx.
import { PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated, apiFetch, fmtWhen, secsToHuman, BRAND } from '../ui.jsx';
import { LineChart } from '../charts.jsx';
import { Globe, Activity, MousePointerClick, LayoutGrid, List } from '../icons.jsx';
...
export default function WanPage() {
  ...
  const [viewPref, setViewPref] = React.useState(null); // null = auto by count

  const load = React.useCallback(() => apiFetch('/unifi/wan')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ wans: [], history: [] })), []);
  ...
  const labels = React.useMemo(() => history.map((h) => fmtWhen(h.capturedAt)), [history]);
  const latencyChart = React.useMemo(() => ({
    labels,
  ...
  const availChart = React.useMemo(() => ({
    labels,
  ...
  const throughputChart = React.useMemo(() => ({
    labels,
    datasets: [
            ...
            <>
              ...
              <div className="grid lg:grid-cols-3 gap-4">
                <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                  ...
                  <LineChart data={latencyChart} options={chartOpts} height={176} />
                </div>
                <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                  <p className="text-sm font-semibold text-ink mb-2">Availability</p>
                  <LineChart data={availChart} options={chartOpts} height={176} />
                </div>
                <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                  <p className="text-sm font-semibold text-ink mb-2">Throughput</p>
                  <LineChart data={throughputChart} options={chartOpts} height={176} />
                </div>
              </div>
            </>
          ) : (
