...
// Compare dotted version strings (e.g. 6.5.12). >0 when a is newer.
function cmpVer(a, b) {
...
export default function Pure1FleetPage() {
  ...
  useEffect(() => { setPage(0); }, [q, sort, pageSize, tagFilter]);
  ...
  // Newest Purity release across the fleet (for version-drift flagging).
  ...
  // Linear-fit forecast of days-to-full from the 30-day used-capacity trend.
  const forecast = useMemo(() => {
    ...
    const slope = den ? num / den : 0; // bytes/day
          ...
          <div className="flex items-center gap-2">
            {allTags.length > 0 && (
              <select value={tagFilter} onChange={(e) => setTagFilter(e.target.value)}
                className="bg-surface border border-cohesity-border text-[13px] text-ink rounded-lg px-2 py-1.5 focus:border-brand/60 max-w-[200px]">
