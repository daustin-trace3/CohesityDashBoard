// UniFi Topology — port of frontend/src/pages/unifi/UnifiTopologyPage.jsx.
// icons.jsx has no ZoomIn/ZoomOut/Maximize2/Search — the zoom/fit controls use
// text glyphs and RotateCcw (reset) instead; the search input has no icon.
// Report loudly per contract: these are workarounds, not invented ui exports.
import { PageHeader, LoadingPanel, RefreshButton, LastUpdated, apiFetch, fmtWhen, typeLabel, BRAND } from '../ui.jsx';
import { Share2, RotateCcw, X } from '../icons.jsx';
...
export default function TopologyPage() {
  ...
  const [clientFilter, setClientFilter] = React.useState('all'); // all | wired | wireless
  const [typeFilter, setTypeFilter] = React.useState(() => new Set(TYPE_ORDER));
  ...
  const load = React.useCallback(() => apiFetch('/unifi/topology')
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ vertices: [], edges: [], deviceMeta: {}, clientMeta: {} })), []);
  ...
  const totalH = React.useMemo(() => (tree ? layoutTree(tree) : 0), [tree]);
  ...
  const visibleMacs = React.useMemo(() => new Set(visibleNodes.map((n) => n.mac)), [visibleNodes]);
  ...
  const edgesToDraw = React.useMemo(() => {
    if (!tree) return [];
    const list = [];
    (function walk(node) {
          ...
          <div className="flex-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search nodes…"
                className="bg-surface-overlay border border-cohesity-border rounded-lg px-3 py-1.5 text-sm text-ink focus:border-brand/60 outline-none w-48" />
              ...
              <div className="flex items-center gap-1 ml-auto">
                <button onClick={() => zoomBy(1.25)} title="Zoom in" className="flex items-center justify-center h-7 w-7 rounded-md border border-cohesity-border text-ink-muted hover:text-ink cursor-pointer font-semibold text-sm">+</button>
                <button onClick={() => zoomBy(0.8)} title="Zoom out" className="flex items-center justify-center h-7 w-7 rounded-md border border-cohesity-border text-ink-muted hover:text-ink cursor-pointer font-semibold text-sm">−</button>
                <button onClick={fitView} title="Fit to view" className="flex items-center justify-center h-7 w-7 rounded-md border border-cohesity-border text-ink-muted hover:text-ink cursor-pointer text-[10px] font-semibold">FIT</button>
            ...
            <div className="w-72 shrink-0 panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
              ...
              {selected.isClient ? (
                <div className="flex flex-col gap-2 text-xs">
                  ...
                  <ReactRouterDOM.Link to={`/unifi/clients?q=${encodeURIComponent(clientQuery(selected))}`} className="text-brand text-xs underline mt-2">View in Clients</ReactRouterDOM.Link>
                </div>
              ) : (
                <div className="flex flex-col gap-2 text-xs">
                  ...
                  <ReactRouterDOM.Link to={`/unifi/devices?q=${encodeURIComponent(nodeLabel(selected))}`} className="text-brand text-xs underline mt-2">View in Devices</ReactRouterDOM.Link>
                </div>
              )}
            </div>
          )}
