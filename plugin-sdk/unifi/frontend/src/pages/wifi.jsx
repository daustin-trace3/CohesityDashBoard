// UniFi WiFi — port of frontend/src/pages/unifi/UnifiWifiPage.jsx onto the
// plugin ui/charts/icons kit. React/ReactRouterDOM/Chart come from build
// globals; no react/react-chartjs-2/lucide-react imports.
import {
  PageHeader, Badge, LoadingPanel, RefreshButton, LastUpdated,
  useTableControls, SortTh, TableControls, TablePager,
  apiFetch, fmtNum, BRAND,
} from '../ui.jsx';
import { LineChart } from '../charts.jsx';
import { Wifi, Radio, ShieldAlert, Activity, Lock, Repeat } from '../icons.jsx';
...
function TrafficCharts({ history, hours, setHours }) {
  ...
  const siteSources = React.useMemo(() => [...new Set(site.map((h) => h.sourceId))], [site]);

  const bytesChart = React.useMemo(() => {
    const times = [...new Set(site.map((h) => h.time))].sort((a, b) => a - b);
    return {
      labels: siteLabels,
          ...
          {site.length > 0 && (
            <>
              <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                ...
                <LineChart data={bytesChart} options={bytesOpts} height={160} />
              </div>
              <div className="panel p-4" style={{ borderTop: `3px solid ${BRAND}` }}>
                <p className="text-xs font-semibold text-ink mb-2">Wireless Clients</p>
                <LineChart data={staChart} options={chartOpts} height={160} />
              </div>
            </>
          )}
          {aps.length > 0 && (
            <div className="panel p-4 lg:col-span-2" style={{ borderTop: `3px solid ${BRAND}` }}>
              ...
              <LineChart data={apChart} options={chartOpts} height={176} />
            </div>
          )}
        </div>
      )}
...
export default function WifiPage() {
  ...
  const load = React.useCallback((h) => apiFetch(`/unifi/wifi?hours=${h ?? hours}`)
    .then((json) => { setData(json); setLastRefreshed(new Date()); })
    .catch(() => setData({ wlans: [], radios: [], rogues: [], signalBuckets: {}, roaming: [], history: {} })), [hours]);

  React.useEffect(() => { load(hours); }, [hours]); // eslint-disable-line react-hooks/exhaustive-deps
