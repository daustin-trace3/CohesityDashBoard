...
export default function AriaSettingsPage() {
  ...
  const [form, setForm] = useState({ name: '', host: '', username: '', password: '', domain: '', sslVerify: false, pollingIntervalMinutes: 15 });
  const [saving, setSaving] = useState(false);
  const [testing, setTesting] = useState(false);
  const [testResult, setTestResult] = useState(null);
  const [refreshingId, setRefreshingId] = useState(null);
  const [probeInstance, setProbeInstance] = useState(null);
  const [leaseWarnDays, setLeaseWarnDays] = useState('');
  const [certWarnDays, setCertWarnDays] = useState('');
  const [requestFailLookbackHours, setRequestFailLookbackHours] = useState('');
  const [savingConfig, setSavingConfig] = useState(false);

  const loadInstances = () => client.get('/aria/instances')
    .then(({ data }) => setInstances(data))
    .catch(() => setInstances([]));

  useEffect(() => {
    loadInstances();
    client.get('/aria/config')
      .then(({ data }) => {
        setLeaseWarnDays(String(data.leaseWarnDays));
        setCertWarnDays(String(data.certWarnDays));
        setRequestFailLookbackHours(String(data.requestFailLookbackHours));
      })
  ...
  const saveConfig = async () => {
    ...
    try {
      const { data } = await client.put('/aria/config', {
      ...
      toast({ type: 'error', title: 'Save failed', message: err?.response?.data?.error || 'Check the threshold values are within range.' });
    } finally {
  ...
  const test = async () => {
    ...
    setTestResult(null);
    try {
      const { data } = await client.post('/aria/instances/test', {
  ...
  const add = async () => {
    ...
    try {
      ...
      if (editingId) {
        ...
        await client.put(`/aria/instances/${editingId}`, body);
        toast({ type: 'success', title: 'Aria instance updated', message: form.password ? 'Credentials replaced — next poll uses them.' : 'Saved. Stored password unchanged.' });
      } else {
        body.password = form.password;
        await client.post('/aria/instances', body);
        toast({ type: 'success', title: 'Aria instance registered', message: 'First poll started — data appears shortly.' });
      }
      setEditingId(null);
      blankForm();
      await loadInstances();
  ...
  const remove = async (o) => {
    ...
    try {
      await client.delete(`/aria/instances/${o.id}`);
      await loadInstances();
  ...
  const refresh = async (o) => {
    ...
    try {
      await client.post(`/aria/instances/${o.id}/refresh`, {}, { timeout: 300000 });
      await loadInstances();
